...
- `ctx.ui.layout.split(spec)` returns the Core `SplitController`. Its `stateSnapshot()` distinguishes `preferredSize`/`preferredRatio` and placement/collapse intent from the current `effectiveSize`. Viewport or orientation clamping never overwrites the stored preference.
- Native Mobile split ceilings are resolved by the same Core controller. A plugin must not replace `limits`, preview or commit methods at runtime.
- Workspace regions declare `chain`, `contain`, `viewport` or `none`. `layoutDiagnostics({limit})` examines only registered regions on request and performs no recovery writes; fix reported overflow at the plugin-owned semantic container.
