...
4. **Visual ownership is release-gated.** Foundation is reset-only, Structure owns geometry only, Presentation owns component paint, Theme supplies tokens/material recipes, and first-party plugin CSS owns domain layout/state without application colors. Keep these boundaries rather than adding late override blocks.
