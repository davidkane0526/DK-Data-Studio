...
Studio UI, scripts, the built-in AI agent and MCP share the Plugin Kernel domain-command registry. Use `commands.list` / `commands.get` to discover explicitly exposed actions, `commands.run` to execute them, `commands.history` to inspect bounded execution provenance, and `commands.replay` for opt-in replayable commands. Replay checks recorded input Artifact revisions by default. UI-only legacy commands are intentionally not exposed through this discovery surface.
...
The machine-readable authoring corpus includes SDK 1.40.0 / Plugin API 1.19.0 / Theme Contract 3.10.0 contract JSON, manifest schema, TypeScript API definitions, UI/workspace/data-model guides and official plugin templates.
