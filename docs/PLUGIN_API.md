...
For `pluginType:"workbench"`, declaring a non-empty `data.accepts` list also makes the dedicated window a live Artifact consumer by default. Core sends the current Artifact snapshot/revision and applies the normal assignment scope before plugin runtime creation. `window.artifactHydration` may still override this default explicitly.
