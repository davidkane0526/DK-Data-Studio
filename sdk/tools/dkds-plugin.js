...
function referencedFiles(manifest,folder){
  const set=new Set([manifest.entry,...(manifest.scripts||[]),...(manifest.styles||[]),...PlatformPresentation.referencedPlatformAssets(manifest),...(manifest.window?.runtime?[manifest.window.runtime]:[]),...(manifest.window?.scripts||[]),...(manifest.tasks||[]).flatMap(row=>[row?.entry,...(row?.imports||[])]).filter(Boolean)]);
  if(fs.existsSync(path.join(folder,'README.md')))set.add('README.md');
  return [...set].map(relFile);
}
const usage=[
  ['runtime',/ctx\.runtime\b/],['events',/ctx\.events\b/],['status',/ctx\.status\b/],['io',/ctx\.io\b/],['science',/ctx\.science\b/],['performance',/ctx\.performance\b/],['execution.tasks',/ctx\.tasks\b/],['execution.commands',/ctx\.commands\.(?:list|history|replay)\b|domainCommand\s*:/],
...
const forbidden=[
  [/\bctx\.host\b/,'private host bypass'],[/new\s+(?:Worker|SharedWorker)\s*\(/,'private Worker pool (use ctx.tasks)'],[/window\.electronAPI|\belectronAPI\./,'Electron bridge'],[/window\.Plotly|\bPlotly\./,'raw Plotly'],
...
async function validate(folder){
  ...
  if(m.tasks!==undefined){if(!Array.isArray(m.tasks)||!m.tasks.length)errors.push('tasks must be a non-empty array when declared');const ids=new Set();for(const row of (Array.isArray(m.tasks)?m.tasks:[])){const id=String(row?.id||'').trim();if(!pluginId(id)||ids.has(id))errors.push(`invalid or duplicate task id: ${id||'(missing)'}`);ids.add(id);try{relFile(row?.entry);if(row?.imports!==undefined&&!Array.isArray(row.imports))throw new Error('imports must be an array');for(const file of row?.imports||[])relFile(file);}catch(e){errors.push(`invalid task entry/import: ${e.message}`);}}if(!(m.requiresCore||[]).includes('execution.tasks'))errors.push('tasks requires execution.tasks in requiresCore');}
