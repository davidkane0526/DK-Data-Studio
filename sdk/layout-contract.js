...
function inspectWorkspaceStyles({apiVersion,pluginType,workspace,ui={},styles=[]}={}){
  ...
  for(const style of styles){
    ...
    for(const rule of cssRules(css)){
      ...
      if(layoutRelevant&&overflowHidden&&semanticTarget)errors.push(`${name}: "${selector}" clips semantic UI with overflow:hidden/clip. Plugin API 1.19 requires visible overflow or scrolling; Core provides the final safety net.`);
