(() => {
  ...
  DKDSPlugins.define(manifest, async ctx => {
    ...
    workspace.mountPrimary({id:'main',label:'主界面',scroll:'contained',mount:({main})=>{const shell=ctx.ui.dom.create('div',{className:'sdk-tool-main'}),plot=ctx.ui.dom.create('div',{className:'sdk-tool-plot'});ctx.ui.dom.append(shell,plot);ctx.ui.dom.append(main,shell);surface=ctx.ui.scientificPlot.create(plot,{minHeight:200,xTitle:'X',yTitle:'Y',getCurves:()=>[],getMarkers:()=>[]});return()=>{surface?.dispose?.();surface=null;};}});
