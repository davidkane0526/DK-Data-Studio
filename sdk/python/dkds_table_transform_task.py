...
def analyze_table_transform_execution(plan:dict[str,Any])->dict[str,Any]:
    ...
    if not isinstance(plan,dict) or plan.get("schema")!="dkds.table-transform-plan.v1":
        ...
        return {"schema":EXECUTION_SCHEMA,"executable":False,"diagnostics":diagnostics,"sourceInputs":[],"resultSymbols":[]}
    ...
    for op in plan.get("operations",[]):
        ...
        if kind=="source.table":
            ...
            if not output:
                ...
                symbols.add(output);sources.append(output)
            continue
        inputs=[]
        ...
        output=str(op.get("output",""))
        if output:
            if output in symbols and kind!="value.alias":
                diagnostics.append(_diag(op,"TABLE_TASK_SYMBOL_REDEFINED",f"Symbol {output} is defined more than once."))
            symbols.add(output)
        ...
        if kind=="table.concat" and int(op.get("axis",0)) not in {0,1}:
