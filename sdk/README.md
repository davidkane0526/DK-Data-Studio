# DK Data Studio Plugin SDK 1.51.82
SDK 1.51.82 adds bounded **`scipy.optimize.curve_fit`** authoring without shipping SciPy or Python. The planner proves the SciPy import/alias, accepts only optimal-parameter extraction through `curve_fit(...)[0]` or `popt, _ = curve_fit(...)`, and lowers 1D numeric Series/Array `xdata` + `ydata` into one `fit.curve` Core Task operation. Fit models must be unique top-level synchronous functions or inline lambdas that compile through the shared pure scalar callback lowerer. Proven NumPy aliases may use the approved scalar math subset such as `np.exp`, which lowers to `Math.exp`; no NumPy runtime is shipped. Runtime fitting uses a bounded numerical-Jacobian Levenberg-Marquardt-style solver, supports 1–6 fit parameters and optional literal finite `p0`, and returns `popt` through the existing bounded `array.vector` / Table Unit projection. `pcov`, bounds, sigma, jac, method, nan-policy and other advanced SciPy semantics remain fail-closed. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.81 adds bounded numeric **Series.apply()** without introducing a Python runtime. The callback must be one unique top-level synchronous function or an inline lambda that the shared portable-expression lowerer can prove pure and expression-only. Named callbacks may use one `return` expression, scalar literal defaults, approved pure builtins, and the existing `math.*` whitelist; `args=(...)` may supply bounded scalar literal positional extras. Authoring compiles the callback into JavaScript and the ordinary Core Task maps one bounded numeric Series while preserving its index and metadata. Missing numeric cells enter the callback as `NaN`; an unhandled `NaN` maps back to `null`, while non-numeric outputs and non-finite outputs from finite inputs fail. `Series.apply()` in this stage is strictly per-element and exposes no DataFrame-style axis semantics. `DataFrame.apply`, GroupBy.apply, closures/free names, multi-statement callbacks, arbitrary Python callables and runtime Python remain fail-closed. No new Unit, Presenter/UI contract or alternate runtime is added. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.80 maps bounded **DataFrame.to_csv() Host Export** onto the existing Plugin API I/O boundary. A literal non-empty `path_or_buf` is required, but its directory is never runtime filesystem authority: DKDS keeps only the basename as the save-dialog suggestion and writes only through `ctx.io.saveText()` after the JavaScript Core Task returns. The generated action explicitly owns one `nativeSave:'export'` intent, and the source-authored directory path is not packaged into runtime output. Bounded options cover `sep`, `na_rep`, boolean `header/index`, literal `columns`, UTF-8/UTF-8-SIG encoding and LF/CRLF line terminators. Internal DataFrame indexes remain available through detached `rawTables` so GroupBy index semantics can be serialized without changing canonical DataTable publication. One workflow action may own one CSV export. Pathless `to_csv()`, `to_excel()`, compression/append/custom formatter options and direct filesystem writes remain fail-closed. The compute Worker contains no `ctx.io`, Electron or filesystem access. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.79 adds a bounded **NumPy-shaped 1D numeric Array IR** to the existing Python/Jupyter Table Transform/Core Task path without shipping NumPy. Internal `array.vector` values are capped at 65,536 items and can be created from numeric literal `np.array/asarray`, Series `.to_numpy()/.values`, `np.linspace`, and `np.arange`. The first executable slice covers one-dimensional literal indexing/slicing, unary `+/-`, `np.abs/sqrt/log/log10/exp`, elementwise `+ - * / ** %` with scalar broadcasting or equal-length arrays only, `np.diff`, and NumPy `mean/median/std` with bounded axis/ddof validation. Terminal Arrays reuse the existing Table Unit as Index/Value rows; scalar reductions/items reuse Metric. 2D arrays, generalized broadcasting, masks, arbitrary ufuncs, SciPy and file export remain fail-closed. No new Unit, Presenter/UI contract, Python runtime or NumPy runtime is added. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.78 adds bounded **GroupBy + aggregate** semantics to the existing Python/Jupyter Table Transform/Core Task path. `DataFrame.groupby()` accepts one literal column label or a non-empty literal list/tuple of labels plus literal `sort`, `dropna`, and `as_index` booleans. DataFrameGroupBy/SeriesGroupBy `mean()/median()/std()` execute inside the ordinary JavaScript Core Task runtime; `agg()/aggregate()` accepts one reducer string or a DataFrameGroupBy literal `column -> reducer` mapping using `mean|median|std`. Grouped single-column selection becomes an internal SeriesGroupBy value. Grouped indexes stay internal and are materialized as canonical DKDS DataTable key columns only when leaving the Task; `as_index=False` emits key columns directly. Terminal GroupBy objects, multiple reducers/MultiIndex columns, named aggregation, lambdas/custom callables, NumPy and file export remain fail-closed. No new Unit, Presenter/UI contract or Python/Pandas runtime is added. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.77 adds bounded **Series + mean / median / std** semantics to the existing Python/Jupyter Table Transform/Core Task path. Literal single-column DataFrame selection (for example `df["signal"]`) lowers to internal `table.series`; direct chained aggregates lower through the same Series selector. Series `mean()/median()/std()` return scalars, while DataFrame `mean()/median()/std()` return Series for bounded `axis=0/index` or `axis=1/columns` with literal `skipna`, `numeric_only`, and integer `std(ddof=...)` validation. Terminal Series/scalars reuse existing Table/Metric Unit projections; terminal DataFrames retain canonical Artifact Store publication. No new Unit, Presenter/UI contract, Python/Pandas runtime, GroupBy, NumPy Array IR, or file-export path is added. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.76 restores the first notebook **Host effects without contaminating compute Tasks**. A mapped `DataFrame.plot()` produces a public Unit ScientificPlot projection from the Task's detached table result, while `DataFrame.to_clipboard()` serializes that result through `ctx.io.clipboard.writeText`. Host-required intermediate tables are retained as Task outputs only when needed by a mapped effect. The generated Worker code contains no UI/clipboard/Store/Electron/Python/Pandas access. `to_csv/to_excel`, unsupported plot options, `loc`, and aggregate Series semantics remain fail-closed with exact cell/line diagnostics. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.75 closes **explicit multi-source workflow binding** without adding a SourcePicker-specific Unit. The existing Field Unit now exposes generic `setOptions(...)`; generated workflow source selectors are ordinary Unit select Fields populated from `ctx.data.sources.list()`. Literal `read_csv/read_excel` filenames survive as non-authoritative hints. A picker auto-selects only when the scoped source set has one member or the hint uniquely matches `name/sourceName/path/sourcePath`; otherwise it remains unselected and task execution fails until the user chooses. `artifact-table` / `artifact-column` bindings can reference `sourceField`, so replay/canonical source identity uses artifact IDs rather than source-array position. Unit Templates remain 2.5.38 (same 41 Units / 73 recipes); this is an additive handle/API capability, not a new Unit or geometry contract.

SDK 1.51.74 makes the first **script-shaped / no-function workflow** buildable as a normal plugin. When a workflow has exactly one scoped DataTable input and its Table Transform execution report is closed, Authoring exposes `workflow:table-transform` beside ordinary function candidates. The generated package reuses the same `PluginBuilder.add_compiled_task()` registration path, bounded `artifact-table` hydration, Core Task Runner, canonical DataTable publication with lineage, package validator, export and install transaction. Result column structure may remain runtime-derived and is validated before canonical publication. Multi-source workflows remain blocked until a runtime Source Picker can bind each source explicitly; source list index is never used as a silent multi-file guess. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.73 makes the first Table Transform subset **actually executable**. A closed `dkds.table-transform-plan.v1` can lower into an ordinary JavaScript `DKDSTaskDefinition` through `dkds.table-transform-execution.v1`; the Task consumes only detached 1.51.71 `artifact-table` snapshots and has no Store, Host, Python, Pandas, or notebook-kernel access. Executable v1 covers positional `iloc`, `abs`, `copy`, `reset_index`, `diff(axis=0)`, `dropna(axis=0)`, `sort_index(axis=0)`, and `concat(axis=0/1)`. `loc`, aggregate Series results (`mean/median/std`), and Host effects (`plot/clipboard/export`) remain exact blockers until their semantics are mapped. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.72 introduces **Table Transform IR v1** for real Python/Jupyter workflows. Static authoring now structures a conservative first set of Pandas-shaped statements—DataTable source replacement for `read_csv/read_excel`, static `iloc/loc` slices, `abs/copy/reset_index/diff/dropna/sort_index/mean/median/std`, bounded `pd.concat`, plus plot/clipboard/export host operations. Every unlowered statement remains an exact cell/line blocker. The IR is an authoring plan only at this stage: it does not execute Pandas and is not yet itself a runtime backend. The 1.51.71 bounded `artifact-table` input is the shared runtime substrate for the next executable lowering step. Plugin API remains 1.19.0; Unit Templates remain 2.5.38.

SDK 1.51.71 adds **bounded whole-DataTable task input** for generated plugins. An `artifact-table` binding selects one scoped `data.table`, enumerates only `columnMetadata()`, enforces author-declared `maxRows` and `maxColumns`, and hydrates each column only with bounded `readColumnRange()`. The task receives a detached structured-cloneable `{kind:'data.table', artifactId, columns, rowCount}` snapshot; generated code does not call full `artifacts.get()`. This is the common input contract for the next Table Transform IR stage and does not add a Python/Pandas runtime. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.70 advances real Python/Jupyter ingestion from function discovery to a **static Source Workflow graph**. `dkds.python-source-model.v2` records code-cell definitions/reads, cross-cell dependencies, imports, roles and classified effects without executing the notebook. Calls such as `read_csv/read_excel`, `plot`, `to_clipboard`, and `to_csv/to_excel` are identified as DKDS host replacements; Pandas/NumPy/SciPy use is reported separately as remaining table/array/scientific transform lowering. This is deliberately fail-closed: a script-like notebook is not marked buildable merely because its I/O can be mapped. Pure portable functions continue to use the existing JavaScript Task lowering and production Plugin Manager install transaction. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.69 adds the product-facing **Python / Jupyter Source Import authoring pipeline**. `.py` files and `.ipynb` code cells are statically parsed into one `dkds.python-source-model.v1`; top-level function signatures, defaults, annotations and inferred result shapes feed the existing fail-closed Portable Python compiler. Compatibility errors retain source line or notebook cell/line coordinates. A buildable function receives one bounded `dkds.declarative-blueprint.v1`, can be previewed in Desktop Plugin Manager, built and validated through the existing production package contracts, exported as `.dkplugin`, or installed through the existing Plugin Manager transaction. User source is never executed; Python 3 is required only by the desktop authoring host for standard-library AST parsing, and generated plugins ship JavaScript tasks with no Python/Jupyter runtime dependency. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.68 connects the five fixed **Resonance declarative Inspector operation buttons** to generic domain-bound Surface Actions. Generated buttons use the existing public Action/Layout Units, read `selectedPeak.id` only for enablement, and invoke the dependency-scoped live Domain Adapter; that adapter delegates to the existing production Inspector mutation owner for accept/lock toggle, FWHM automatic-window reset, deletion and source-sweep selection. Dynamic category palette/label editing remains production-owned. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.67 freezes **Resonance Inspector read-only reconstruction parity** without expanding the Unit vocabulary. The accepted production Inspector and the Domain Adapter now share one `inspector-detail-projection.js`; the declarative specimen renders `inspector.title` through Status and `inspector.rows` through the existing public Table live binding. Editing, FWHM-window mutation and auxiliary transform-chart behavior remain production-owned. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.66 adds bounded **live marker-array ScientificPlot projection**. Marker visibility/meaning must already be resolved by the Domain Adapter; the generator only maps declared marker fields, projects the authoritative selected marker id through public `getSelectedMarkerIds()`, and routes marker selection through one declared adapter action. Resonance now shares one `main-marker-projection.js` between its accepted production main plot and declarative adapter, so showPoints/showRejected/visible-sweep/shape semantics are not duplicated. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.65 adds authoritative selected-curve **focus projection** to live curve-array ScientificPlot bindings. `selectedIdPath` reads the selected id from the declared Domain Adapter snapshot and feeds the existing public `getSelectedCurveId()` ScientificCurveSurface contract. The generated string is presentation-only and refreshed with the same snapshot pass; it is not a second selection model or bus. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.64 extends live curve-array ScientificPlot projection with one bounded interaction: optional `selectAction`. A curve click forwards only `{id: curve.id}` to the declared Domain Adapter action; the generated shell stores no selected-sweep/curve state and does not interpret domain fields. Selection semantics, linked views and inspector behavior remain owned by the production domain service. Plugin API remains 1.19.0; Unit Templates remain frozen at 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.63 adds bounded **live curve-array ScientificPlot projection**. A Unit-owned curve plot can consume one array from the declared Domain Adapter snapshot and map only declared curve/point keys into the existing public ScientificPlot contract. The generator keeps no scientific/domain state: visibility and analysis remain with the production owner, while the generated array is only a detached render projection refreshed through the shared live-binding pass. Runtime-owned/non-curve plots and duplicate static points fail closed. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.62 adds bounded **two-way live Domain Adapter bindings** to the canonical parameter PRIME. Number/Text/Select/Checkbox fields can read one authoritative snapshot path and invoke one declared adapter action with a dynamic value plus bounded static scalar arguments. Updates use only public Unit `onChange` contracts and the existing shared snapshot refresh pass; generated code owns no duplicate domain state. Plugin API remains 1.19.0 and Unit Templates remain frozen at 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.61 adds read-only live Domain Adapter snapshot bindings for public Status, Metric, Field/Check and Table Unit handles. One shared refresh reads one authoritative snapshot and updates all bound Units through their public handles; no bound node owns a second state copy. Summary is deliberately excluded until its public Unit handle exposes an update contract. Unit Templates remain 2.5.38 with 41 Units / 73 recipes; Plugin API remains 1.19.0.

SDK 1.51.60 adds dependency-scoped declarative Domain Adapter consumption. Generated plugins may declare one `domainAdapter` reference and bind PRIME `choice-menu` actions with `domainAction + statePath`; the menu reads `liveDomain.snapshot()` and invokes the same production owner through `liveDomain.invoke()`. The previous `commandId` + Core command-history path remains valid. No second state store, adapter runtime, Unit, Layout recipe, or Presenter branch is introduced. Unit Templates remain 2.5.38 / 41 Units / 73 recipes; Plugin API remains 1.19.0.

SDK 1.51.59 closes the remaining Resonance-shaped local header-action gap through the existing Core ActionGroup contract. Generic PRIME surfaces may declare a bounded `choice-menu`: the menu reuses the canonical Header action host, reads its selected value from Core domain-command history, invokes exactly one declared command, and re-renders after completion. It does not create a plugin-local preference store, private menu runtime, new Unit, new Layout recipe, or Presenter branch. Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes; Plugin API remains 1.19.0.

SDK 1.51.58 aligns declarative ParameterForm `layoutOwner` with the public `core | host` contract and extends the generic **PRIME/SUB Surface IR** with bounded non-parameter PRIME detail geometry, command-reference lifecycle adapters, Status, direct Action, FloatingChrome and runtime-owned ScientificPlot children. The Resonance-shaped reconstruction specimen now expresses two movable PRIME surfaces and three scientific SUB views through the same Workbench/Unit path; lifecycle behavior is referenced by command id through Plugin API 1.19 rather than embedded JavaScript. The remaining Resonance-specific structural gap is the dynamic local group-columns menu/action model. Unit Templates remain frozen at 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.57 completes the first **TER-shaped declarative reconstruction gate** on the frozen public platform. PlotGroup children can now declare bounded placements and accepted detailGeometry that is executed by the existing PlotView owner; the reference reconstruction combines the canonical titleless parameter PRIME, nested ParameterForm, seven runtime-owned scientific plots, and two result tables without private CSS or a TER-specific Unit. SDK 1.51.56 added generic non-parameter PRIME/SUB composition from bounded recursive public Unit children, while SDK 1.51.55 added nested ParameterForm composition inside grouped parameter controls. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.54 adds the third **production-shaped scientific blueprint parity** gate. The generic authoring layer now composes the existing Summary, retained List, EmptyState and portable PlotView Units in generated hosted scientific workbenches, together with ScientificPlot, Table and the titleless data-control PRIME. The accepted Pulse Analysis blueprint is independent evidence only; no Pulse-specific id branch, batch/segmentation state, private DOM/CSS, Presenter path or alternate runtime is generated. Plugin API remains 1.19.0; Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.53 adds the second independent **production-shaped blueprint reconstruction**. The declarative/Python layer now expresses public Tool-designer composition used by the accepted Pulse Sampler structure: bounded Workspace left-rail geometry, content end inset, explicit titleless PRIME priority/embedded/auto-open policy, compact Tabs, separate field-layout recipes, Action Grid, ordinary/segment Toolbar and a parameter-owned Table. Pulse Sampler is evidence only; generated source contains no Pulse/plugin-id branch, copied domain state, waveform algorithm, private CSS, alternate Presenter or second runtime. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.52 extends Phase F **production-shaped reconstruction to grouped control surfaces** without modifying production plugins or Unit Templates. A titleless data-control PRIME can now declaratively compose multiple headed/plain Panels using the existing Header, Chip, Field, Check, Note and Toolbar Units; PageHeader and group Toolbars select actions from one validated action catalog with icon/order metadata. Hosted TOP/Tool pages may opt into the existing workspace close lifecycle, while standalone output emits no workspace-only close code. The Transfer Vth blueprint remains independent acceptance evidence only. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.51 adds the first **production-shaped Unit blueprint reconstruction** to Phase F without modifying a production plugin. The generic declarative/Python layer can now compose a `fill-rows` PRIMARY, canonical Metric grid, fill-capable ScientificPlot Panel and Core-owned resizable SplitPane/Table result region, while one lowered JavaScript Task projects Plot + Table + Metric results together. The accepted Transfer Vth blueprint is used only as independent reconstruction evidence; generated source contains no Vth/plugin-id branch, private CSS, alternate resizer or copied domain runtime. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.50 extends Phase F generation to **hosted TOP / Tool workspaces** using the existing Plugin API lifecycle. A bounded `host` declaration may keep the generated plugin standalone or select TOP/Tool hosting; hosted output emits the normal manifest `workspace`/`window` contract, registers `ctx.ui.activities`, `ctx.ui.pages` and `ctx.ui.topWorkspace`, derives `scientific-renderer` for plotted dedicated windows, and mirrors generated parameter PRIME metadata into the canonical data-control presentation. No private window runtime, host fork, Presenter branch or specialized Unit is generated. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.49 closes Phase F **scientific interaction + replay-safe command generation** on the existing runtime. Generated plots may declare stable series identity, reference-only selection, dimension/unit-checked viewport links and bounded legend links through `ctx.ui.interaction`; generated portable Tasks may register validated domain commands with replay metadata and stale-input rejection through `ctx.commands`. No alternate Interaction Hub, command bus, renderer, Worker pool, Store, Python runtime or Presenter path is introduced. Plugin API remains 1.19.0 and Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.48 extends Phase F generation to **multi-source, multi-result scientific workbenches** while reusing the frozen runtime. Declarative content can compose the existing Unit PlotGroup/PlotView/ScientificPlot stack; one lowered JavaScript Task may consume multiple independently scoped bounded DataTables, update multiple generated plots/tables, and publish multiple declared canonical DataTable outputs with complete source lineage. The generator still creates no Python runtime, private Worker pool, alternate Store, private scientific renderer, Presenter branch, or specialized Unit. Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.

SDK 1.51.47 extends Phase F generation to **bounded Artifact/DataTable pipelines** while keeping the application runtime JavaScript-only. Generated actions consume only the plugin-scoped `ctx.data.sources` catalog, resolve columns with `columnMetadata()`, read explicitly bounded payloads through `readColumnRange()` (`maxRows <= 65536`), execute the lowered JavaScript task through `ctx.tasks`, and may project results into Unit Table/ScientificPlot surfaces or publish a declared canonical DataTable with lineage. No Python provider, fallback, interpreter or second data backend is added. Unit Templates remain 2.5.38 with 41 Units / 73 Layout recipes.
