(function(root,factory){
  ...
  function inspectDataText(file,options={}){
    ...
    let maxCols=0;for(const row of tokenRows)if(row.length>maxCols)maxCols=row.length;
    ...
    const columns=Array.from({length:maxCols},(_,c)=>{
      ...
      let finiteCount=0,min=Infinity,max=-Infinity;for(const value of vals)if(Number.isFinite(value)){finiteCount++;if(value<min)min=value;if(value>max)max=value;}
      return {
        index:c,
        header:headers[c],
        finiteCount,
        numericFraction:numericRows.length?finiteCount/numericRows.length:0,
  ...
  function createNumericTableStream(file,options={},inspection=null){
    const opt=normalizeImportOptions(options),ins=inspection&&Array.isArray(inspection.headers)?inspection:null;
    if(!ins)throw new Error('Streaming numeric-table import requires a completed preview inspection.');
    const delimiter=opt.delimiter==='auto'?ins.delimiter:opt.delimiter;
    const dataStartSourceLine=Math.max(opt.skipRows+1,Number(ins.dataStartSourceLine)||opt.skipRows+1),width=ins.headers.length;
    const columns=Array.from({length:width},()=>[]),sourceLines=[];let lastSourceLine=0;
    async function pushLines(lines,meta={}){
      const rows=Array.isArray(lines)?lines:[],startLine=Math.max(1,Math.floor(Number(meta.startLine)||1));
      for(let index=0;index<rows.length;index++){
        const sourceLine=startLine+index;lastSourceLine=Math.max(lastSourceLine,sourceLine);
        if(sourceLine<dataStartSourceLine||sourceLine<=opt.skipRows)continue;if(opt.endRow>0&&sourceLine>opt.endRow)continue;
        const line=rows[index];if(lineIsComment(line,opt.commentPrefix))continue;const cells=parseDelimitedLine(line,delimiter);if(cells.length<2)continue;
        const values=Array.from({length:width},(_,c)=>numericCell(cells[c],opt.decimalSeparator));if(values.filter(Number.isFinite).length<2)continue;
        for(let c=0;c<width;c++)columns[c].push(values[c]);sourceLines.push(sourceLine);
      }
    }
    function finish(meta={}){return {headers:ins.headers.slice(),columns,sourceLines,rowCount:sourceLines.length,inspection:{...ins,streamed:true,rowCount:sourceLines.length,sourceLineCount:Number(meta?.lineCount)||lastSourceLine}};}
    return Object.freeze({pushLines,finish,snapshot:()=>Object.freeze({delimiter,width,rowCount:sourceLines.length,lastSourceLine})});
  }

  function createFlexibleDataStream(file,options={},inspection=null){
    const opt=normalizeImportOptions(options);
    const ins=inspection&&Array.isArray(inspection.headers)?inspection:null;
    if(!ins)throw new Error('Streaming flexible import requires a completed preview inspection.');
    const layout=opt.layout==='auto'?ins.suggestedLayout:opt.layout;
    const xCol=Number.isFinite(opt.xCol)?opt.xCol:ins.suggestedX;
    let yCols=opt.yCols.length?opt.yCols.slice():ins.suggestedYCols.slice();
    const delimiter=opt.delimiter==='auto'?ins.delimiter:opt.delimiter;
    const baseName=String(file?.name||'data').replace(/\.[^.]+$/,'');
    const dataStartSourceLine=Math.max(opt.skipRows+1,Number(ins.dataStartSourceLine)||opt.skipRows+1);
    const specs=[];
    function addSeries(xc,yc,seriesIndex,labelOverride=''){
      if(xc<0||yc<0||xc>=ins.headers.length||yc>=ins.headers.length||xc===yc)return;
      const xHeader=ins.headers[xc]||`Col ${xc+1}`;
      const yHeader=ins.headers[yc]||`Col ${yc+1}`;
      const xf=selectedUnitFactor(opt.voltageUnit,xHeader,'voltage');
      ...
      const vg=vgForImportedSeries(file,seriesLabel,opt,yc);
      const suffix=layout==='single'?'':` · ${seriesLabel}`;
      specs.push({xc,yc,xf,yf,seriesIndex,seriesLabel,vg,xHeader,yHeader,name:`${baseName}${suffix}`,vValues:[],iValues:[],sourceLines:[]});
    }
    if(layout==='paired'){
      const start=Math.max(0,opt.pairStart||0);let n=0;
      for(let c=start;c+1<ins.headers.length;c+=2)addSeries(c,c+1,n++,`${ins.headers[c+1]||`I${n}`}`);
    }else if(layout==='sharedX'){
      yCols=[...new Set(yCols)].filter(c=>c!==xCol);
      yCols.forEach((yc,k)=>addSeries(xCol,yc,k,ins.headers[yc]));
    }else{
      const yc=Number.isFinite(opt.yCol)?opt.yCol:(yCols[0]??ins.suggestedYCols[0]??1);
      addSeries(xCol,yc,0,ins.headers[yc]);
    }
    if(!specs.length)throw new Error('当前列映射没有可导入的数据序列。');
    let consumedLines=0,acceptedRows=0,lastSourceLine=0;
    async function pushLines(lines,meta={}){
      const rows=Array.isArray(lines)?lines:[];
      const startLine=Math.max(1,Math.floor(Number(meta.startLine)||1));
      for(let index=0;index<rows.length;index++){
        const sourceLine=startLine+index;lastSourceLine=Math.max(lastSourceLine,sourceLine);consumedLines++;
        if(sourceLine<dataStartSourceLine||sourceLine<=opt.skipRows)continue;
        if(opt.endRow>0&&sourceLine>opt.endRow)continue;
        const line=rows[index];if(lineIsComment(line,opt.commentPrefix))continue;
        const cells=parseDelimitedLine(line,delimiter);if(cells.length<2)continue;
        let rowAccepted=false;
        for(const spec of specs){
          const xv=numericCell(cells[spec.xc],opt.decimalSeparator),yv=numericCell(cells[spec.yc],opt.decimalSeparator);
          if(!Number.isFinite(xv)||!Number.isFinite(yv))continue;
          spec.vValues.push(xv*spec.xf);spec.iValues.push(yv*spec.yf);spec.sourceLines.push(sourceLine);rowAccepted=true;
        }
        if(rowAccepted)acceptedRows++;
      }
    }
    function finish(meta={}){
      const datasets=[];
      for(const spec of specs){
        if(spec.vValues.length<3)continue;
        datasets.push({
          name:spec.name,
          path:`${file.path}::series::${spec.seriesIndex}::x${spec.xc+1}y${spec.yc+1}`,
          sourcePath:file.path,
          sourceName:file.name,
          text:String(file?.text||''),
          encoding:String(meta?.encoding||file?.encoding||opt.encoding||'auto'),
          vg:spec.vg,
          columnar:{v:spec.vValues,i:spec.iValues,sourceLine:spec.sourceLines},
          importSpec:{...opt,resolvedDelimiter:delimiter,resolvedLayout:layout,xCol:spec.xc,yCol:spec.yc,seriesLabel:spec.seriesLabel,xHeader:spec.xHeader,yHeader:spec.yHeader,vgOverride:Object.prototype.hasOwnProperty.call(opt.vgOverrides||{},String(spec.yc))?Number(opt.vgOverrides[String(spec.yc)]):null}
        });
      }
      return {datasets,inspection:{...ins,streamed:true,streamedAcceptedRows:acceptedRows,streamedSourceLineCount:Number(meta?.lineCount)||lastSourceLine||consumedLines}};
    }
    return Object.freeze({pushLines,finish,snapshot:()=>Object.freeze({layout,delimiter,series:specs.length,acceptedRows,consumedLines,lastSourceLine})});
  }
  ...
  return {IMPORT_ENCODING_LABELS,defaultImportOptions,normalizeImportOptions,inspectDataText,parseFlexibleData,createNumericTableStream,createFlexibleDataStream,parseVgFromImportHeader,parseCsv,parseVg};
});
