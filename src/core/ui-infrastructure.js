(() => {
  ...
  class ScientificCurveSurface {
    ...
    computeLegendLayout(width,height,curves,baseMargin){const options=this.legendOptions(),entries=this.legendEntries(curves);if(!options.enabled||entries.length<2)return {enabled:false,placement:'none',count:entries.length,rows:0,width:0,height:0,reserve:0,entries};const availableW=Math.max(120,width-(Number(baseMargin.left)||0)-(Number(baseMargin.right)||0)-12),entryWidths=entries.map(row=>Math.min(196,36+row.label.length*7)),total=entryWidths.reduce((a,b)=>a+b,0)+Math.max(0,entries.length-1)*6,rows=Math.max(1,Math.ceil(total/availableW)),horizontalHeight=rows*25+10,maxLabel=Math.max(...entryWidths,72),sideWidth=Math.min(Math.max(maxLabel+12,118),Math.max(118,width*.30));let placement=options.placement;if(!['top','bottom','right','left'].includes(placement)){const horizontalFits=rows<=options.maxRows&&horizontalHeight/Math.max(1,height)<=.25;placement=horizontalFits?'top':(rows<=options.maxRows+1&&horizontalHeight/Math.max(1,height)<=.31?'bottom':(width>=720?'right':'bottom'));}const horizontal=placement==='top'||placement==='bottom',reserve=horizontal?horizontalHeight:sideWidth+10;return {enabled:true,placement,count:entries.length,rows:horizontal?rows:entries.length,width:horizontal?availableW:sideWidth,height:horizontal?horizontalHeight:Math.min(height-(Number(baseMargin.top)||0)-(Number(baseMargin.bottom)||0),entries.length*27+8),reserve,entries};}
