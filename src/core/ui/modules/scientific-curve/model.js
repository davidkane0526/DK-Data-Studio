...
const {selectionReferences}=require('../selection/data-interaction');
  ...
  class ScientificCurveSurface {
    ...
    pointReference(curve,point,index=-1){
      const source=point?.raw??point,sourceIndex=Number.isInteger(Number(point?.sourceIndex))?Number(point.sourceIndex):Number(index);try{if(typeof this.spec.pointReference==='function'){const ref=this.spec.pointReference({curve,point:source,index:sourceIndex,surface:this});if(ref)return selectionReferences.normalize(ref);}}catch(err){console.warn('[DKDS ScientificCurveSurface pointReference]',err);}
      if(source?.ref){try{return selectionReferences.normalize(source.ref);}catch{return null;}}
      const baseRef=curve?.ref||{},artifactId=String(curve?.artifactId||baseRef?.artifactId||'').trim(),seriesId=String(curve?.seriesId||baseRef?.seriesId||'').trim(),rowId=String(curve?.rowIds?.[sourceIndex]??source?.rowId??source?.sourceRowId??(artifactId&&sourceIndex>=0?`row:${sourceIndex}`:'')).trim();if(!artifactId||!rowId)return null;
      try{return selectionReferences.row(artifactId,rowId,{seriesId:seriesId||undefined,artifactRevision:curve?.artifactRevision??baseRef?.artifactRevision});}catch{return null;}
    }
    selectedPointReferenceKeys(){const exact=new Set(),broad=new Set();for(const item of this.selectionSnapshot?.items||[]){const ref=item?.ref;if(!ref?.artifactId||!ref?.rowId)continue;try{if(ref.seriesId)exact.add(selectionReferences.identity(ref));else{const key=selectionReferences.sourceRowKey(ref);if(key)broad.add(key);}}catch{}}return {exact,broad};}
    pointSelected(curve,point,index=-1,keys=null){const ref=this.pointReference(curve,point,index);if(!ref)return false;const selected=keys||this.selectedPointReferenceKeys();try{if(ref.seriesId&&selected.exact.has(selectionReferences.identity(ref)))return true;const rowKey=selectionReferences.sourceRowKey(ref);return !!rowKey&&selected.broad.has(rowKey);}catch{return false;}}
    selectPoint(curve,point,index=-1,{source='scientific-curve-point',additive=false}={}){const ref=this.pointReference(curve,point,index);if(!ref||!this.interaction?.selectRef)return false;try{this.interaction.selectRef(ref,{type:'data.point',role:'point',source,additive});return true;}catch{return false;}}
