...
function applyScientificCurveNavigation(ScientificCurveSurface){
  class ScientificCurveNavigationMixin {
    ...
    installNavigationTools(){
      ...
      const drag=document.createElement('span');drag.className='dkds-scientific-nav-drag';drag.setAttribute('role','button');drag.setAttribute('tabindex','0');drag.setAttribute('aria-label','拖动图形工具条');drag.setAttribute('aria-label','拖动工具条；双击恢复默认位置');drag.textContent='⋮';tools.appendChild(drag);
      const rows=[['zoom-in','＋','放大'],['zoom-out','−','缩小'],['home','⌂','恢复全部数据']];
      for(const [action,label,title] of rows){const button=document.createElement('button');button.type='button';button.dataset.action=action;button.textContent=label;button.setAttribute('aria-label',title);button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();if(action==='home')this.resetView({reason:'toolbar-home'});else this.zoomBy(action==='zoom-in'?0.72:1.38,{reason:`toolbar-${action}`});});tools.appendChild(button);}
