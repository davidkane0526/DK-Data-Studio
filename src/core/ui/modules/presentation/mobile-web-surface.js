...
class MobileWebSurfacePresenter {
  ...
  drawerBounds(){
    ...
    const max=Math.max(autoMax,Math.min(680,hardMax,viewport*.88));
  ...
  measureDrawerOverflow(frame){
    ...
    let overflow=0;const contentRect=content.getBoundingClientRect?.();
    ...
    for(const node of nodes){
      ...
      if(contentRect&&rect&&Number.isFinite(rect.right)&&Number.isFinite(contentRect.right)){
        overflow=Math.max(overflow,rect.right-contentRect.right,contentRect.left-rect.left);
        continue;
      }
  ...
  solveCompactDrawerWidth(frame){
    ...
    for(let pass=0;pass<6;pass+=1){
      ...
      if(!(overflow>2))break;
      const next=Math.min(bounds.autoMax,Math.ceil(target+overflow+8));
      if(next<=target+1)break;target=next;
    }
    return Math.max(bounds.min,Math.min(bounds.autoMax,Math.ceil(target)));
  }
