...
const {GridController}=require('../grid/controller');
  ...
  class AnalysisWorkbench {
    ...
    renderNav(){
      ...
      if(this.primary&&primaryHost){const b=document.createElement('button');b.type='button';b.className='dkds-analysis-nav-btn';const active=!this.activeSub;b.classList.toggle('active',active);b.setAttribute('aria-current',active?'page':'false');b.textContent=this.primary.label||'主界面';b.onclick=()=>this.showPrimary();primaryHost.appendChild(b);}
      for(const row of [...this.primes.values()].sort((a,b)=>(a.order||100)-(b.order||100))){const b=document.createElement('button');b.type='button';b.className='dkds-analysis-nav-btn dkds-analysis-prime-btn';b.classList.toggle('selected',row.mounted);b.setAttribute('aria-pressed',row.mounted?'true':'false');b.textContent=row.label||row.title||row.id;b.onclick=()=>this.togglePrime(row.id);primeHost?.appendChild(b);}
      for(const row of [...this.subs.values()].sort((a,b)=>(a.order||100)-(b.order||100))){const b=document.createElement('button');b.type='button';b.className='dkds-analysis-nav-btn dkds-analysis-sub-btn';const active=this.activeSub===row.id;b.classList.toggle('active',active);b.setAttribute('aria-current',active?'page':'false');b.textContent=row.label||row.title||row.id;b.onclick=()=>this.openSub(row.id);subHost?.appendChild(b);}
