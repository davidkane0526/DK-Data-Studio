...
const {SelectionChannel, DataTypeRegistry, dataTypeRegistry, SelectionModel, InteractionRuntime, selectionReferences, interactionTransactions}=require('../selection/data-interaction');
...
const {PluginScope}=require('../scope/plugin-scope');
  ...
  const api={
    ...
    selectionReferences,interactionTransactions,
    ...
    lifecycleSnapshot(){const rows=[];for(const group of scopes.values())for(const scope of group)rows.push({owner:scope.owner,resize:scope.resizeScheduler?.state?.()||null,plots:scope.scientificRenderer?.lifecycleState?.()||null,interactions:scope.interactionLifecycleState?.()||null});return {scopes:rows.length,rows};},
    diagnostics(){const rows=[];for(const group of scopes.values())for(const scope of group)rows.push({owner:scope.owner,series:scope.series?.snapshot?.()||null,legendGroups:[...scope.legendGroups.values()].map(x=>x.snapshot()),pluginWorkspaces:(scope.pluginWorkspaces||[]).map(x=>x.layoutDiagnostics?.()).filter(Boolean),tables:[...new Set(globalTableSurfaceRegistry.rows.values())].filter(x=>x.owner===scope.owner).length,plots:scope.scientificRenderer?.lifecycleState?.()||null,interactions:scope.interactionLifecycleState?.()||null});return Object.freeze({version:VERSION,scopes:rows.length,rows:Object.freeze(rows)});},
    ...
    ActionGroup,InteractionBinding,InteractionBehaviorProfile,SelectionChannel,SelectionModel,InteractionRuntime,selectionReferences,interactionTransactions,SelectionViewBinding,HorizontalWheelScroller,DataTypeRegistry,SeriesRegistry,LegendGroup,ActiveLayoutSolver,TooltipService,DeclarativeTooltipRuntime,GroupPlot,ResizeScheduler,ContextMenu,SplitController,MovableSurface,WorkspaceLayout,PortableView,ChartSurface,PlotView,PlotViewRegistry,DialogService,SettingsSurface,SettingsRegistry,TableSurface,TableSurfaceRegistry,TableView,TableViewRegistry,ScientificCurveSurface,ViewHost,GridController,GroupAreaController,AnalysisWorkbench,PluginWorkspace,
