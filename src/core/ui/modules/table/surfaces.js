...
const {selectionReferences}=require('../selection/data-interaction');



  class TableSurface {
    constructor(scope,table,spec={}){
      this.scope=scope||null;this.owner=String(spec.owner||scope?.owner||'core.table');this.table=resolveElement(table);this.spec={minColumnWidth:56,maxColumnWidth:640,sortable:true,headerMenu:true,cellMenu:true,copyTable:true,persist:true,...spec};this.disposed=false;this.refreshing=false;this.rowOrder=new WeakMap();this.rowOrderSeq=0;this.rowDataOrder=new WeakMap();this.rowDataOrderSeq=0;this.cleanups=[];this.selectionView=null;this.menu=new ContextMenu(this.owner);this.state={widths:{},hidden:{},sort:null};
      ...
      this.applyAppearance();this.restorePersistedState();this.bindEvents();this.observe();this.refresh();this.bindSelection();
    }
    applyAppearance(){
    ...
    registerRows(){for(const row of this.bodyRows())if(!this.rowOrder.has(row)){const sourceIndex=this.rowOrderSeq++;this.rowOrder.set(row,sourceIndex);if(row?.dataset&&String(row.dataset.dkdsSourceRowIndex??'').trim()==='')row.dataset.dkdsSourceRowIndex=String(sourceIndex);}}
    sourceIndexForData(row,fallback=0){if(row&&typeof row==='object'){if(!this.rowDataOrder.has(row))this.rowDataOrder.set(row,this.rowDataOrderSeq++);return this.rowDataOrder.get(row);}return Math.max(0,Math.trunc(Number(fallback)||0));}
    selectionSpec(){return this.spec.selection&&typeof this.spec.selection==='object'?this.spec.selection:null;}
    rowReference(row){
      const spec=this.selectionSpec();if(!spec)return null;const rowIndex=Number(row?.dataset?.dkdsSourceRowIndex);const context={row,rowData:row?.__dkdsRowData,sourceIndex:Number.isInteger(rowIndex)?rowIndex:this.bodyRows().indexOf(row),surface:this};
      if(typeof spec.reference==='function'){try{const ref=spec.reference(context);return ref?selectionReferences.normalize(ref):null;}catch(err){console.warn('[DKDS table selection reference]',err);return null;}}
      const value=(entry,fallback='')=>{try{return typeof entry==='function'?entry(context):entry??fallback;}catch{return fallback;}};
      const artifactId=String(value(spec.artifactId,row?.dataset?.artifactId)||'').trim(),rowId=String(value(spec.rowId,row?.dataset?.rowId)||'').trim();if(!artifactId||!rowId)return null;
      const seriesId=String(value(spec.seriesId,row?.dataset?.seriesId)||'').trim(),artifactRevision=value(spec.artifactRevision,undefined);return selectionReferences.row(artifactId,rowId,{seriesId:seriesId||undefined,artifactRevision});
    }
    bindSelection(){
      this.selectionView?.dispose?.();this.selectionView=null;const spec=this.selectionSpec(),interaction=this.spec.interaction||spec?.interaction;if(!spec||!interaction?.bindView)return null;
      const keys=snapshot=>new Set((snapshot?.items||[]).map(item=>selectionReferences.sourceRowKey(item?.ref||{})).filter(Boolean));const focus=snapshot=>selectionReferences.sourceRowKey((snapshot?.focus||snapshot?.items?.at?.(-1))?.ref||{});
      this.selectionView=interaction.bindView(`table:${this.stableId||this.key()}:source-rows`,this.table,{selector:'tbody > tr',itemVariant:'row',entityLinked:false,revealFocus:spec.revealFocus!==false,ignore:spec.ignore||'button,input,select,textarea,a,[data-dkds-no-selection]',itemKey:row=>selectionReferences.sourceRowKey(this.rowReference(row)||{}),selectedKeys:keys,focusKey:focus,onActivate:({event,element})=>{const ref=this.rowReference(element);if(!ref)return;interaction.selectRef(ref,{type:spec.type||'data.point',role:spec.role||'row',source:spec.source||'table-row',additive:!!(event?.ctrlKey||event?.metaKey)});}});return this.selectionView;
    }
    bindEvents(){
      const click=event=>{
