...
const STYLE_SOURCE='src/core/ui/modules/tooltip/group-plot.js';
const tooltipSet=(el,property,value)=>StyleGate.set(el,property,value,{owner:'core.tooltip-service',scope:'runtime-tooltip',source:STYLE_SOURCE});
const groupToken=(el,token,value)=>StyleGate.setToken(el,token,value,{owner:'core.group-plot-layout',scope:'runtime-group-plot-token',source:STYLE_SOURCE});
...
const {ActiveLayoutSolver}=require('../series/layout');
  ...
  class DeclarativeTooltipRuntime {
    constructor(){this.service=new TooltipService(null);this.active=null;this.bound=false;this.observerCleanup=null;this.bind();}
    ...
    bind(){if(this.bound)return;if(typeof document==='undefined'||typeof document.addEventListener!=='function')return;this.bound=true;this.normalize(document);
      ...
      if(hub?.subscribe)this.observerCleanup=hub.subscribe('core.declarative-tooltip',rows=>{for(const row of rows){if(row.type==='attributes'){this.normalizeTarget(row.target);continue;}for(const node of row.addedNodes||[])if(node?.nodeType===1)this.normalize(node);}if(this.active&&!this.active.isConnected){this.active=null;this.service.hide();}},{subtree:true,childList:true,attributes:true,attributeFilter:['title']});
    }
    dispose(){this.observerCleanup?.();this.observerCleanup=null;if(!this.bound){this.service.dispose();this.active=null;return;}this.bound=false;if(typeof document!=='undefined'&&typeof document.removeEventListener==='function'){document.removeEventListener('mouseover',this.onOver);document.removeEventListener('mousemove',this.onMove);document.removeEventListener('mouseout',this.onOut);document.removeEventListener('focusin',this.onFocus);document.removeEventListener('focusout',this.onBlur);}this.service.dispose();this.active=null;}
  }
