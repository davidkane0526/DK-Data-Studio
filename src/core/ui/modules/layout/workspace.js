...
const {hostState, isElement, resolveElement, cleanupCall, readJson, writeJson}=require('../foundation/shortcuts');
  ...
  class SplitController {
    ...
    bind(){
      const down=e=>{if(e.button!==0)return;const rect=this.container.getBoundingClientRect();this.drag={start:this.axis==='x'?e.clientX:e.clientY,size:this.size,rect,pointerId:e.pointerId};document.documentElement?.classList?.add('dkds-split-drag-active');this.handle.classList.add('is-dragging');this.handle.setPointerCapture?.(e.pointerId);e.preventDefault();};
      const move=e=>{if(!this.drag||e.pointerId!==this.drag.pointerId)return;const point=this.axis==='x'?e.clientX:e.clientY;const sign=this.spec.reverse?-1:1;this.apply(this.drag.size+(point-this.drag.start)*sign,{persist:false,emit:false,notify:false});e.preventDefault();};
      const up=e=>{if(!this.drag||(e?.pointerId!==undefined&&e.pointerId!==this.drag.pointerId))return;this.handle.releasePointerCapture?.(this.drag.pointerId);this.drag=null;document.documentElement?.classList?.remove('dkds-split-drag-active');this.handle.classList.remove('is-dragging');this.apply(this.size,{persist:true,emit:false,notify:false});this.scope.emitResize?.({reason:'split-end',id:this.spec.id,size:this.size});};
