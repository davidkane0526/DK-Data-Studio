...
const PORTABLE_SEMANTIC_KINDS=new Set(['panel','inspector']);
  ...
  class PortableView {
    constructor(scope,id,node,spec={}){
      ...
      if(spec.sizing!==undefined&&!['content','fill'].includes(spec.sizing))throw new Error(`Unknown PortableView sizing: ${spec.sizing}`);
      ...
      const homeHeader=homePage?.querySelector?.(':scope > .analysis-page-header')||null;
    ...
    ensureWrapper(){
      ...
      wrapper.classList.add('dkds-portable-view');wrapper.dataset.portableId=this.id;wrapper.dataset.dkdsPortableSizing=this.spec.sizing||'content';
    ...
    resolveFloatingPositionMode(){
      if(this.floatingPositionMode)return this.floatingPositionMode;
      const position=String(globalThis.getComputedStyle?.(this.wrapper)?.position||'').toLowerCase();
      // Portable geometry is persisted in zone-local coordinates. Most
      // AnalysisWorkbench overlays use absolute positioning, but the generic
      // fallback is fixed. Dedicated windows expose a non-zero workspace origin
      // below their native titlebar, so writing zone-local coordinates directly
      // to a fixed element moves its draggable header underneath that titlebar.
      this.floatingPositionMode=position==='fixed'?'fixed':'local';
      return this.floatingPositionMode;
    }
    setFloatingPosition(left,top,metrics=null){
      const m=metrics||this.floatingZoneMetrics(this.wrapper?.dataset?.placement||'float');
      const fixed=this.resolveFloatingPositionMode()==='fixed';
      const originLeft=fixed?Number(m?.rect?.left)||0:0,originTop=fixed?Number(m?.rect?.top)||0:0;
      portableSet(this.wrapper,'left',`${Math.round(originLeft+(Number(left)||0))}px`);
      portableSet(this.wrapper,'top',`${Math.round(originTop+(Number(top)||0))}px`);
      return {left:Number(left)||0,top:Number(top)||0};
    }
    ...
    floatingZoneMetrics(mode='float'){
      const global=mode==='global'||this.wrapper?.classList?.contains('is-global-floating');
      const zone=global?(this.zone('global')||hostState.root||document.body):(this.zone('overlay')||hostState.root||document.body);
      const rect=zone?.getBoundingClientRect?.()||{left:0,top:0,width:window.innerWidth,height:window.innerHeight,right:window.innerWidth,bottom:window.innerHeight};
      const width=Math.max(0,Number(rect.width)||Number(window.innerWidth)||0),height=Math.max(0,Number(rect.height)||Number(window.innerHeight)||0);
      let minTop=0;
      // Any floating zone that geometrically overlaps the originating page
      // header must treat that header as protected chrome. Usually the local
      // overlay begins below the header and this resolves to zero; a global
      // overlay starts at the window top and therefore gets a positive bound.
      // Keeping the rule geometry-based also covers future Presenter/overlay
      // arrangements without a plugin-specific exception.
      const header=this.original?.header,page=this.original?.page;
      const pageVisible=!page?.classList?.contains?.('hidden');
      const headerRect=pageVisible&&header?.isConnected?header.getBoundingClientRect?.():null;
      if(headerRect&&Number(headerRect.height)>0){
        const relativeBottom=Number(headerRect.bottom)-Number(rect.top||0);
        if(Number.isFinite(relativeBottom)&&relativeBottom>0)minTop=Math.max(0,Math.min(Math.max(0,height-32),Math.ceil(relativeBottom+4)));
      }
      const left=Number(rect.left)||0,top=Number(rect.top)||0;
      return {zone,rect:{left,top,width,height,right:Number(rect.right)||left+width,bottom:Number(rect.bottom)||top+height},minTop};
    }
    availablePlacements(){
    ...
    place(value,{persist=true,bounds=null,source=''}={}){
      ...
      if(placement==='home')this.restoreHome();
        ...
        const metrics=this.floatingZoneMetrics(placement),zone=metrics.zone;zone.appendChild(this.wrapper);this.wrapper.classList.add('is-floating');if(placement==='global')this.wrapper.classList.add('is-global-floating');this.wrapper.dataset.placement=placement;
        ...
        const resolvedTop=Math.max(safeTop,Math.min(Math.max(safeTop,zoneHeight-resolvedHeight-(mobileHost?6:0)),requestedTop));
        this.setFloatingPosition(resolvedLeft,resolvedTop,this.floatingZoneMetrics(placement));
        ...
        else if(this.spec.sizing!=='fill'&&!scientificPlot&&Number(docked.height)>0)portableSet(this.wrapper,'height',`${Number(docked.height)}px`);
      }
    ...
    avoidFloatOverlap(){
      const placement=normalizePlacement(this.wrapper?.dataset?.placement),metrics=this.floatingZoneMetrics(placement),zone=metrics.zone;if(!zone||!['float','global'].includes(placement)&&!this.wrapper.classList.contains('is-floating'))return this.bounds();
      const z=metrics.rect;if(!z?.width||!z?.height)return this.bounds();
      const gap=Math.max(6,Number(this.spec.collisionGap)||10),current=this.bounds(),width=Math.min(current.width,z.width),height=Math.min(current.height,z.height),minTop=Math.max(0,Number(metrics.minTop)||0);
      const clamp=(left,top)=>({left:Math.max(0,Math.min(Math.max(0,z.width-width),left)),top:Math.max(minTop,Math.min(Math.max(minTop,z.height-height),top)),width,height});
      ...
      let start=clamp(current.left,current.top);if(!peers.some(peer=>intersects(start,peer))){this.setFloatingPosition(start.left,start.top,metrics);return start;}
      ...
      const next=viable[0]||start;this.setFloatingPosition(next.left,next.top,metrics);return next;
    }
    ...
    bindFloatDrag(mode='float'){
      const head=this.wrapper.querySelector('.drag-handle')||this.wrapper.querySelector('.dkds-portable-header');if(!head)return ()=>{};
      const nativeClient=document.documentElement?.classList?.contains('react-native-client')===true||document.documentElement?.dataset?.dkdsHost==='mobile';
      let state=null,nativeFrame=0,nativePoint=null;
      // Capture all geometry once at pointerdown / native touch start. Desktop pointer dragging
      // keeps the established direct left/top path. Native Mobile uses the raw
      // TouchEvent stream plus one compositor transform per animation frame, so
      // React Native WebView pointer throttling and left/top layout work cannot
      // make the surface lag behind the finger. Bounds are arithmetic-only while
      // moving; snap/collision repair is intentionally deferred until release.
      const capture=(clientX,clientY,id,input='pointer')=>{
        if(state)return false;this.raiseLayer();const metrics=this.floatingZoneMetrics(mode),r=this.wrapper.getBoundingClientRect(),z=metrics.rect;
        const startLeft=r.left-z.left,startTop=r.top-z.top;
        state={id,input,dx:clientX-r.left,dy:clientY-r.top,width:r.width,height:r.height,metrics,zoneLeft:z.left,zoneTop:z.top,zoneWidth:z.width,zoneHeight:z.height,minTop:Math.max(0,Number(metrics.minTop)||0),startLeft,startTop,currentLeft:startLeft,currentTop:startTop};
        return true;
      };
      const target=(clientX,clientY)=>{if(!state)return null;const left=Math.max(0,Math.min(Math.max(0,state.zoneWidth-state.width),clientX-state.dx-state.zoneLeft)),top=Math.max(state.minTop,Math.min(Math.max(state.minTop,state.zoneHeight-state.height),clientY-state.dy-state.zoneTop));return {left,top};};
      const down=e=>{if(e.button!==undefined&&e.button!==0||e.target.closest('button')||e.target.closest('.dkds-portable-resize-handle')||(nativeClient&&e.pointerType==='touch'))return;if(!capture(e.clientX,e.clientY,e.pointerId,'pointer'))return;head.setPointerCapture?.(e.pointerId);e.preventDefault();};
      const move=e=>{if(!state||state.input!=='pointer'||e.pointerId!==state.id)return;const next=target(e.clientX,e.clientY);if(!next)return;const {left,top}=next;state.currentLeft=left;state.currentTop=top;this.setFloatingPosition(left,top,state.metrics);if(e.cancelable)e.preventDefault();};
      const up=e=>{if(!state||state.input!=='pointer'||(e?.pointerId!==undefined&&e.pointerId!==state.id))return;finish();};
      const applyNative=()=>{nativeFrame=0;if(!state||state.input!=='touch'||!nativePoint)return;const next=target(nativePoint.clientX,nativePoint.clientY);if(!next)return;state.currentLeft=next.left;state.currentTop=next.top;portableSet(this.wrapper,'transform',`translate3d(${Math.round(next.left-state.startLeft)}px,${Math.round(next.top-state.startTop)}px,0)`);};
      const queueNative=point=>{if(!state||state.input!=='touch'||!point)return;nativePoint=point;if(!nativeFrame)nativeFrame=requestAnimationFrame(applyNative);};
      const commitNativePreview=()=>{if(!state||state.input!=='touch')return;if(nativeFrame){cancelAnimationFrame(nativeFrame);nativeFrame=0;}applyNative();portableRemove(this.wrapper,'transform');this.setFloatingPosition(state.currentLeft,state.currentTop,state.metrics);nativePoint=null;};
      const finish=()=>{if(!state)return;if(state.input==='touch')commitNativePreview();state=null;
        const r=this.wrapper.getBoundingClientRect(),snap=Math.max(24,Number(this.spec.snapDistance)||44),metrics=this.floatingZoneMetrics(mode),z=metrics.rect;
      ...
      const previousTouchAction=head.style?.touchAction||'';
      if(nativeClient)portableSet(head,'touch-action','none');
      const touchCleanup=nativeClient?NativeTouchDrag.bind(head,{
        onStart(point,event){if(event?.target?.closest?.('button,.dkds-portable-resize-handle'))return false;return capture(point.clientX,point.clientY,'touch','touch')?{}:false;},
        onMove(point){queueNative(point);},
        onEnd(point){if(point)queueNative(point);if(state?.input==='touch')finish();},
        onCancel(point){if(point)queueNative(point);if(state?.input==='touch')finish();}
      }):()=>{};
