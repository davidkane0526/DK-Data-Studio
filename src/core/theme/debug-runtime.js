(() => {
  ...
  function render(row){const host=ensureOverlay();host.hidden=isPaused();if(isPaused())return;host.innerHTML=`<div class="dkds-theme-debug-header"><div><b>Theme Inspector</b> · ${esc(globalThis.DKDSTheme?.contractVersion||'')} ${pinned?'· PINNED':''}</div><button type="button" class="dkds-theme-debug-exit" data-theme-debug-act="exit" aria-label="退出 Theme Inspector" title="退出 Theme Inspector"><svg viewBox="0 0 16 16" aria-hidden="true" focusable="false"><path d="M4 4l8 8M12 4l-8 8"/></svg></button></div>`+
