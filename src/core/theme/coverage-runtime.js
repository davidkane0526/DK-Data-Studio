(() => {
  ...
  const ROLE_AREAS=Object.freeze([
    ...
    {id:'elevated-pages',label:'Settings / Plugin Manager / Automation / Dedicated Panels',role:'elevated',selector:'#pluginManagerPage,#automationTestPage,.dkds-settings-dialog,.dkds-dialog-shell,.update-panel,.lan-web-panel,.import-workbench,.project-save-choice-card,.dkds-theme-settings-dialog,.dkds-material-role-elevated'},
  ...
  function scan(){
    const core=areaCoverage(),issues=pluginStyleIssues(),present=core.filter(x=>x.status!=='not-present');
