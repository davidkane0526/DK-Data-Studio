(() => {
  ...
  if(!ThemeContract||ThemeContract.version!=='3.7.0')throw new Error('Theme Contract 3.7 runtime is unavailable.');
  ...
  function scientificSnapshot(){const scientific=previewProfile(activeProfile,current).scientific||{};return Object.freeze({seriesPalette:Object.freeze([...(scientific.seriesPalette||[])])});}
  function roleAppearanceSnapshot(){const appearance=previewProfile(activeProfile,current).appearance||{roles:{}};return Object.freeze({roles:Object.freeze(Object.fromEntries(MATERIAL_ROLES.map(role=>[role,Object.freeze({...appearance.roles?.[role]})]))) });}
  ...
  window.DKDSTheme=Object.freeze({version:'3.7.0',contractVersion:'3.7.0',supports,rendererCapabilities,current:()=>current,system:systemTheme,set,toggle,isDark:()=>current==='dark',profile:()=>activeProfile,preferredProfile:()=>preferredProfile,setProfile,registerProfile,unregisterProfile,listProfiles,settings,setSetting,resetSettings,recipePolicy,tokens:snapshotTokens,tokenNames:()=>TOKEN_KEYS.slice(),materialRoles:()=>MATERIAL_ROLES.slice(),materials:materialSnapshot,appearanceRoles:roleAppearanceSnapshot,scientific:scientificSnapshot,platformUnits:()=>ThemeContract.platformUnits,preview:previewProfile,coverage:()=>globalThis.DKDSThemeCoverage?.scan?.()||Object.freeze({version:'0.0.0',summary:{ok:false,reason:'Theme Coverage Runtime unavailable'}})});
