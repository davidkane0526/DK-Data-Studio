(() => {
  ...
  if(!FrameScheduler?.schedule)throw new Error('Theme runtime requires DKDSFrameScheduler.');
  const THEME_REVISION_CONSUMERS=Object.freeze(['raster','computed-style']);
  const themeRevisionSubscribers=new Map();
  const themeRevisionStats={publishes:0,flushes:0,notifications:0,coalesced:0};
  let themeRevision=0,pendingThemeRevision=null;
  function revisionSnapshot(){return Object.freeze({revision:themeRevision,subscribers:themeRevisionSubscribers.size,consumers:Object.freeze(Object.fromEntries(THEME_REVISION_CONSUMERS.map(kind=>[kind,[...themeRevisionSubscribers.values()].filter(row=>row.consumer===kind).length]))),pending:!!pendingThemeRevision,...themeRevisionStats});}
  function subscribeRevision(owner,listener,{consumer}={}){
    const id=String(owner||'').trim(),kind=String(consumer||'').trim();if(!id)throw new Error('Theme revision subscriber owner is required.');if(typeof listener!=='function')throw new Error(`Theme revision listener is required: ${id}`);if(!THEME_REVISION_CONSUMERS.includes(kind))throw new Error(`Theme revision consumer must be raster or computed-style: ${id}`);
    themeRevisionSubscribers.set(id,Object.freeze({owner:id,consumer:kind,listener}));return()=>themeRevisionSubscribers.delete(id);
  }
  function publishThemeRevision(reason,detail={}){
    themeRevision+=1;themeRevisionStats.publishes+=1;if(pendingThemeRevision)themeRevisionStats.coalesced+=1;pendingThemeRevision=Object.freeze({revision:themeRevision,reason:String(reason||'theme'),theme:current,profile:activeProfile,...detail});
    FrameScheduler.schedule('theme.revision.notify',()=>{const snapshot=pendingThemeRevision;if(!snapshot)return;pendingThemeRevision=null;themeRevisionStats.flushes+=1;for(const row of [...themeRevisionSubscribers.values()]){try{row.listener(snapshot);}catch(error){console.warn('[DKDS Theme revision]',row.owner,error);}themeRevisionStats.notifications+=1;}},{priority:FrameScheduler.PRIORITY.GENERAL});return pendingThemeRevision;
  }
  ...
  function setSetting(profileId,id,value){
    ...
    const normalized=normalizeSettingInput(row,value);settingsStore={...settingsStore,[pid]:{...(settingsStore[pid]||{}),[row.id]:normalized}};saveSettingsStore();const active=pid===activeProfile;if(active){applyProfileTokens(current);refreshVisualComposition();publishThemeRevision('setting',{settingId:row.id});}try{globalThis.dispatchEvent(new CustomEvent('dkds:theme-settings-changed',{detail:{profile:pid,id:row.id,value:normalized,theme:current,visualSynchronized:active}}));if(active)globalThis.dispatchEvent(new CustomEvent('dkds:theme-changed',{detail:{theme:current,previous:current,profile:pid,tokens:snapshotTokens(),visualSynchronized:true,reason:'setting'}}));}catch{}return normalized;
  }
  function resetSettings(profileId=activeProfile){const pid=String(profileId||activeProfile);if(settingsStore[pid]!==undefined){const next={...settingsStore};delete next[pid];settingsStore=next;saveSettingsStore();const active=pid===activeProfile;if(active){applyProfileTokens(current);refreshVisualComposition();publishThemeRevision('settings-reset');}try{globalThis.dispatchEvent(new CustomEvent('dkds:theme-settings-changed',{detail:{profile:pid,reset:true,theme:current,visualSynchronized:active}}));if(active)globalThis.dispatchEvent(new CustomEvent('dkds:theme-changed',{detail:{theme:current,previous:current,profile:pid,tokens:snapshotTokens(),visualSynchronized:true,reason:'settings-reset'}}));}catch{}}return true;}
  ...
  function apply(theme,{persist=false,emit=true,broadcast=false,nativeSync=false}={}){
    ...
    if(previous!==next)publishThemeRevision('mode',{previous});
    if(emit&&(previous!==next)){try{globalThis.dispatchEvent(new CustomEvent('dkds:theme-changed',{detail:{theme:next,previous,profile:activeProfile,tokens:snapshotTokens(),visualSynchronized:true,reason:'mode'}}));}catch{}}
  ...
  function commitProfile(key,{emit=true,detail={}}={}){
    ...
    if(previous!==next)publishThemeRevision('profile',{previousProfile:previous,...detail});
    if(emit&&previous!==next){try{const tokens=snapshotTokens(),shared={profile:next,previous,theme:current,tokens,visualSynchronized:true,...detail};globalThis.dispatchEvent(new CustomEvent('dkds:theme-profile-changed',{detail:shared}));globalThis.dispatchEvent(new CustomEvent('dkds:theme-changed',{detail:{theme:current,previous:current,profile:next,tokens,visualSynchronized:true,reason:'profile'}}));}catch{}}
    endVisualTransaction(transaction);return Object.freeze({profile:next,previous,changed:previous!==next});
  }
  function registerProfile(id,spec={}){const profile=normalizeProfile(id,spec);profiles.set(profile.id,profile);if(pendingProfile===profile.id||preferredProfile===profile.id&&activeProfile!==profile.id){pendingProfile='';commitProfile(profile.id,{detail:{restored:true}});}else if(activeProfile===profile.id){applyProfileTokens(current);refreshVisualComposition();publishThemeRevision('profile-refresh');try{globalThis.dispatchEvent(new CustomEvent('dkds:theme-changed',{detail:{theme:current,previous:current,profile:activeProfile,tokens:snapshotTokens(),visualSynchronized:true,reason:'profile-refresh'}}));}catch{}}return Object.freeze({id:profile.id,dispose:()=>unregisterProfile(profile.id)});}
