(() => {
  ...
  function cacheNamespace(row){return `pipeline.${row.owner}.${row.id}`;}
  function rememberCache(row,performanceRuntime){if(!performanceRuntime)return;const key=row.key;if(!cacheBindings.has(key))cacheBindings.set(key,new Set());cacheBindings.get(key).add(performanceRuntime);}
  function clearStageCaches(key,row=null){const bindings=cacheBindings.get(key);if(!bindings)return;const target=row||registry.get(key);if(target)for(const runtime of bindings){try{runtime.trim?.(cacheNamespace(target),{targetEntries:0,dropWeak:true,reason:'pipeline-stage-unregister'});}catch{}}cacheBindings.delete(key);}
  ...
  function register(owner,id,spec={}){
    ...
    if(typeof row.run!=='function')throw new Error(`Scientific pipeline stage ${name} requires run().`);if(existing)clearStageCaches(key,existing);registry.set(key,row);if(!ownerIndex.has(o))ownerIndex.set(o,new Set());ownerIndex.get(o).add(key);return row;
  }
  function unregister(owner,id){const key=stageKey(owner,id),row=registry.get(key);if(!row)return false;clearStageCaches(key,row);registry.delete(key);ownerIndex.get(String(owner||''))?.delete(key);stats.delete(key);return true;}
  function removeOwner(owner){const o=String(owner||'');for(const key of [...(ownerIndex.get(o)||[])]){const row=registry.get(key);clearStageCaches(key,row);registry.delete(key);stats.delete(key);}ownerIndex.delete(o);}
  ...
  async function execute(owner,id,input,options={}){
    ...
    const parameters=clone(options.parameters||{}),revision=revisionKey(row,input,parameters,context),parameterKey=hash({stageVersion:row.version,parameters}),m=metric(row.key),started=globalThis.performance?.now?.()??Date.now();m.runs+=1;
    const compute=()=>{const raw=row.run(input,{...context,parameters,revision,stage:row});return raw&&typeof raw.then==='function'?Promise.resolve(raw).then(value=>normalizeResult(row,value,input,parameters,context)):normalizeResult(row,raw,input,parameters,context);};
    try{
      let result;
      if(row.cache&&row.execution!=='async'&&context.performance?.stage){rememberCache(row,context.performance);result=await context.performance.stage(cacheNamespace(row),revision,parameterKey,compute,{limit:row.cacheLimit});}else result=await compute();
  ...
  function executeSync(owner,id,input,options={}){
    ...
    try{
      if(row.cache&&context.performance?.stage)rememberCache(row,context.performance);
      const result=row.cache&&context.performance?.stage?context.performance.stage(cacheNamespace(row),revision,parameterKey,compute,{limit:row.cacheLimit}):compute();
