(() => {
  ...
  const isPromiseLike=value=>!!value&&(typeof value==='object'||typeof value==='function')&&typeof value.then==='function';
  ...
  function memoWeak(namespace,target,key,compute,options={}){
    ...
    const cached={value,at:Date.now(),dispose:typeof options.dispose==='function'?options.dispose:null};bucket.entries.set(id,cached);weakEntryCounts.set(ns,(weakEntryCounts.get(ns)||0)+1);updateEntries(ns);
    if(isPromiseLike(value)){
      const pending=Promise.resolve(value).then(resolved=>{if(bucket.entries.get(id)===cached){cached.value=resolved;cached.at=Date.now();}return resolved;},error=>{if(bucket.entries.get(id)===cached){bucket.entries.delete(id);weakEntryCounts.set(ns,Math.max(0,(weakEntryCounts.get(ns)||0)-1));updateEntries(ns);}throw error;});
      cached.value=pending;
    }
  ...
  function memo(namespace,key,compute,options={}){
    ...
    m.misses+=1;m.computes+=1;const started=now();const value=compute();recordCompute(m,started);const cached={value,at:Date.now(),dispose:typeof options.dispose==='function'?options.dispose:null};cache.set(id,cached);
    if(isPromiseLike(value)){
      const pending=Promise.resolve(value).then(resolved=>{if(cache.get(id)===cached){cached.value=resolved;cached.at=Date.now();}return resolved;},error=>{if(cache.get(id)===cached){cache.delete(id);updateEntries(ns);}throw error;});
      cached.value=pending;
    }
    while(cache.size>cfg.limit){const oldest=cache.keys().next().value;const removed=cache.get(oldest);cache.delete(oldest);disposeValue(ns,removed,'lru');m.evictions+=1;}updateEntries(ns);return cached.value;
  }
