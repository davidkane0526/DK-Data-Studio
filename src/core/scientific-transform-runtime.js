(() => {
  ...
  function runCurve(id,input,options={}){
    ...
    const result=row.run?row.run(sweep,{...options,parameters,transform:row}):science?.transformSweep?.(sweep,row.transformKey,parameters);
    if(!result)throw new Error(`${row.id}: transform returned no result.`);
    return {...result,type:row.id,transformId:row.id,semanticType:row.outputType||result.semanticType||'',label:result.label||row.label,unit:result.unit||row.unit,quantity:row.quantity,diverging:row.diverging};
  }
  ...
  function runScalarField(id,input,options={}){
    ...
    const params={...clone(options),type:row.transformKey,transformId:row.id,vgs:groups,targets};delete params.groups;
    const result=row.field?row.field(sweeps,params,{transform:row}):science?.computeSweepScalarField?.(sweeps,targets,groups,params)||science?.computeSweepTransformMatrix?.(sweeps,targets,groups,params);
    if(!result)throw new Error(`${row.id}: scalar-field transform returned no result.`);
    return {...result,type:row.id,transformId:row.id,semanticType:row.fieldType||'science.scalar-field',label:result.label||row.label,unit:result.unit||row.unit,quantity:row.quantity,diverging:row.diverging};
  }
  ...
  const BUILTINS=[
    {id:'raw',title:'原始 I–V',outputType:'science.iv.raw',fieldType:'science.transport.current-field',quantity:'current',unit:'A',diverging:true,tags:['transport','iv','raw']},
    {id:'detrend',title:'去背景 I−Ibg',outputType:'science.iv.background-removed',fieldType:'science.transport.background-removed-current-field',quantity:'current',unit:'A',diverging:true,tags:['transport','iv','transform']},
    {id:'didv',title:'dI/dV（微分电导）',outputType:'science.transport.didv',fieldType:'science.transport.conductance-field',quantity:'conductance',unit:'A/V',diverging:true,tags:['transport','conductance','transform']},
    {id:'d2idv2',title:'d²I/dV²',outputType:'science.transport.d2idv2',fieldType:'science.transport.second-derivative-current-field',quantity:'second-derivative-current',unit:'A/V²',diverging:true,tags:['transport','transform']},
    {id:'dlog',title:'d ln|I|/dV',outputType:'science.transport.dlnabsidv',fieldType:'science.transport.log-current-slope-field',quantity:'log-current-slope',unit:'1/V',diverging:true,tags:['transport','transform']},
    {id:'dvdi',title:'dV/dI（微分电阻）',outputType:'science.transport.dvdi',fieldType:'science.transport.differential-resistance-field',quantity:'differential-resistance',unit:'V/A',diverging:true,tags:['transport','resistance','transform']},
    {id:'resistance',title:'R = |V/I|',outputType:'science.transport.resistance',fieldType:'science.transport.resistance-field',quantity:'resistance',unit:'Ω',diverging:false,tags:['transport','resistance','transform']}
  ];
