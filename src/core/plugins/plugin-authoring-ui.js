(() => {
  ...
  function render(){
    ...
    const workflowSummary=`<div class="plugin-manager-authoring-workflow"><strong>Source Workflow</strong><span>${escapeHtml(String(workflow.codeCellCount||0))} 个 code cell · ${escapeHtml(String(workflow.crossCellDependencyCount||0))} 条跨 Cell 依赖</span><span>Host 映射：${escapeHtml(hostCaps)}</span><span>待 lowering：${escapeHtml(transforms)}</span><span>状态：${escapeHtml(candidate.status||'—')}</span></div><div class="plugin-manager-authoring-ir"><strong>Table Transform IR</strong><span>${escapeHtml(String(tablePlan.lowerableStatementCount||0))} / ${escapeHtml(String(tablePlan.statementCount||0))} 条语句已结构化</span><span>覆盖率：${escapeHtml(String(Math.round(Number(tablePlan.coverage||0)*100)))}%</span><span>${tablePlan.buildable?'IR 已闭合':'仍有未 lowering 语句'}</span></div>${workflowIssues}`;
