(() => {
  ...
  function render(){
    ...
    const diagnostics=fn?.workflowCandidate?(fn.blueprint?.diagnostics||[]):(state.report?.diagnostics||[]).filter(row=>!row.functionId||row.functionId===fn?.id);
  ...
  async function open(){
    ...
    const funcs=result.report?.sourceModel?.functions||[],workflowBlueprint=result.report?.sourceModel?.workflow?.blueprint||null;
    const preferred=funcs.find(row=>row.blueprint?.buildable)||((workflowBlueprint?.buildable)?{id:workflowBlueprint.candidateId}:null)||funcs[0]||(workflowBlueprint?{id:workflowBlueprint.candidateId}:null);
