(() => {
  ...
  function create(context){
    ...
    function invalidatePeakMetric(p,{geometry=false,reason='peak-edit',refresh=true}={}){
      if(!p)return;peakMetricCache.delete(p);peakMetricRevision+=1;actions.invalidatePhysics();const keys=[reactivePeakMetricInput(p),'resonance.peak.metric-input'];if(geometry)keys.push(reactivePeakGeometry(p),'resonance.peak.geometry');
    ...
    async function runPeakDetector(provider,sweep,settings,options={}){
      if(!provider){const d=missingLockedAlgorithm('peak-detector',live.workspace.activeDetector);if(d)throw new Error(`工程锁定的寻峰算法版本缺失：${d.requested.id}@${d.requested.version}；可用版本：${d.alternatives.map(x=>x.version).join('、')||'无'}`);return S.detectPeaks(sweep,live.workspace.algorithms||{},options||{});}
      ...
      if(provider.detect)return await provider.detect(sweep,settings||{},options||{});if(provider.run)return await provider.run(sweep,{parameters:settings||{},...options});return S.detectPeaks(sweep,live.workspace.algorithms||{},options||{});
    }
    ...
    function storePeakMetric(p,signature,value,provider){if(!value||typeof value!=='object')return null;const sw=actions.sweepById(p?.sweepId),current=metricProvider();if(!sw||!current||metricSignature(p,sw,current)!==signature)return null;const ref={id:provider.id,version:provider.version,category:'peak-metrics'},next={...value,algorithm:live.algorithmRuntime?.provenance?.(ref)||{pluginId:provider.owner||'',algorithmId:provider.id,algorithmVersion:provider.version,category:'peak-metrics',title:provider.title||provider.id}};peakMetricCache.set(p,{signature,value:next,promise:null});peakMetricRevision+=1;actions.invalidatePhysics();publishPeakMetricValue(p);return next;}
    function tryPeakMetricSync(p,sw,provider,signature){const ref={id:provider.id,version:provider.version,category:'peak-metrics'};installPipeline();try{let value;if(live.pipelineRuntime?.runSync&&live.algorithmRuntime)value=live.pipelineRuntime.runSync('peaks.metrics',{peak:p,sweep:sw},{parameters:{algorithmRef:ref,settings:{}},publish:false})?.value;else{value=provider.run?.({peak:p,sweep:sw},{parameters:{}});if(value&&typeof value.then==='function')return null;}return storePeakMetric(p,signature,value,provider);}catch(err){if(!/async/i.test(String(err?.message||err)))console.warn('[resonance peak metrics sync]',p?.id,err);return null;}}
    async function refreshPeakMetric(p){const sw=actions.sweepById(p?.sweepId),provider=metricProvider();if(!p||!sw||!provider)return null;const signature=metricSignature(p,sw,provider),cached=peakMetricCache.get(p);if(cached?.signature===signature){if(cached.value)return cached.value;if(cached.promise)return cached.promise;}const ref={id:provider.id,version:provider.version,category:'peak-metrics'};installPipeline();const promise=(async()=>{try{const compute=async()=>{if(live.pipelineRuntime?.run&&live.algorithmRuntime){const result=await live.pipelineRuntime.run('peaks.metrics',{peak:p,sweep:sw},{parameters:{algorithmRef:ref,settings:{}},publish:false});return result?.value;}return provider.run?.({peak:p,sweep:sw},{parameters:{}});};const task=live.reactiveRuntime?.runLatest?await live.reactiveRuntime.runLatest(`resonance.metric:${p.id}`,compute,{dependsOn:[reactivePeakGeometry(p),reactivePeakMetricInput(p)]}):{accepted:true,value:await compute()};if(!task?.accepted)return null;return storePeakMetric(p,signature,task.value,provider);}catch(err){console.warn('[resonance peak metrics algorithm]',p?.id,err);}if(metricSignature(p,actions.sweepById(p?.sweepId),metricProvider()||provider)===signature)peakMetricCache.set(p,{signature,value:null,promise:null});return null;})();peakMetricCache.set(p,{signature,value:null,promise});return promise;}
    function peakMetrics(p){const sw=actions.sweepById(p?.sweepId);if(!sw||!p)return null;const provider=metricProvider();if(!provider&&missingLockedAlgorithm('peak-metrics',live.workspace.activeMetricAlgorithm))return null;if(provider){const signature=metricSignature(p,sw,provider),cached=peakMetricCache.get(p);if(cached?.signature===signature&&cached.value)return cached.value;if(!cached||cached.signature!==signature||!cached.promise){const sync=tryPeakMetricSync(p,sw,provider,signature);if(sync)return sync;void refreshPeakMetric(p);}return null;}return S.peakMetrics?.(p,sw)||null;}
    function resetMetricCache({reason='metric-reset'}={}){peakMetricCache=new WeakMap();peakMetricRevision+=1;actions.invalidatePhysics();reactiveTouch(['resonance.peak.metric-input','resonance.peak.metrics'],{reason});}
    function setDetectorRuntime(runtime){detectorRuntime=runtime||null;}
    return Object.freeze({runDetection,detectRange,peakMetrics,commitPeakMetricEdit,scheduleMetricRefresh,installPipeline,resetPipeline,resetMetricCache,setDetectorRuntime,revision:()=>peakMetricRevision});
  }
