(() => {
  ...
  function registerDataTypes(ctx){
    ...
    for(const [id,spec] of [
      ['resonance.dataset',{title:'共振数据集',parent:'data.artifact',kind:'data',key:v=>v?.path||v?.id,selection:v=>{const id=v?.path||v?.id;return {id,ref:entityRef(id,{datasetPath:v?.path||''}),meta:{label:v?.name||id}};}}],
      ['resonance.sweep',{title:'共振扫描',parents:['data.sweep','science.iv.raw'],kind:'data',key:v=>v?.id,selection:v=>({id:v?.id,ref:entityRef(v?.id,{sweepId:v?.id||'',datasetPath:v?.datasetPath||''}),meta:{vg:v?.vg,direction:v?.direction}})}],
      ['resonance.peak',{title:'共振峰',parents:['data.point','science.resonance.peak'],kind:'result',key:v=>v?.id,selection:v=>({id:v?.id,ref:entityRef(v?.id,{peakId:v?.id||'',sweepId:v?.sweepId||'',datasetPath:v?.datasetPath||''}),meta:{peakOrder:v?.peakOrder,peakLabel:v?.peakLabel||''}})}],
      ['resonance.fwhm',{title:'共振半峰全宽',parent:'science.resonance.fwhm',kind:'result',key:v=>v?.id||`${v?.peakId||''}:fwhm`,selection:v=>{const id=v?.id||`${v?.peakId||''}:fwhm`;return {id,ref:entityRef(id,{peakId:v?.peakId||''}),meta:{unit:v?.unit||'V'}};}}],
      ['resonance.peak-family',{title:'共振峰族',parent:'result.analysis',kind:'result',key:v=>v?.id||`${v?.direction||0}:${v?.label||''}`,selection:v=>{const id=v?.id||`${v?.direction||0}:${v?.label||''}`;return {id,ref:entityRef(id),meta:{label:v?.label||'',direction:v?.direction,order:v?.order}};}}],
      ['resonance.range',{title:'共振电压选区',parent:'data.range',kind:'region',key:v=>v?.id||`${v?.sweepId||''}:${v?.min||''}:${v?.max||''}`}],
      ['resonance.analysis-result',{title:'共振派生分析',parent:'result.analysis',kind:'result',key:v=>v?.id,selection:v=>{const id=v?.id;return {id,ref:v?.artifactId?refs.artifact(v.artifactId):entityRef(id,{resultId:id||''}),meta:{label:v?.name||id,kind:v?.kind||''}};}}],
  ...
  function buildTrendModel(controller){
    ...
    for(const label of labels){
      ...
      const data=service?.resonantTerForLabel?.(label,[...visibleIds])||[];
