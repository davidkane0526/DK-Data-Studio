(() => {
  function create(context){
    ...
    async function computeGateTer(settings={}){const ref=gateTerAlgorithmRef(),row=live.algorithmRuntime?.resolve?.(ref,{category:'ter-analysis'});if(!row||!live.algorithmRuntime?.run)return null;const input=Array.isArray(live.analysisDatasets)?live.analysisDatasets:live.datasets;const value=await live.algorithmRuntime.run({id:row.id,version:row.version,category:'ter-analysis'},input,{category:'ter-analysis',parameters:{settings,executionKey:'resonance-gate'}});return {...value,algorithm:live.algorithmRuntime.provenance?.({id:row.id,version:row.version,category:'ter-analysis'})||value?.algorithm||null};}
    ...
    function installPipeline(){
      ...
      pipeline.register('gate-analysis',{
        ...
        run:async(_input,{parameters})=>{
          ...
          try{terResult=await computeGateTer(parameters?.terSettings||gateTerSettings());}catch{}
        ...
        selection:({artifacts,value})=>artifacts[0]?[{type:'resonance.gate-analysis',id:artifacts[0].id,ref:{artifactId:artifacts[0].id},meta:{rows:value?.rows?.length||0}}]:[],
    ...
    async function computeGate(){
      ...
      const dataRevision=artifacts?.revision?.('data.table')||0,terSettings=gateTerSettings(),terAlgorithmRef=gateTerAlgorithmRef();
      const key=`${dataRevision}::metric:${live.peakMetricRevision}::${JSON.stringify(s)}::${JSON.stringify(terSettings)}::${JSON.stringify(terAlgorithmRef)}::${peakKey}`;gateComputeKey=key;
      if(live.pipelineRuntime?.run){const source=(artifacts?.list?.({kind:'data.table',includeTransient:true})||[]).filter(a=>String(a?.semanticType||'')==='science.transport.iv');const executed=await live.pipelineRuntime.run('gate-analysis',source,{parameters:{settings:{...s},terSettings:{...terSettings},terAlgorithmRef,peakKey,metricRevision:live.peakMetricRevision},publish:true,revision:dataRevision});gateResult=executed?.value||null;return gateResult;}
      const Arows=gateSeriesRows(s.seriesA),Brows=gateSeriesRows(s.seriesB),terResult=await computeGateTer(terSettings),rows=S.pairGateSeries?.(Arows,Brows,terResult?.terMaxByVg||[],s)||[],hysteresis=gateHysteresisRows(s.hysteresisLabel),summary=S.summarizeGateRows?.(rows,hysteresis)||{fits:{},correlations:{}},featureField=gateFeatureField(s);gateResult={settings:{...s},seriesA:gateOption(s.seriesA),seriesB:gateOption(s.seriesB),Arows,Brows,rows,hysteresis,terResult,featureField,fits:summary.fits||{},correlations:summary.correlations||{}};return gateResult;
    }
    ...
    async function renderGate(){
      populateGate();let r;try{r=await computeGate();}catch(err){console.warn('[Resonance gate analysis]',err);setStatus(`栅压分析失败：${err?.message||err}`);return null;}if(!r)return null;const rows=r.rows||[],a=r.seriesA?.name||'ridge A',b=r.seriesB?.name||'ridge B';
