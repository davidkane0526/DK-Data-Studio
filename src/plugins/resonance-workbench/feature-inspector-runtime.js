(() => {
  // Inspector rendering is a view responsibility. It consumes live selection
  // and peak metrics through FeatureContext without owning scientific state.
  function create(context){
    const {live,services,actions,utils}=context;
    const {$,charts,S,transforms,setStatus}=services;
    ...
    function render(){
      ...
      if(p){
        ...
        const categoryButtons=(live.workspace.peakCategories||[]).map(c=>`<button type="button" class="peak-category-choice dkds-action-button dkds-choice-button ${Number(c.order)===Number(p.peakOrder)?'selected':''}" data-peak-category="${Number(c.order)}"><span class="category-pair-swatch dkds-series-swatch-pair"><i style="background:${esc(actions.colorForPeakOrder(c.order,1))}"></i><i style="background:${esc(actions.colorForPeakOrder(c.order,-1))}"></i></span><span>${esc(c.label)}</span></button>`).join('');
        ...
        host.querySelectorAll('[data-peak-category]').forEach(btn=>btn.onclick=()=>actions.assignPeakCategory(p,btn.dataset.peakCategory));
        host.querySelector('#reswinAddPeakCategory').onclick=()=>actions.createPeakCategoryForPeak(p);
        host.querySelector('#reswinApplyPeakLabel').onclick=()=>actions.renameSelectedCategory(host.querySelector('#reswinPeakLabelInput')?.value);
        host.querySelector('#reswinAcceptPeak').onclick=()=>actions.updatePeak(p.id,{accepted:p.accepted===false});
        host.querySelector('#reswinLockPeak').onclick=()=>actions.updatePeak(p.id,{locked:!p.locked});
        host.querySelector('#reswinResetFwhmWindow').onclick=()=>{delete p.analysisLeft;delete p.analysisRight;delete p.analysisManual;actions.commitPeakMetricEdit(p,{reason:'fwhm-window-reset'});actions.scheduleSnapshot();setStatus('已恢复自动 FWHM 分析窗口。');};
        host.querySelector('#reswinDeletePeak').onclick=()=>actions.deletePeak(p.id);
        host.querySelector('#reswinSelectCurve').onclick=()=>{const row=actions.sweepById(p.sweepId);if(row)actions.publishSweepSelection(row,'resonance-inspector');};
      }else{
      ...
      const plot=host.querySelector('#reswinInspectPlot');if(plot&&charts&&sw){
        ...
        const traces=[{x:t.points.map(q=>q.v),y:t.points.map(q=>q.y),mode:'lines',name:t.label,line:{width:1.8,color:'#315efb'}}],peaks=(live.workspace.peaks||[]).filter(q=>q.sweepId===sw.id&&q.accepted!==false);
