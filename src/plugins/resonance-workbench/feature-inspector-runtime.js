  ...
  function create(context){
    ...
    function render(){
      ...
      if(p){
        ...
        const categoryButtons=(live.workspace.peakCategories||[]).map(c=>`<button type="button" class="peak-category-choice dkds-action-button dkds-choice-button ${Number(c.order)===Number(p.peakOrder)?'selected':''}" data-peak-category="${Number(c.order)}"><span class="category-pair-swatch dkds-series-swatch-pair"><i style="--dkds-series-color:${esc(actions.colorForPeakOrder(c.order,1))}"></i><i style="--dkds-series-color:${esc(actions.colorForPeakOrder(c.order,-1))}"></i></span><span>${esc(c.label)}</span></button>`).join('');
