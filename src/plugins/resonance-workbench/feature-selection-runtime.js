(() => {
  ...
  function create(context){
    ...
    function publishRangeSelection(range,source='resonance-main'){
      ...
      if(interactionSelection){const selected=peaksInRange(selectedRange).map(selectionPeakItem).filter(Boolean);if(interactionSelection.selectRegion)interactionSelection.selectRegion(selectedRange,selected,{rangeType:'resonance.range',source,context:{range:selectedRange,sweepId:selectedRange.sweepId||''}});else interactionSelection.setRange(selectedRange,{type:'resonance.range',source});}
      return true;
    }
