...
<table class="analysis-table" id="spacingTable"></table>
          ...
          <section class="respar-primary" data-reswin-view-panel="main">
            <aside class="respar-left-panel">
              ...
              <section>
                ...
                <label class="respar-select-label">峰宽/基线算法<select id="reswinMetricAlgorithmSelect"></select></label>
  ...
  const TOP_STYLES=`
    ...
    #resonanceDedicatedPage .reswin-dataset:hover{background:#f7f8fc}
    ...
    #resonanceDedicatedPage .respar-legend-chip{flex:0 0 auto;display:inline-flex;align-items:center;gap:5px;height:26px;padding:3px 7px;border:1px solid #e1e6ef;border-radius:6px;background:#fff;color:#475569;font-size:10px}#resonanceDedicatedPage .respar-legend-line{width:18px;height:0;border-top:3px solid currentColor}#resonanceDedicatedPage .respar-legend-line.reverse{border-top-style:dashed}
  ...
  function mountUnified(ctx,controller,{mode='top',adapter={}}={}){
    ...
    const renderDetectorPicker=()=>{const select=page.querySelector('#reswinDetectorSelect'),note=page.querySelector('#reswinDetectorDescription'),paramHost=page.querySelector('#reswinDetectorParams');if(!select)return;const rows=(ctx.analysis.algorithms?.list?.({category:'peak-detector'})||[]).map(row=>({...row,id:`${row.id}@${row.version}`,algorithmId:row.id,shortName:row.metadata?.shortName||row.title,name:row.title,presets:row.metadata?.presets||[]})),state=R.getState?.(),saved=String(state?.workspace?.activeDetector||''),current=String(rows.find(row=>row.id===saved||row.algorithmId===saved)?.id||rows.find(row=>row.default)?.id||rows[0]?.id||'');select.innerHTML=rows.map(row=>`<option value="${String(row.id).replace(/"/g,'&quot;')}">${String(row.shortName||row.name||row.id)} · v${String(row.version||'')}</option>`).join('');if(rows.some(row=>String(row.id)===current))select.value=current;const renderActive=()=>{const row=rows.find(item=>String(item.id)===select.value);if(note)note.textContent=row?`${row.description||'寻峰算法'} · ${row.algorithmId}@${row.version}`:'选择当前使用的寻峰算法。';detectorParamPanel?.dispose?.();detectorParamPanel=null;if(paramHost)paramHost.replaceChildren();if(row?.parameterSchema&&paramHost&&ctx.parameters?.render){const ws=R.getState?.()?.workspace||{},value=ws.detectorSettings?.[row.id]||ws.detectorSettings?.[row.algorithmId]||ws.algorithms||{};detectorParamPanel=ctx.parameters.render(paramHost,row.parameterSchema,{value,onChange:next=>R.setDetectorSettings?.(row.id,next)});}};select.onchange=()=>{R.setActiveDetector?.(select.value);renderActive();};renderActive();};
    renderDetectorPicker();
    const renderMetricAlgorithmPicker=()=>{const select=page.querySelector('#reswinMetricAlgorithmSelect'),note=page.querySelector('#reswinMetricAlgorithmDescription');if(!select)return;const rows=ctx.analysis.algorithms?.list?.({category:'peak-metrics'})||[],state=R.getState?.(),saved=String(state?.workspace?.activeMetricAlgorithm||''),current=String(rows.find(row=>`${row.id}@${row.version}`===saved||row.id===saved)?.id?`${rows.find(row=>`${row.id}@${row.version}`===saved||row.id===saved).id}@${rows.find(row=>`${row.id}@${row.version}`===saved||row.id===saved).version}`:(rows.find(row=>row.default)?`${rows.find(row=>row.default).id}@${rows.find(row=>row.default).version}`:(rows[0]?`${rows[0].id}@${rows[0].version}`:'')));select.innerHTML=rows.map(row=>`<option value="${esc(`${row.id}@${row.version}`)}">${esc(row.metadata?.shortName||row.title||row.id)} · v${esc(row.version)}</option>`).join('');if(current)select.value=current;const active=rows.find(row=>`${row.id}@${row.version}`===select.value);if(note)note.textContent=active?`${active.description||'峰度量算法'} · ${active.id}@${active.version}`:'未发现峰度量算法插件。';select.onchange=()=>{R.setActiveMetricAlgorithm?.(select.value);const row=rows.find(item=>`${item.id}@${item.version}`===select.value);if(note)note.textContent=row?`${row.description||'峰度量算法'} · ${row.id}@${row.version}`:'';};};
