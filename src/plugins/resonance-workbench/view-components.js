...
<table class="analysis-table" id="spacingTable"></table>
      ...
      <div class="analysis-page-body resonance-dedicated-body">
        <div class="resonance-parity-root">
          <section class="respar-primary" data-reswin-view-panel="main">
            <aside class="respar-left-panel">
              ...
              <section>
                ...
                <label class="respar-select-label">寻峰算法<select id="reswinDetectorSelect"></select></label><button id="reswinRecoverDetector" type="button" class="wide hidden">定位/恢复缺失寻峰算法</button>
                <label class="respar-select-label">峰宽/基线算法<select id="reswinMetricAlgorithmSelect"></select></label><button id="reswinRecoverMetricAlgorithm" type="button" class="wide hidden">定位/恢复缺失峰宽算法</button>
                <div id="reswinMetricAlgorithmDescription" class="respar-note">FWHM、峰高、面积与局部基线由可版本化算法插件计算。</div>
          ...
          <section class="respar-derived hidden" data-reswin-view-panel="gate">
            ...
            <div id="reswinGateSummary" class="ter-summary reswin-summary"></div><div class="reswin-gate-grid"><div class="analysis-chart-card"><div class="analysis-chart-title">共振 ridge</div><div id="reswinGateRidges" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">共振中心 V0</div><div id="reswinGateV0" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">有效分裂 δ</div><div id="reswinGateDelta" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">峰宽与 |δ|/w</div><div id="reswinGateWidthPlot" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">TERmax</div><div id="reswinGateTer" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">最佳读出偏压 Vd*</div><div id="reswinGateVStar" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">正反扫回滞</div><div id="reswinGateHysteresisPlot" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">峰高与有效权重</div><div id="reswinGateAmplitude" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">TERmax vs |δ|/w</div><div id="reswinGateTerCorrelation" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">Vd* vs V0</div><div id="reswinGateReadoutCorrelation" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">局域背景与峰/背景比</div><div id="reswinGateBackground" class="analysis-chart"></div></div><div class="analysis-chart-card"><div class="analysis-chart-title">载流子浓度依赖（可选）</div><div id="reswinGateDensity" class="analysis-chart"></div></div><div class="analysis-chart-card reswin-feature-field-card"><div class="analysis-chart-title"><span id="reswinGateFeatureFieldTitle">跨曲线特征场</span></div><div id="reswinGateFeatureFieldMeta" class="respar-note"></div><div id="reswinGateFeatureField" class="analysis-chart reswin-feature-field-plot"></div></div></div><div id="reswinGateReport" class="reswin-report"></div><div class="analysis-table-wrap"><table id="reswinGateTable" class="analysis-table"></table></div>
          </section>
        </div>
      </div>`;
  }
  ...
  const TOP_STYLES=`
    #resonanceDedicatedPage .resonance-dedicated-body{padding:0!important;overflow:hidden!important;background:var(--app-bg)}
    #resonanceDedicatedPage .resonance-parity-root{position:relative;width:100%;height:100%;min-width:0;min-height:0;overflow:hidden;background:var(--app-bg);font-size:12px}
    #resonanceDedicatedPage .respar-header-actions{display:flex;align-items:center;gap:5px;flex-wrap:nowrap;overflow-x:auto}#resonanceDedicatedPage .respar-header-divider{width:1px;height:22px;background:var(--border-subtle);flex:0 0 auto}
    #resonanceDedicatedPage .respar-primary{width:100%;height:100%;min-width:0;min-height:0;display:grid;grid-template-columns:280px minmax(0,1fr);background:var(--surface-primary)}
    #resonanceDedicatedPage .respar-left-panel{min-width:0;overflow:auto;padding:12px;background:var(--surface-primary);border-right:1px solid var(--border-subtle)}
    #resonanceDedicatedPage .respar-left-panel section{border-bottom:1px solid var(--border-subtle);padding-bottom:12px;margin-bottom:12px}
    #resonanceDedicatedPage .respar-left-panel h3{font-size:14px;margin:0 0 8px;color:var(--text-primary)}
    #resonanceDedicatedPage .respar-left-panel label{font-size:12px;color:var(--text-secondary)}
    ...
    #resonanceDedicatedPage .reswin-dataset{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:5px;padding:7px;border:1px solid transparent;border-radius:7px;background:var(--surface-primary);cursor:pointer}
    #resonanceDedicatedPage .reswin-dataset:hover{background:var(--surface-hover)}
    #resonanceDedicatedPage .reswin-dataset-title{grid-column:1/-1;min-width:0}#resonanceDedicatedPage .reswin-dataset-title label{font-weight:600;color:var(--text-primary)}
    ...
    #resonanceDedicatedPage .respar-note,#resonanceDedicatedPage .respar-hint{font-size:11px;line-height:1.6;color:var(--text-secondary)}#resonanceDedicatedPage .respar-note{padding:7px;background:var(--surface-secondary);border:1px solid var(--border-subtle);border-radius:7px}
    #resonanceDedicatedPage .respar-preset-row label{display:flex;align-items:center;gap:7px}#resonanceDedicatedPage .respar-preset-row select{flex:1;height:29px}#resonanceDedicatedPage .respar-advanced{margin-top:7px}#resonanceDedicatedPage .respar-advanced summary{font-size:11px;color:var(--text-secondary);cursor:pointer;padding:4px 0}
    ...
    #resonanceDedicatedPage .respar-peak-legend{margin-top:8px;padding:7px;border:1px solid var(--border-subtle);border-radius:7px;background:var(--surface-secondary);font-size:11px;display:flex;flex-wrap:wrap;gap:5px 9px}#resonanceDedicatedPage .respar-peak-legend span{display:inline-flex;align-items:center;gap:4px}#resonanceDedicatedPage .respar-peak-legend i{width:9px;height:9px;border-radius:50%;display:inline-block}
    #resonanceDedicatedPage .respar-main-area{position:relative;min-width:0;min-height:0;overflow:hidden;display:grid;grid-template-rows:minmax(0,1fr) auto;background:var(--surface-primary)}
    #resonanceDedicatedPage .respar-main-workspace{position:relative;min-width:0;min-height:0;overflow:hidden;background:var(--surface-primary);display:grid;grid-template-columns:minmax(0,1fr);height:100%}#resonanceDedicatedPage .respar-plot-wrap{position:relative;min-width:0;min-height:0;overflow:hidden;background:var(--surface-primary)}
    ...
    #resonanceDedicatedPage .respar-main-tools{display:flex;gap:4px;padding:4px;background:color-mix(in srgb,var(--surface-primary) 94%,transparent);border:1px solid var(--border-subtle);border-radius:8px;box-shadow:var(--surface-shadow);flex:0 0 auto}#resonanceDedicatedPage .respar-main-tools button{min-height:0;padding:4px 8px;font-size:11px;background:var(--surface-primary);border:1px solid var(--border-subtle);border-radius:6px}
    #resonanceDedicatedPage .respar-main-legend{min-width:0;flex:1;height:36px;display:flex;align-items:center;gap:5px;overflow-x:auto;white-space:nowrap;padding:3px 5px;border:1px solid var(--border-subtle);border-radius:8px;background:color-mix(in srgb,var(--surface-primary) 94%,transparent);box-shadow:var(--surface-shadow)}
    #resonanceDedicatedPage .respar-legend-chip{flex:0 0 auto;display:inline-flex;align-items:center;gap:5px;height:26px;padding:3px 7px;border:1px solid var(--border-subtle);border-radius:6px;background:var(--surface-primary);color:var(--text-secondary);font-size:10px}#resonanceDedicatedPage .respar-legend-line{width:18px;height:0;border-top:3px solid currentColor}#resonanceDedicatedPage .respar-legend-line.reverse{border-top-style:dashed}
    #resonanceDedicatedPage .respar-main-svg{position:absolute;left:0;top:0;width:100%;height:100%;display:block;overflow:hidden}#resonanceDedicatedPage .respar-axis path,#resonanceDedicatedPage .respar-axis line{stroke:var(--text-tertiary)}#resonanceDedicatedPage .respar-axis text{fill:var(--text-secondary);font-size:11px}#resonanceDedicatedPage .respar-curve-hit{fill:none;stroke:transparent;stroke-width:14px;pointer-events:stroke;cursor:pointer}#resonanceDedicatedPage .respar-peak-hit{fill:transparent;stroke:none;pointer-events:all;cursor:pointer}#resonanceDedicatedPage .respar-peak-hit.editable{cursor:grab}
    #resonanceDedicatedPage .respar-axis-title{font-size:12px;fill:var(--text-primary);font-weight:520}#resonanceDedicatedPage .respar-peak-point{pointer-events:none}#resonanceDedicatedPage .respar-peak-point.dimmed{opacity:.12}#resonanceDedicatedPage .respar-physics-label{font-size:11px;font-weight:750;pointer-events:none}#resonanceDedicatedPage .respar-width-band{opacity:.075;pointer-events:none}#resonanceDedicatedPage .respar-width-line{stroke-width:1.5;pointer-events:none}#resonanceDedicatedPage .respar-width-handle{cursor:ew-resize;pointer-events:all}#resonanceDedicatedPage .respar-direct-box{pointer-events:none;stroke-width:1.4}#resonanceDedicatedPage .respar-direct-box.range{fill:rgba(58,96,246,.08);stroke:#3a60f6;stroke-dasharray:5 3}#resonanceDedicatedPage .respar-direct-box.zoom{fill:rgba(124,58,237,.07);stroke:#7c3aed;stroke-dasharray:3 3}#resonanceDedicatedPage .respar-persisted-range{fill:rgba(58,96,246,.04);stroke:#3a60f6;stroke-width:1;stroke-dasharray:5 4;pointer-events:none}
    #resonanceDedicatedPage .respar-status-row{display:flex;align-items:center;justify-content:space-between;gap:8px;min-height:38px;padding:4px 8px;border-top:1px solid var(--border-subtle);background:var(--surface-secondary)}#resonanceDedicatedPage .respar-summary{display:flex;gap:10px;font-size:11px;color:var(--text-secondary);white-space:nowrap;overflow:hidden}#resonanceDedicatedPage .respar-export-row{display:flex;gap:4px;flex:0 0 auto}#resonanceDedicatedPage .respar-export-row button{min-height:28px;padding:3px 7px;font-size:11px}
    #resonanceDedicatedPage .respar-hover-tip{position:absolute;z-index:80;max-width:320px}
    #resonanceDedicatedPage .respar-range-menu{position:absolute;z-index:90;width:290px;padding:8px;background:var(--surface-primary);border:1px solid var(--border-subtle);border-radius:9px;box-shadow:var(--surface-shadow-raised)}#resonanceDedicatedPage .respar-range-summary{font-size:11px;font-weight:650;color:var(--text-secondary);margin-bottom:6px}#resonanceDedicatedPage .respar-range-grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}#resonanceDedicatedPage .respar-range-grid button{font-size:11px;padding:4px 6px}#resonanceDedicatedPage .respar-range-footer{display:flex;justify-content:space-between;align-items:center;gap:8px;border-top:1px solid var(--border-subtle);margin-top:7px;padding-top:6px;font-size:10px;color:var(--text-tertiary)}
    #resonanceDedicatedPage .respar-range-identity{display:grid;grid-template-columns:1fr;gap:5px;margin-top:7px;padding-top:7px;border-top:1px solid var(--border-subtle);font-size:11px;color:var(--text-secondary)}#resonanceDedicatedPage .respar-range-identity select,#resonanceDedicatedPage .respar-range-identity input,#resonanceDedicatedPage .respar-range-identity button{width:100%;min-height:28px;font-size:11px}#resonanceDedicatedPage .respar-floating-panel{position:absolute;z-index:160;background:var(--surface-primary);border:1px solid var(--border-subtle);border-radius:10px;box-shadow:var(--surface-shadow-raised);resize:both;overflow:hidden;min-width:320px;min-height:220px}#resonanceDedicatedPage .respar-inspector-panel{right:24px;top:56px;width:390px;height:560px}#resonanceDedicatedPage .respar-group-panel{right:24px;bottom:44px;width:min(880px,calc(100% - 48px));height:620px}#resonanceDedicatedPage .respar-floating-header{height:36px;display:flex;align-items:center;justify-content:space-between;padding:0 9px;background:var(--surface-secondary);border-bottom:1px solid var(--border-subtle);font-weight:650;cursor:move;user-select:none}#resonanceDedicatedPage .respar-floating-header>div{display:flex;gap:5px}#resonanceDedicatedPage .respar-floating-header button{min-height:0;padding:3px 7px;font-size:11px}#resonanceDedicatedPage .respar-panel-close{border:none;background:transparent;font-size:18px!important}#resonanceDedicatedPage .respar-floating-body{height:calc(100% - 36px);overflow:auto;padding:10px}#resonanceDedicatedPage .respar-group-panel.collapsed{height:36px!important;min-height:36px!important;resize:none!important}#resonanceDedicatedPage .respar-group-panel.collapsed .respar-floating-body{display:none}
    ...
    #resonanceDedicatedPage .respar-group-toolbar{display:flex;justify-content:space-between;align-items:center;gap:10px;padding-bottom:8px;margin-bottom:10px;border-bottom:1px solid var(--border-subtle);font-size:11px;color:var(--text-secondary);position:sticky;top:0;background:var(--surface-primary);z-index:3}#resonanceDedicatedPage .respar-group-cols{display:flex;align-items:center;gap:4px}#resonanceDedicatedPage .respar-group-cols button{min-height:0;padding:3px 7px;font-size:11px}#resonanceDedicatedPage .respar-group-cols button.active{background:var(--accent-soft);border-color:#7c9cff;color:var(--accent-primary);font-weight:650}
    #resonanceDedicatedPage .reswin-group-grid{--reswin-group-cols:3;display:grid;grid-template-columns:repeat(var(--reswin-group-cols),minmax(0,1fr));gap:12px;align-items:start;width:100%}#resonanceDedicatedPage .reswin-group-card{border:1px solid var(--border-subtle);border-radius:8px;overflow:hidden;background:var(--surface-primary);min-width:0;display:grid;grid-template-rows:auto minmax(210px,var(--reswin-group-height,240px)) auto}#resonanceDedicatedPage .reswin-group-head{min-height:34px;height:auto;padding:5px 8px;box-sizing:border-box;font-size:12px;font-weight:650;background:var(--surface-secondary);display:flex;justify-content:space-between;align-items:center;gap:8px}#resonanceDedicatedPage .reswin-group-head button{min-height:24px;height:24px;box-sizing:border-box;padding:2px 6px;font-size:11px;display:inline-flex;align-items:center;justify-content:center}#resonanceDedicatedPage .reswin-group-legend{display:flex;flex-wrap:wrap;gap:5px 12px;padding:7px 8px 8px;border-top:1px solid var(--border-subtle);font-size:11px;color:var(--text-secondary)}
    #resonanceDedicatedPage .respar-derived{position:absolute;inset:0;z-index:130;background:var(--app-bg);overflow:auto;padding:12px}#resonanceDedicatedPage .respar-derived-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px}#resonanceDedicatedPage .respar-derived-header h3{margin:0;font-size:17px}#resonanceDedicatedPage .reswin-two-col,#resonanceDedicatedPage .reswin-gate-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}#resonanceDedicatedPage .reswin-medium-plot{height:380px}#resonanceDedicatedPage .reswin-feature-field-card{grid-column:1/-1}#resonanceDedicatedPage .reswin-feature-field-plot{height:360px;min-height:320px}#resonanceDedicatedPage .reswin-report{padding:12px;font-size:12px;line-height:1.65;color:var(--text-secondary)}
    ...
    #resonanceDedicatedPage .respar-dataset-item{display:grid;grid-template-columns:22px minmax(0,1fr);gap:6px;padding:7px;border-radius:7px;border:1px solid transparent;cursor:pointer;background:var(--surface-primary)}
    #resonanceDedicatedPage .respar-dataset-item:hover{background:var(--surface-hover)}#resonanceDedicatedPage .respar-dataset-item.is-excluded{opacity:.52;background:var(--surface-secondary)}#resonanceDedicatedPage .respar-dataset-item>.reswin-master{align-self:start;margin-top:2px}
    #resonanceDedicatedPage .respar-dataset-content{min-width:0}#resonanceDedicatedPage .respar-dataset-title{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:12px;color:var(--text-primary)}
    #resonanceDedicatedPage .respar-dataset-vg{display:flex;align-items:center;gap:4px;margin-top:3px;color:var(--text-secondary);font-size:11px}
    #resonanceDedicatedPage .respar-dataset-vg input{width:76px;height:24px;padding:2px 5px;border:1px solid var(--border-subtle);border-radius:5px;color:var(--text-primary);background:var(--surface-primary);font-size:11px;box-shadow:none}
    #resonanceDedicatedPage .respar-dataset-vg input:focus{outline:2px solid color-mix(in srgb,var(--accent-primary) 18%,transparent);border-color:var(--accent-primary)}
    #resonanceDedicatedPage .respar-scan-toggle{display:flex;gap:12px;margin-top:5px;font-size:11px;color:var(--text-secondary)}#resonanceDedicatedPage .respar-scan-toggle label{display:flex;align-items:center;gap:4px;cursor:pointer}
    #resonanceDedicatedPage .respar-dataset-transform{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:5px;margin-top:5px;font-size:10px;color:var(--text-secondary)}#resonanceDedicatedPage .respar-dataset-transform select{min-width:0;width:100%;height:24px;padding:2px 4px;border:1px solid var(--border-subtle);border-radius:5px;background:var(--surface-primary);color:var(--text-secondary);font-size:10px}
    #resonanceDedicatedPage .respar-range-menu{position:absolute;z-index:150;width:260px;padding:8px;border:1px solid var(--border-subtle);border-radius:9px;background:color-mix(in srgb,var(--surface-primary) 98%,transparent);box-shadow:var(--surface-shadow-raised);backdrop-filter:blur(4px)}
    #resonanceDedicatedPage .respar-range-summary{font-size:11px;font-weight:700;color:var(--text-primary);padding:2px 3px 7px;margin:0}#resonanceDedicatedPage .respar-range-grid{display:grid;grid-template-columns:1fr 1fr;gap:5px}#resonanceDedicatedPage .respar-range-grid button{min-height:29px;font-size:11px}
    #resonanceDedicatedPage .respar-range-footer{display:flex;align-items:center;justify-content:space-between;gap:7px;margin-top:7px;padding-top:6px;border-top:1px solid var(--border-subtle);font-size:9px;color:var(--text-tertiary)}
    #resonanceDedicatedPage .respar-range-identity{margin-top:7px;padding-top:7px;border-top:1px solid var(--border-subtle);display:grid;grid-template-columns:86px minmax(0,1fr);gap:5px;font-size:10px;color:var(--text-secondary)}
    #resonanceDedicatedPage .respar-range-identity>div{grid-column:1/-1;font-size:10px;font-weight:750}#resonanceDedicatedPage .respar-range-identity select,#resonanceDedicatedPage .respar-range-identity input{min-width:0;width:auto;height:29px;border:1px solid var(--border-subtle);border-radius:5px;padding:3px 6px;font-size:10px;background:var(--surface-primary)}#resonanceDedicatedPage .respar-range-identity button{grid-column:1/-1;min-height:29px;font-size:11px}
    #resonanceDedicatedPage .respar-inspector-body{font-size:12px;color:var(--text-primary)}#resonanceDedicatedPage .respar-inspector-section{border-bottom:1px solid var(--border-subtle);padding-bottom:10px;margin-bottom:10px}#resonanceDedicatedPage .respar-inspector-section h4{margin:0 0 7px;font-size:13px}
    #resonanceDedicatedPage .respar-inspector-kv{display:grid;grid-template-columns:130px minmax(0,1fr);gap:4px 8px;font-size:12px}#resonanceDedicatedPage .respar-inspector-kv .k{color:var(--text-secondary)}
    #resonanceDedicatedPage .respar-inspector-hint{font-size:12px;line-height:1.7;color:var(--text-secondary);margin-bottom:7px}
    #resonanceDedicatedPage .respar-inspector-action-grid{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:8px}
    #resonanceDedicatedPage .respar-peak-class-grid{display:grid;grid-template-columns:110px minmax(0,1fr);gap:7px;margin-top:8px}#resonanceDedicatedPage .respar-peak-class-grid label{display:flex;flex-direction:column;gap:3px;font-size:11px;color:var(--text-secondary);min-width:0}#resonanceDedicatedPage .respar-peak-class-grid input{min-width:0;width:100%;height:auto;border:1px solid var(--border-subtle);border-radius:6px;padding:5px;color:var(--text-primary);background:var(--surface-primary);font-size:11px;box-shadow:none}
    ...
    #resonanceDedicatedPage .respar-inspector-transform{margin-top:10px;border-top:1px solid var(--border-subtle);padding-top:10px}#resonanceDedicatedPage .respar-inspect-plot{height:210px;min-height:180px}
    #resonanceDedicatedPage .peak-category-palette{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin:8px 0}#resonanceDedicatedPage .peak-category-choice{display:flex;align-items:center;gap:7px;justify-content:flex-start;padding:6px 8px;min-width:0;font-size:11px}#resonanceDedicatedPage .peak-category-choice.selected{background:var(--accent-soft);border-color:var(--accent-primary);box-shadow:inset 0 0 0 1px var(--accent-primary)}#resonanceDedicatedPage .category-pair-swatch{display:inline-flex;align-items:center;gap:2px;flex:0 0 auto}#resonanceDedicatedPage .category-pair-swatch i{display:block;width:14px!important;height:18px!important;border-radius:5px!important;box-shadow:0 0 0 1px var(--border-subtle)!important}
    ...
    #resonanceDedicatedPage .respar-group-context{font-size:10px;font-weight:500;color:var(--text-tertiary);margin-left:6px}
