(() => {
  ...
  function create(context){
    ...
    function showRangeMenu(range,event){
      ...
      if(orderSelect){const rows=live.workspace.peakCategories||[];orderSelect.innerHTML=rows.map(c=>`<option value="${Number(c.order)}">${esc(c.label||`峰${c.order}`)}</option>`).join('');const selected=actions.peaksInRange(range)[0];if(selected&&rows.some(c=>Number(c.order)===Number(selected.peakOrder)))orderSelect.value=String(selected.peakOrder);}
    ...
    function renderLegend(curveColor){
      const host=$('#resparMainLegend');if(!host)return;host.innerHTML='';const current=actions.selectedSweep();
    ...
    function ensure(){
      ...
      mainSurface=factory.create(node,{
        ...
        onMarkerHover:({marker,event,phase})=>{const tip=$('#resparHoverTip');if(!tip)return;if(phase==='leave'){tip.classList.add('hidden');return;}const p=marker?.source;if(!p)return;if(phase==='enter'){tip.innerHTML=`<b>${esc(directionName(p.direction))} · ${esc(actions.peakLabel(p))}</b><br>Vg=${fmt(p.vg,4)} V · Vd=${fmt(p.v,6)} V<br>I=${fmt(p.i,6)} A${p.locked?' · 已锁定':''}`;tip.classList.remove('hidden');}const wrap=$('#resparMainPlotWrap'),wr=wrap?.getBoundingClientRect?.();if(wr){tip.style.left=`${event.clientX-wr.left+12}px`;tip.style.top=`${event.clientY-wr.top+12}px`; }},
    ...
    function dispose(){mainSurface?.dispose?.();mainSurface=null;}
