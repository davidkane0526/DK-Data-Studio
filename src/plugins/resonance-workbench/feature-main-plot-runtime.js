(() => {
  ...
  function create(context){
    ...
    function markerWidthSpec(marker){
      ...
      const metricLeft=Number(m.fwhmLeft),metricRight=Number(m.fwhmRight),savedLeft=Number(p.widthLeft),savedRight=Number(p.widthRight),halfResidual=Number(m.halfResidual);
      let left=Number.isFinite(metricLeft)?metricLeft:savedLeft,right=Number.isFinite(metricRight)?metricRight:savedRight;if(Number.isFinite(left)&&Number.isFinite(right)&&left>right)[left,right]=[right,left];
      ...
      const windowLeft=Number.isFinite(metricWindowLeft)?metricWindowLeft:rawWindowLeft,windowRight=Number.isFinite(metricWindowRight)?metricWindowRight:rawWindowRight;
      const hasWidth=Number.isFinite(left)&&Number.isFinite(right)&&right>left,hasWindow=Number.isFinite(windowLeft)&&Number.isFinite(windowRight)&&windowRight>windowLeft;if(!hasWidth&&!hasWindow)return null;
      const exactWidth=Number.isFinite(metricLeft)&&Number.isFinite(metricRight)&&metricRight>metricLeft&&Number.isFinite(halfResidual);
      return {left,right,yLeft:exactWidth?sign*(baselineAt(left)+halfResidual):NaN,yRight:exactWidth?sign*(baselineAt(right)+halfResidual):NaN,measureLine:exactWidth,provisional:hasWidth&&!exactWidth,windowLeft,windowRight,baseline:hasWindow&&Number.isFinite(Number(m.baselineSlope))?{x1:windowLeft,y1:sign*baselineAt(windowLeft),x2:windowRight,y2:sign*baselineAt(windowRight)}:null,handlePosition:'top'};
    }
    ...
    function ensure(){
      ...
      mainSurface=factory.create(node,{
        ...
        getSelectedCurveIds:()=>{
          ...
          if(focus?.type==='resonance.dataset'){const path=String(focus.ref?.datasetPath||'');return actions.visibleSweeps().filter(sw=>String(sw.datasetPath)===path).map(sw=>String(sw.id));}
