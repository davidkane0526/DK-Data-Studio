  ...
  const PAGE_HTML=`
        ...
        <div class="analysis-page-body data-center-body">
          <aside class="dc-card dc-artifact-pane dkds-surface">
            <div class="dc-section-head dkds-surface-header"><div class="dkds-surface-heading"><strong>数据对象</strong><span id="dcArtifactCount" class="dkds-meta">0</span></div><div class="dkds-surface-actions"><select id="dcAssignmentFilter" class="dc-assignment-filter dkds-field-control" title="按分析用途筛选"><option value="all">全部用途</option></select></div></div>
            <div class="dc-filter-stack">
              <div class="dc-filter-row"><select id="dcLineageFilter" class="dkds-field-control" title="按数据层级筛选"><option value="all">全部层级</option><option value="raw">原始数据</option><option value="derived">派生数据</option></select><select id="dcFieldFilter" class="dc-field-filter dkds-field-control" title="按实际列标题 / 数据字段筛选"><option value="">全部字段</option></select></div>
              <div class="dc-selection-tools" data-dkds-tooltip="Shift 连选 · Ctrl/Cmd 多选"><button id="dcSelectAllBtn" type="button">全选</button><button id="dcInvertSelectionBtn" type="button">反选</button><button id="dcClearSelectionBtn" type="button">清除</button></div>
            </div>
