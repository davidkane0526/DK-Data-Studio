(() => {
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    const artifactSelectionView=controller?.interaction?.bindView?.('data-center-artifacts',$('#dcArtifactList'),{selector:'.dc-artifact-item',itemVariant:'row',itemKey:el=>el.dataset.artifactId||el.dataset.selectionKey,entityLinked:true,revealFocus:true,onActivate:({element})=>{const id=String(element.dataset.artifactId||'');const a=artifacts().find(row=>String(row.id)===id);if(!a)return;state.activeArtifactId=a.id;lastExecution=null;controller?.select?.({id:a.id,kind:a.kind,name:a.name},{source:'data-center-artifact'});renderAllUi();}});
