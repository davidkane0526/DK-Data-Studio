  ...
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    if(!ctx.runtime.isAuxiliaryWindow&&ctx.ui.menus?.add){
      const menuRows=[
        ...
        ['dc-export-chart-png','数据中心图形预览 · PNG',30,()=>ctx.ui.charts.saveImage('dcChart','data_center_chart','png')],
    ...
    function renderAllUi(){renderArtifacts();renderPreview();renderFormula();refreshProviderSelect();renderSteps();renderSavedRecipes();renderProvenance();renderChartControls();dom.frame(()=>{try{ctx.ui.charts.resize($('#dcChart'));}catch{}});}
...
$('#dcApplyFormula').onclick=applyFormula;$$('[data-dc-tab]').forEach(b=>b.onclick=()=>switchTab(b.dataset.dcTab));$('#dcStepType').onchange=refreshProviderSelect;$('#dcAddStep').onclick=addStep;$('#dcSaveRecipe').onclick=saveRecipe;$('#dcLoadRecipe').onclick=loadRecipe;$('#dcSavedRecipe').onchange=loadRecipe;$('#dcChartProvider').onchange=renderChartParams;$('#dcRenderChart').onclick=renderChart;$('#dcCopyProvenance').onclick=()=>ctx.io.clipboard.writeText(JSON.stringify(activeArtifact()?.provenance||[],null,2));
    ctx.events.on('data:artifacts-changed',()=>{if(!page.classList.contains('hidden'))renderAllUi();});ctx.events.on('layout:resize',()=>{if(!page.classList.contains('hidden'))dom.frame(()=>{try{ctx.ui.charts.resize($('#dcChart'));}catch{}});});const platformOff=ctx.platform.onChange(()=>{if(!page.classList.contains('hidden'))dom.frame(()=>{try{ctx.ui.charts.resize($('#dcChart'));}catch{}});});
    ...
    return {deactivate(){platformOff?.();quickPanel?.destroy?.();chartPanel?.destroy?.();stepPanels.forEach(h=>h?.destroy?.());try{ctx.ui.charts.purge($('#dcChart'));}catch{}}};
  }
