  ...
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    ctx.charts.register('xy-line',{
      ...
      render({container,artifact,parameters}){
        ...
        return ctx.ui.scientificPlot.react(container,traces,{margin:{l:72,r:22,t:30,b:62},xaxis:{title:`${x.name}${x.unit?` (${x.unit})`:''}`,automargin:true,gridcolor:'#edf0f5'},yaxis:{title:ys.length===1?`${ys[0].name}${ys[0].unit?` (${ys[0].unit})`:''}`:'Value',automargin:true,gridcolor:'#edf0f5'},legend:{orientation:'h',y:1.08},showlegend:parameters.showLegend!==false,hovermode:'closest',paper_bgcolor:'#fff',plot_bgcolor:'#fff',autosize:true},{responsive:true,displaylogo:false,scrollZoom:true},{interaction:controller?.interaction,source:'data-center-chart',renderKey:`data-center:${artifact.id}:${ctx.data.artifacts.revision?.(artifact.kind)||0}:${JSON.stringify(parameters||{})}`,traceEntity:(trace,index)=>{const y=ys[index];return y?{id:`data-center.column:${artifact.id}:${y.key}`,type:'data-center.column',parents:[artifact.id],label:y.name,ref:{artifactId:artifact.id,column:y.key},value:{id:`data-center.column:${artifact.id}:${y.key}`,tableId:artifact.id,key:y.key,name:y.name,unit:y.unit,role:y.role}}:{id:artifact.id,type:'data-center.artifact',label:artifact.name};}});
      }
    });
    ...
    ctx.ui.styles.add('data-center',`
      .data-center-body:not(.dkds-unified-workbench-body){display:grid;grid-template-columns:290px minmax(0,1fr);gap:12px;padding:12px;overflow:auto;background:var(--app-bg)}.dc-card{background:var(--surface-primary);border:1px solid var(--border-subtle);border-radius:10px;min-width:0;overflow:hidden}.dc-artifact-pane{position:sticky;top:0;align-self:start;max-height:calc(100vh - 105px);display:flex;flex-direction:column}.dc-section-head,.dc-tool-title{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border-bottom:1px solid var(--border-subtle);background:var(--surface-secondary)}.dc-section-head>.dc-section-copy,.dc-tool-title>div:not(.dc-inline-actions):not(.dc-chart-toolbar){display:flex;flex-direction:column;gap:2px}.dc-section-head strong,.dc-tool-title strong{font-size:12px;color:var(--text-primary)}.dc-section-head span,.dc-tool-title span{font-size:9px;color:var(--text-tertiary)}.dc-assignment-filter{max-width:145px;height:28px;border:1px solid var(--border-subtle);border-radius:6px;background:var(--surface-primary);color:var(--text-secondary);font-size:9px;padding:0 6px}.dc-data-nav{display:flex;align-items:center;gap:6px;padding:6px 8px;border-bottom:1px solid var(--border-subtle);background:var(--surface-primary);min-width:0}.dc-breadcrumb{display:flex;align-items:center;gap:4px;min-width:0;font-size:8.5px;color:var(--text-tertiary);white-space:nowrap}.dc-breadcrumb>span:first-child{font-weight:650;color:var(--text-secondary)}.dc-breadcrumb-sep{color:var(--text-tertiary)}.dc-breadcrumb select,.dc-field-filter{height:26px;min-height:26px;border:1px solid var(--border-subtle);border-radius:6px;background:var(--surface-primary);color:var(--text-secondary);font-size:8.5px;padding:0 22px 0 6px;min-width:0}.dc-breadcrumb select{max-width:92px}.dc-field-filter{flex:1;max-width:145px;margin-left:auto}.dc-selection-tools{display:flex;align-items:center;justify-content:space-between;gap:6px;padding:6px 8px;border-bottom:1px solid var(--border-subtle);background:var(--surface-secondary);font-size:8.5px;color:var(--text-tertiary)}.dc-selection-tools>div{display:flex;gap:4px}.dc-selection-tools button{min-height:25px;padding:3px 7px;font-size:8.5px}.dc-artifact-list{overflow:auto;padding:7px;min-height:180px;outline:none}.dc-artifact-item{display:block;width:100%;text-align:left;padding:8px;margin-bottom:6px;border:1px solid var(--border-subtle);border-radius:8px;background:var(--surface-primary);cursor:pointer}.dc-artifact-item.dkds-selection-selected:not(.dkds-selection-focused){border-color:color-mix(in srgb,var(--accent,var(--accent-primary)) 35%,var(--border-subtle))!important;background:color-mix(in srgb,var(--accent,var(--accent-primary)) 5%,var(--surface-primary))!important}.dc-artifact-name{font-size:10px;font-weight:700;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.dc-artifact-item.is-excluded{opacity:.58;background:var(--surface-secondary)}.dc-artifact-item.is-excluded .dc-artifact-name{text-decoration:line-through;text-decoration-thickness:1px}.dc-artifact-meta{font-size:8.5px;color:var(--text-tertiary);margin-top:3px}.dc-main{display:flex;flex-direction:column;gap:12px;min-width:0}.dc-preview-actions{display:flex!important;flex-direction:row!important;align-items:center;gap:7px;flex-wrap:nowrap;justify-content:flex-end;min-width:max-content}.dc-preview-actions>button{height:30px;min-height:30px;align-self:center;white-space:nowrap}.dc-tabs{display:flex;gap:4px}.dc-tabs button.active{background:var(--accent-primary);color:var(--accent-contrast)}.dc-table-preview{max-height:250px;overflow:auto}.dc-json-preview{margin:0;padding:10px 12px;font:9px/1.5 ui-monospace,Consolas,monospace;color:var(--text-secondary);white-space:pre-wrap;word-break:break-word}.dc-preview-table{width:100%;border-collapse:collapse;font-size:9px;font-variant-numeric:tabular-nums}.dc-preview-table th{position:sticky;top:0;background:var(--surface-secondary);z-index:2}.dc-preview-table th,.dc-preview-table td{border-bottom:1px solid var(--border-subtle);padding:5px 7px;text-align:right;white-space:nowrap}.dc-preview-table th:first-child,.dc-preview-table td:first-child{text-align:center}.dc-tool-pane{padding-bottom:10px}.dc-tool-pane>.schema-parameter-panel,.dc-chart-pane>.schema-parameter-panel{padding:10px 12px}.dc-inline-actions{display:flex;gap:5px;align-items:center}.dc-formula-refs{padding:0 12px;display:flex;gap:5px;flex-wrap:wrap}.dc-ref-chip{font-family:ui-monospace,Consolas,monospace;font-size:9px;padding:3px 6px;border:1px solid var(--border-subtle);border-radius:999px;background:var(--surface-secondary);cursor:pointer}.dc-recipe-bar,.dc-add-step{display:flex;gap:7px;align-items:end;padding:9px 12px;border-bottom:1px solid var(--border-subtle)}.dc-recipe-bar label{display:flex;flex-direction:column;gap:3px;font-size:9px;color:var(--text-secondary)}.dc-recipe-bar input,.dc-recipe-bar select,.dc-add-step select{height:30px;border:1px solid var(--border-strong);border-radius:6px;padding:3px 7px;min-width:150px}.dc-workflow-steps{padding:10px 12px}.dc-step-card{border:1px solid var(--border-subtle);border-radius:8px;margin-bottom:8px;overflow:hidden}.dc-step-head{display:flex;align-items:center;justify-content:space-between;padding:7px 9px;background:var(--surface-secondary);border-bottom:1px solid var(--border-subtle)}.dc-step-head strong{font-size:10px}.dc-step-actions{display:flex;gap:4px}.dc-step-params{padding:7px}.dc-workflow-status{margin:0 12px;padding:8px;border:1px solid var(--border-subtle);border-radius:7px;background:var(--surface-secondary);font-size:9px;color:var(--text-secondary)}.dc-workflow-status.running{border-color:#9db1ee;background:var(--accent-soft)}.dc-workflow-status.done{border-color:color-mix(in srgb,var(--success) 35%,var(--border-subtle));background:var(--success-soft);color:var(--success)}.dc-workflow-status.error{border-color:color-mix(in srgb,var(--danger) 35%,var(--border-subtle));background:var(--danger-soft);color:var(--danger)}.dc-provenance-list{padding:10px 12px;max-height:430px;overflow:auto}.dc-prov-item{display:grid;grid-template-columns:120px minmax(0,1fr);gap:10px;padding:8px 0;border-bottom:1px solid var(--border-subtle)}.dc-prov-time{font-size:8px;color:var(--text-tertiary)}.dc-prov-main strong{display:block;font-size:10px;color:var(--text-primary)}.dc-prov-main div{font-size:8.5px;color:var(--text-secondary);margin-top:2px;word-break:break-word}.dc-chart{height:430px;min-height:320px}.schema-parameter-panel{display:grid;grid-template-columns:repeat(2,minmax(160px,1fr));gap:8px 10px}.schema-param-group{display:contents}.schema-param-group-title{grid-column:1/-1;font-size:10px;font-weight:700;color:var(--text-secondary)}.schema-param-field{display:flex;flex-direction:column;gap:3px;min-width:0}.schema-param-label{font-size:9px;color:var(--text-secondary)}.schema-param-field input,.schema-param-field select,.schema-param-field textarea{width:100%;box-sizing:border-box;border:1px solid var(--border-strong);border-radius:6px;padding:5px 7px;font-size:10px;background:var(--surface-primary);min-height:30px}.schema-param-field textarea{resize:vertical}.schema-param-help{font-size:8px;color:var(--text-tertiary);line-height:1.4}.schema-param-error{min-height:10px;font-size:8px;color:var(--danger)}.schema-param-field.has-error input,.schema-param-field.has-error select,.schema-param-field.has-error textarea{border-color:var(--danger)}.required{color:var(--danger)}@media(max-width:1000px){.data-center-body:not(.dkds-unified-workbench-body){grid-template-columns:230px minmax(0,1fr)}.schema-parameter-panel{grid-template-columns:1fr}}.dkds-size-compact .data-center-body:not(.dkds-unified-workbench-body){grid-template-columns:1fr;padding:7px}.dkds-size-compact .dc-artifact-pane{position:static;max-height:250px}.dkds-size-compact .dc-recipe-bar,.dkds-size-compact .dc-add-step,.dkds-size-compact .dc-tool-title{flex-wrap:wrap}.dkds-size-compact .dc-chart{height:360px}
    `);
    ...
    function assignmentArtifacts(){return ctx.data.artifacts.list({includeTransient:true}).filter(a=>assignmentMatches(a));}
    ...
    function renderDataNavigation(){
      ...
      const current=fieldFilter,rows=availableArtifactFields();field.innerHTML='<option value="">全部字段</option>'+rows.map(row=>`<option value="${esc(row.field)}">${esc(row.field)}${row.count>1?` · ${row.count}`:''}</option>`).join('');
    ...
    let artifactSelectionAnchorId='';
    function artifactSelectionItem(a){return {type:'data-center.artifact',id:String(a?.id||''),value:{id:a?.id,kind:a?.kind,name:a?.name}};}
    function selectedArtifactIds(){return new Set((controller?.getSelection?.()?.items||[]).filter(row=>row?.type==='data-center.artifact').map(row=>String(row.id||'')).filter(Boolean));}
    function selectArtifacts(rows,{focusId='',source='data-center-multi'}={}){
      const unique=[];const seen=new Set();for(const a of rows||[]){const id=String(a?.id||'');if(!id||seen.has(id))continue;seen.add(id);unique.push(a);}
      if(focusId){const idx=unique.findIndex(a=>String(a.id)===String(focusId));if(idx>=0)unique.push(unique.splice(idx,1)[0]);}
      controller?.selection?.selectMany?.(unique.map(artifactSelectionItem),{source});
      return unique;
    }
    function selectAllVisibleArtifacts(){const rows=artifacts();selectArtifacts(rows,{focusId:state.activeArtifactId,source:'data-center-select-all'});artifactSelectionAnchorId=String(state.activeArtifactId||rows.at(-1)?.id||'');artifactSelectionView?.refresh?.({reveal:false});return rows.length;}
    function invertVisibleArtifactSelection(){const rows=artifacts(),selected=selectedArtifactIds(),next=rows.filter(a=>!selected.has(String(a.id)));selectArtifacts(next,{focusId:next.at(-1)?.id||'',source:'data-center-invert'});if(next.length){state.activeArtifactId=next.at(-1).id;artifactSelectionAnchorId=String(next.at(-1).id);}artifactSelectionView?.refresh?.({reveal:false});return next.length;}
    function clearVisibleArtifactSelection(){controller?.clearSelection?.({source:'data-center-clear'});artifactSelectionAnchorId='';artifactSelectionView?.refresh?.({reveal:false});return true;}
    function renderAssignmentFilter(){const select=$('#dcAssignmentFilter');if(!select)return;const targets=assignmentTargets(),value=assignmentFilter;select.innerHTML='<option value="all">全部用途</option><option value="unassigned">仅数据中心</option>'+targets.map(row=>`<option value="${esc(row.id)}">${esc(row.icon||'◇')} ${esc(row.label||row.name||row.id)}</option>`).join('');select.value=[...select.options].some(option=>option.value===value)?value:'all';assignmentFilter=select.value;}
    async function setArtifactAssignments(a,ids){if(!isImportedSource(a)||!sourceCapability?.setAssignments)return false;const ref=sourceRef(a);if(!ref.path&&!ref.sourcePath&&!ref.artifactId)return false;await sourceCapability.setAssignments(ref,ids);renderAllUi();return true;}
    function assignmentActionItems(a){if(!isImportedSource(a)||!sourceCapability?.setAssignments)return [];const targets=assignmentTargets(),raw=artifactAssignments(a)||[],expanded=raw.includes('*')?targets.map(row=>String(row.id)):raw.slice(),selected=new Set(expanded);return [
      {type:'separator'},
      {id:'assignment-title',label:'数据用途',icon:'↦',enabled:false},
      ...targets.map(row=>({id:`assignment:${row.id}`,label:`${selected.has(String(row.id))?'✓':'○'} ${row.label||row.name||row.id}`,onInvoke:()=>void setArtifactAssignments(a,selected.has(String(row.id))?expanded.filter(id=>id!==String(row.id)):[...new Set([...expanded,String(row.id)])])})),
    ...
    async function renameArtifact(a=activeArtifact()){
      if(!a)return false;const label=window.prompt('修改数据标签',String(a.name||''));if(label===null)return false;const name=String(label||'').trim();if(!name)return false;
      if(isImportedSource(a)&&sourceCapability?.rename){await sourceCapability.rename(sourceRef(a),name);}
      else{const next={...D.deepClone(a),name};if(historyCapability?.commitArtifactMutation)await historyCapability.commitArtifactMutation({label:`修改数据标签 · ${name}`,before:{upserts:[D.deepClone(a)],removedIds:[]},after:{upserts:[next],removedIds:[]}});else ctx.data.artifacts.upsert(next);}
      state.activeArtifactId=a.id;renderAllUi();ctx.status.set(`数据标签已修改为：${name}`);return true;
    }
    async function toggleArtifactExcluded(a=activeArtifact()){
      if(!a)return false;const next=!isExcluded(a);
      if(isImportedSource(a)&&sourceCapability?.setExcluded){await sourceCapability.setExcluded(sourceRef(a),next);}
      else{const changed={...D.deepClone(a),metadata:{...(a.metadata||{}),excluded:next}};if(historyCapability?.commitArtifactMutation)await historyCapability.commitArtifactMutation({label:`${next?'排除':'恢复'}数据 · ${a.name||a.id}`,before:{upserts:[D.deepClone(a)],removedIds:[]},after:{upserts:[changed],removedIds:[]}});else ctx.data.artifacts.upsert(changed);}
      state.activeArtifactId=a.id;renderAllUi();ctx.status.set(`${next?'已排除':'已恢复'}数据：${a.name||a.id}`);return true;
    }
    async function deleteArtifact(a=activeArtifact()){
      if(!a)return false;if(!window.confirm(`删除“${a.name||a.id}”？`))return false;
      if(isImportedSource(a))return removeActiveSource(a);
      const lineage=ctx.data.artifacts.lineage?.(a.id),removed=[...(lineage?.descendants||[]),a].filter(Boolean),ids=removed.map(row=>String(row.id));if(historyCapability?.commitArtifactMutation)await historyCapability.commitArtifactMutation({label:`删除数据对象 · ${a.name||a.id}`,before:{upserts:removed.map(D.deepClone),removedIds:[]},after:{upserts:[],removedIds:ids}});else ctx.data.artifacts.batch?.(api=>{for(const id of ids.slice().reverse())api.remove?.(id);});state.activeArtifactId='';lastExecution=null;renderAllUi();ctx.status.set(`已删除数据对象：${a.name||a.id}`);return true;
    }
    function dataActionItems(a=activeArtifact()){
      if(!a)return [];
      return [
        {id:'rename',label:'修改标签',icon:'✎',onInvoke:()=>void renameArtifact(a)},
        {id:'exclude',label:isExcluded(a)?'恢复参与分析':'排除',icon:isExcluded(a)?'○':'⊘',onInvoke:()=>void toggleArtifactExcluded(a)},
        ...assignmentActionItems(a),
        {type:'separator'},
        {id:'delete',label:'删除',icon:'×',onInvoke:()=>void deleteArtifact(a)}
      ];
    }
    function openDataActions(event,a=activeArtifact()){
      if(!a)return null;return ctx.ui.contextMenus?.open?.({x:event?.clientX??0,y:event?.clientY??0,items:dataActionItems(a),context:{artifact:a}})||null;
    }
    ...
    function renderArtifacts(){const list=$('#dcArtifactList');if(!list)return;renderAssignmentFilter();renderDataNavigation();const rows=artifacts();$('#dcArtifactCount').textContent=`${rows.length} 个`;list.innerHTML='';if(!rows.length){list.innerHTML='<div class="empty-state">当前筛选条件下没有数据对象。可清除用途/层级/字段筛选，或从主界面导入并调整“数据用途”。</div>';return;}for(const a of rows){ctx.data.entities?.projectArtifact?.(a);const sum=D.summarize(a);const b=ctx.ui.dom.create('button');b.type='button';b.className=`dc-artifact-item${isExcluded(a)?' is-excluded':''}`;b.dataset.selectionKey=String(a.id);b.dataset.artifactId=String(a.id);const dimensions=a.kind==='data.table'?`${sum.rows??'—'} 行 · ${sum.columns??'—'} 列`:a.kind==='data.series'||a.kind==='data.sweep'||a.kind==='data.transform'?`${sum.length??'—'} 点`:a.kind,usage=assignmentSummary(a);b.innerHTML=`<div class="dc-artifact-name">${esc(a.name)}</div><div class="dc-artifact-meta">${esc(a.kind)} · ${esc(dimensions)} · provenance ${sum.provenance}${usage?` · 用途 ${esc(usage)}`:''}${isExcluded(a)?' · 已排除':''}</div>`;list.appendChild(b);}}
    function bindPreviewTable(){const table=$('#dcTablePreview table');if(table)ctx.ui.tables?.bind?.('data-center-preview',table,{persist:true});return table;}
    function renderPreview(){const a=activeArtifact();renderDataAction(a);$('#dcActiveName').textContent=a?.name||'未选择数据';$('#dcActiveMeta').textContent=a?`${a.kind} · ${a.id}`:'—';const host=$('#dcTablePreview');if(!a){host.innerHTML='<div class="empty-state">暂无数据对象</div>';return;}if(a.kind==='data.table'){const n=Math.min(a.rowCount,18);host.innerHTML=`<table class="dc-preview-table"><thead><tr><th>#</th>${a.columns.map(c=>`<th>${esc(c.name)}${c.unit?` (${esc(c.unit)})`:''}</th>`).join('')}</tr></thead><tbody>${Array.from({length:n},(_,r)=>`<tr><td>${r+1}</td>${a.columns.map(c=>`<td>${Number.isFinite(c.values[r])?Number(c.values[r]).toPrecision(7):esc(c.values[r])}</td>`).join('')}</tr>`).join('')}</tbody></table>${a.rowCount>n?`<div class="import-diagnosis">预览前 ${n} / ${a.rowCount} 行</div>`:''}`;bindPreviewTable();return;}if(a.kind==='result.analysis'){const table=a.tables?.[0],rows=table?.rows||[];if(rows.length){const keys=Object.keys(rows[0]);const n=Math.min(rows.length,18);host.innerHTML=`<div class="import-diagnosis">${esc(JSON.stringify(a.summary||{}))}</div><table class="dc-preview-table"><thead><tr>${keys.map(k=>`<th>${esc(k)}</th>`).join('')}</tr></thead><tbody>${rows.slice(0,n).map(row=>`<tr>${keys.map(k=>`<td>${Number.isFinite(row[k])?Number(row[k]).toPrecision(7):esc(row[k])}</td>`).join('')}</tr>`).join('')}</tbody></table>`;bindPreviewTable();return;}}host.innerHTML=`<pre class="dc-json-preview">${esc(JSON.stringify(D.summarize(a),null,2))}</pre>`;}
    ...
    async function applyFormula(){const a=activeArtifact();if(!a||a.kind!=='data.table'){ctx.status.set('公式派生列需要选择 DataTable。');return;}const valid=quickPanel.validate();if(!valid.ok){ctx.status.set('公式参数存在错误。');return;}try{const recipe=ctx.workflow.buildSequentialRecipe({id:'quick.formula',name:'Quick formula',steps:[{type:'processor',provider:'formula.derived-column',parameters:quickPanel.getValue()}]});const exec=await ctx.workflow.run(recipe,{inputs:{main:a}});const out=exec.outputs.result;if(D.isArtifact(out)){ctx.data.artifacts.upsert(out);state.activeArtifactId=out.id;lastExecution=exec;ctx.status.set(`已生成派生 DataTable：${out.name}`);renderAllUi();}}catch(err){ctx.status.set(`公式计算失败：${err.message}`);}}
    ...
    const artifactSelectionView=controller?.interaction?.bindView?.('data-center-artifacts',$('#dcArtifactList'),{selector:'.dc-artifact-item',itemVariant:'row',itemKey:el=>el.dataset.artifactId||el.dataset.selectionKey,entityLinked:true,revealFocus:true,onActivate:({event,element})=>{
      ...
      const additive=!!(event?.ctrlKey||event?.metaKey),range=!!event?.shiftKey;
      if(range){const anchorIndex=rows.findIndex(row=>String(row.id)===artifactSelectionAnchorId),index=rows.findIndex(row=>String(row.id)===id);if(anchorIndex>=0&&index>=0){const lo=Math.min(anchorIndex,index),hi=Math.max(anchorIndex,index),span=rows.slice(lo,hi+1);if(additive){const selected=selectedArtifactIds();selectArtifacts(rows.filter(row=>selected.has(String(row.id))||span.some(item=>String(item.id)===String(row.id))),{focusId:id,source:'data-center-range-add'});}else selectArtifacts(span,{focusId:id,source:'data-center-range'});}else controller?.select?.({id:a.id,kind:a.kind,name:a.name},{source:'data-center-artifact'});}
      else if(additive){controller?.selection?.select?.(artifactSelectionItem(a),{toggle:true,source:'data-center-toggle'});artifactSelectionAnchorId=id;}
      else{controller?.select?.({id:a.id,kind:a.kind,name:a.name},{source:'data-center-artifact'});artifactSelectionAnchorId=id;}
      renderAllUi();
    }});
    const artifactContextBehavior=ctx.ui.interactionBehaviors?.create?.('data-center-artifact-context',{bindings:[{id:'data-center.artifact.context',gesture:'context',target:'artifact',intent:'context-menu',contextActions:context=>dataActionItems(context.artifact)}]});
    artifactContextBehavior?.bind?.($('#dcArtifactList'),{selector:'.dc-artifact-item',gestures:['context'],target:'artifact',targetId:({element})=>String(element.dataset.artifactId||''),payload:({element})=>({artifact:ctx.data.artifacts.get(String(element.dataset.artifactId||''))}),beforeRoute:({input})=>{const a=input.payload?.artifact;if(!a)return;state.activeArtifactId=a.id;lastExecution=null;controller?.select?.({id:a.id,kind:a.kind,name:a.name},{source:'data-center-context'});}});
