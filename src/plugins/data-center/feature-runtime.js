(() => {
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    function artifacts(){return ctx.data.artifacts.list({includeTransient:true}).filter(a=>assignmentMatches(a));}
    ...
    async function renameArtifact(a=activeArtifact()){
      ...
      else ctx.data.artifacts.upsert({...D.deepClone(a),name});
    ...
    async function toggleArtifactExcluded(a=activeArtifact()){
      ...
      else ctx.data.artifacts.upsert({...D.deepClone(a),metadata:{...(a.metadata||{}),excluded:next}});
    ...
    async function deleteArtifact(a=activeArtifact()){
      ...
      const lineage=ctx.data.artifacts.lineage?.(a.id);const ids=[...(lineage?.descendants||[]).map(row=>row.id),a.id];ctx.data.artifacts.batch?.(api=>{for(const id of ids.reverse())api.remove?.(id);});state.activeArtifactId='';lastExecution=null;renderAllUi();ctx.status.set(`已删除数据对象：${a.name||a.id}`);return true;
    }
