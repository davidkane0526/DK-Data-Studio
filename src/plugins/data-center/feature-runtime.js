(() => {
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    let assignmentFilter='all',tagFilters=new Set();
    ...
    function artifactTags(a){return (D.artifactDataTags?.(a)||[]).filter(tag=>!['x','y','group'].includes(String(tag)));}
    function tagMatches(a){if(!tagFilters.size)return true;const tags=new Set(artifactTags(a));return [...tagFilters].every(tag=>tags.has(tag));}
    function assignmentArtifacts(){return ctx.data.artifacts.list({includeTransient:true}).filter(a=>assignmentMatches(a));}
    function artifacts(){return assignmentArtifacts().filter(tagMatches);}
    function availableArtifactTags(){const counts=new Map();for(const a of assignmentArtifacts())for(const tag of artifactTags(a))counts.set(tag,(counts.get(tag)||0)+1);const priority=['id','ig','vg','vd','vth','didv','dvdi','resistance','conductance','time','temperature'];return [...counts.entries()].map(([tag,count])=>({tag,count})).sort((a,b)=>{const ai=priority.indexOf(a.tag),bi=priority.indexOf(b.tag);return (ai<0?999:ai)-(bi<0?999:bi)||b.count-a.count||a.tag.localeCompare(b.tag);});}
    function renderTagFilter(){const host=$('#dcTagChips'),clear=$('#dcTagClearBtn');if(!host||!clear)return;host.replaceChildren();for(const row of availableArtifactTags()){const b=ctx.ui.dom.create('button');b.type='button';b.className=`dc-tag-chip${tagFilters.has(row.tag)?' active':''}`;b.dataset.tag=row.tag;b.textContent=`${D.dataTagLabel?.(row.tag)||row.tag}${row.count>1?` · ${row.count}`:''}`;b.title=`按 ${row.tag} 标签筛选（多标签需同时满足）`;b.onclick=()=>{if(tagFilters.has(row.tag))tagFilters.delete(row.tag);else tagFilters.add(row.tag);renderAllUi();};host.appendChild(b);}clear.disabled=!tagFilters.size;clear.onclick=()=>{tagFilters.clear();renderAllUi();};}
