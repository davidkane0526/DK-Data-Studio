(() => {
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    const sourceCapability=ctx.capabilities.proxy('core.data-sources');
    ...
    function isImportedSource(a){return !!a&&a.kind==='data.table'&&a.metadata?.adapter==='legacy-dataset'&&!!String(a.metadata?.legacyDatasetPath||'');}
    ...
    async function removeActiveSource(target=activeArtifact()){
      ...
      const path=String(a.metadata.legacyDatasetPath||'');
      try{
        const result=await sourceCapability.remove([{path}]);
    ...
    async function renameArtifact(a=activeArtifact()){
      ...
      if(isImportedSource(a)&&sourceCapability?.rename){await sourceCapability.rename({path:String(a.metadata.legacyDatasetPath||'')},name);}
    ...
    async function toggleArtifactExcluded(a=activeArtifact()){
      ...
      if(isImportedSource(a)&&sourceCapability?.setExcluded){await sourceCapability.setExcluded({path:String(a.metadata.legacyDatasetPath||'')},next);}
    ...
    function renderArtifacts(){const list=$('#dcArtifactList');if(!list)return;const rows=artifacts();$('#dcArtifactCount').textContent=`${rows.length} 个`;list.innerHTML='';if(!rows.length){list.innerHTML='<div class="empty-state">先从主界面导入数据。导入结果会自动映射为标准 DataTable。</div>';return;}for(const a of rows){ctx.data.entities?.projectArtifact?.(a);const sum=D.summarize(a);const b=ctx.ui.dom.create('button');b.type='button';b.className=`dc-artifact-item${isExcluded(a)?' is-excluded':''}`;b.dataset.selectionKey=String(a.id);b.dataset.artifactId=String(a.id);const dimensions=a.kind==='data.table'?`${sum.rows??'—'} 行 · ${sum.columns??'—'} 列`:a.kind==='data.series'||a.kind==='data.sweep'||a.kind==='data.transform'?`${sum.length??'—'} 点`:a.kind;b.innerHTML=`<div class="dc-artifact-name">${esc(a.name)}</div><div class="dc-artifact-meta">${esc(a.kind)} · ${esc(dimensions)} · provenance ${sum.provenance}${isExcluded(a)?' · 已排除':''}</div>`;list.appendChild(b);}}
...
$('#dcApplyFormula').onclick=applyFormula;const dataActionsBtn=$('#dcDataActionsBtn');if(dataActionsBtn)dataActionsBtn.onclick=event=>{const rect=dataActionsBtn.getBoundingClientRect();openDataActions({clientX:rect.left,clientY:rect.bottom+4},activeArtifact());event.stopPropagation();};const artifactList=$('#dcArtifactList');if(artifactList)artifactList.oncontextmenu=event=>{const item=event.target?.closest?.('.dc-artifact-item');if(!item)return;const a=artifacts().find(row=>String(row.id)===String(item.dataset.artifactId||''));if(!a)return;event.preventDefault();event.stopPropagation();state.activeArtifactId=a.id;controller?.select?.({id:a.id,kind:a.kind,name:a.name},{source:'data-center-context'});openDataActions(event,a);};$$('[data-dc-tab]').forEach(b=>b.onclick=()=>switchTab(b.dataset.dcTab));$('#dcStepType').onchange=refreshProviderSelect;$('#dcAddStep').onclick=addStep;$('#dcSaveRecipe').onclick=saveRecipe;$('#dcLoadRecipe').onclick=loadRecipe;$('#dcSavedRecipe').onchange=loadRecipe;$('#dcChartProvider').onchange=renderChartParams;$('#dcRenderChart').onclick=renderChart;$('#dcCopyProvenance').onclick=()=>ctx.io.clipboard.writeText(JSON.stringify(activeArtifact()?.provenance||[],null,2));
