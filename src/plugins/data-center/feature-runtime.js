(() => {
  async function mount(ctx,controller=null,views=null,adapter={}){
    ...
    function renderSourceAction(a=activeArtifact()){const button=$('#dcRemoveSource');if(!button)return;const enabled=!!sourceCapability?.remove&&isImportedSource(a);button.disabled=!enabled;button.title=enabled?'从当前工程移除这组导入源数据及其派生结果':'只有直接导入的源数据可以从这里移除';}
    async function removeActiveSource(){
      const a=activeArtifact();if(!isImportedSource(a)||!sourceCapability?.remove){ctx.status.set('当前选择不是可移除的导入源数据。');return false;}
