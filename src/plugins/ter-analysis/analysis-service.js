(() => {
  ...
  window.DKDSPluginModules.define('builtin.ter-analysis','analysis-service',{
    async create({project:initialProject,bootstrap,setStatus,copyTextToClipboard,saveChartImage,scheduleSnapshot,artifacts,science=null,dataModel=null,io=null,dom=null,performance=null,pipeline=null,transforms=null,algorithms=null,reactive=null}){
      ...
      async function runTerAlgorithm(source,algorithmRef,algorithmSettings){const api=algorithmApi(),row=resolveTerAlgorithm(algorithmRef);if(!row&&api?.diagnose){const d=api.diagnose(algorithmRef,{category:'ter-analysis'});if(d.status==='missing-version')throw new Error(`工程锁定的 TER 算法版本缺失：${d.requested.id}@${d.requested.version}；可用版本：${d.alternatives.map(x=>x.version).join('、')||'无'}`);}if(row&&api?.run){const value=await api.run({id:row.id,version:row.version,category:'ter-analysis'},source,{category:'ter-analysis',parameters:{settings:{...algorithmSettings},executionKey:'ter-workbench'}});return {...value,algorithm:algorithmProvenance(row)};}throw new Error('当前没有可用的 TER Algorithm Provider。');}
      ...
      function sourceArtifacts(){
        return (artifacts?.list?.({kind:'data.table',includeTransient:true})||[]).filter(artifact=>String(artifact?.semanticType||'')==='science.transport.iv');
      }
      ...
      function sourceScanReference(vg,sourceFile='',direction=1){
        const targetVg=Number(vg),targetDirection=Number(direction)<0?-1:1;if(!Number.isFinite(targetVg))return null;
        let candidates=allSweeps().filter(sweep=>Number(sweep?.direction)===targetDirection&&Number.isFinite(Number(sweep?.vg))&&Math.abs(Number(sweep.vg)-targetVg)<=Math.max(1e-10,Math.abs(targetVg)*1e-9));
        const preferred=String(sourceFile||'');if(preferred){const exact=candidates.filter(sweep=>String(sweep?.datasetName||'')===preferred);if(exact.length)candidates=exact;}
        const sweep=candidates[0]||null;if(!sweep?.id)return null;
        const dataset=datasets().find(row=>String(row?.path||'')===String(sweep.datasetPath||''))||datasets().find(row=>String(row?.name||'')===String(sweep.datasetName||''));
      ...
      async function calculate(){
        ...
        try{
          if(pipeline?.run){
            const executed=await pipeline.run('ter-matrix',sourceArtifacts(),{parameters:{settings:{...settings},algorithmRef:{...terAlgorithmRef}},publish:true,revision:`${inputCacheKey()}|ter:${terAlgorithmRef.id}@${terAlgorithmRef.version}`});
            result=executed?.value||null;
          }else result=await runTerAlgorithm(datasets(),terAlgorithmRef,settings);
          if(!result)throw new Error('TER pipeline did not return a result.');
          settings={...settings,vmin:result.used.vmin,vmax:result.used.vmax,vstep:result.used.vstep,tolerance:result.used.tolerance,currentFloor:result.used.currentFloor};
          syncInputs();renderResult();publishDerivedArtifacts();reactive?.touch?.('ter.result',{reason:'calculate'});scheduleSnapshot();setStatus(`TER 热图计算完成：${result.vgs.length} 个 Vg × ${result.targets.length} 个 Vd。`);return result;
        }catch(err){result=null;reactive?.touch?.('ter.result',{reason:'calculate-failed'});if($('#terSummary'))dom.html($('#terSummary'),`<span class="dkds-summary-chip dkds-chip danger">计算失败：${String(err.message||err)}</span>`);setStatus(`TER_max 计算失败：${err.message||err}`);return null;}
      }
      ...
      if(pipeline?.register){
        pipeline.register('ter-matrix',{
          ...
          run:async(input,{parameters})=>{
            ...
            const computed=await runTerAlgorithm(source,algorithmRef,parameters?.settings||settings);
          ...
          selection:({artifacts,value})=>artifacts[0]?[{type:'ter.matrix-result',id:artifacts[0].id,ref:{artifactId:artifacts[0].id},meta:{rows:value?.vgs?.length||0,cols:value?.targets?.length||0}}]:[],
      ...
      const service={
        ...
        getTransformSettings:()=>cloneSerializable(transform),getTransformDefinition:()=>cloneSerializable(transformDefinition()),listTransforms:()=>transforms?.list?.({supportsScalarField:true})||[],setTransformSettings,getTransformMatrix:transformMatrix,sourceScanReference,transformCsv,
