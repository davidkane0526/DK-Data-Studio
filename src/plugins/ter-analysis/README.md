...
The second heatmap uses the exact Vg/Vd target grid and source-file choice already established by TER analysis. Core Scientific Transform Registry supplies the selectable transform catalog and generic `scalar-field.<id>` Pipeline stages. Shared Science Runtime provides the canonical `computeSweepScalarField()` scalar-field projection and reuses the same numerical transform primitives used by resonance analysis. Built-in transforms are raw I–V, detrended I−Ibg, dI/dV, d²I/dV², d ln|I|/dV, dV/dI and R=|V/I|; compatible public transforms registered by plugins can be discovered without adding TER-specific branches. Forward and reverse sweeps are selected explicitly and are never mixed. Missing unmatched Vd samples stay missing.
