(() => {
  DKDSPlugins.define({
    ...
    const runtimeItem=ctx.ui.statusBar.add({
      ...
      order:10,
      icon:'◉',
      label:ctx.host?.isWebClient?'网页版':'桌面端',
      state:'info',
      className:'compact',
      title:'当前运行模式',
      onClick:()=>{
        const runtime=runtimeStatus?.runtime|| (ctx.host?.isWebClient?'web':'desktop');
        const platform=runtimeStatus?.platform||navigator.platform||'';
        ctx.host?.setStatus?.(`运行模式：${runtime} ${platform?`· ${platform}`:''}`);
      }
    });
    ...
    const lanItem=ctx.ui.statusBar.add({
      ...
      order:30,
      icon:'●',
      label:ctx.host?.isWebClient?'网页版 已连接':'网页版 检查中',
      state:ctx.host?.isWebClient?'ok':'info',
      title:'局域网网页版状态；点击打开/恢复面板',
      onClick:()=>{
        if(ctx.host?.isWebClient){
          ctx.host?.setStatus?.(`当前就是局域网网页版：${location.origin}`);
          return;
        }
        void ctx.host?.openLanWebPanel?.();
      }
    });
    ...
    async function refreshRuntime(){
      try{
        const status=await ctx.host?.getRuntimeStatus?.();
    ...
    async function refreshLan(){
      if(ctx.host?.isWebClient){applyLan();return;}
      try{
        const status=await ctx.host?.getLanWebStatus?.();
    ...
    const timer=setInterval(()=>{void refreshRuntime();void refreshLan();},1500);

    return {
      deactivate(){stopped=true;clearInterval(timer);},
