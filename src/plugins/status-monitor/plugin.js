(() => {
  DKDSPlugins.define({"id":"builtin.status-monitor","name":"Status Monitor","version":"1.3.6","apiVersion":"1.19.0","requiresCore":["runtime","events","status","services","workspace","ui.dom","ui.status-bar","ui.theme"],"entry":"plugin.js","enabled":true,"order":7,"description":"Unified bottom status bar for theme selection, runtime memory, DevTools, and LAN web service state.","capabilities":["ui.status-bar","system.runtime-status","lan.web-status","ui.theme"],"pluginType":"foundation","systemCritical":true,"platformPresentation":{"desktop":{"mode":"adaptive"},"mobile":{"mode":"adaptive"}},"scripts":["theme-layout.js","plugin.js"]}, async ctx => {
    ...
    ctx.ui.dom.on(themeList,'click',event=>{
      ...
      try{window.DKDSTheme?.setProfile?.(id);}catch(err){ctx.status.set(`主题切换失败：${err?.message||err}`);}
    });
    ctx.ui.dom.on(themePanel,'click',event=>{
      const button=event.target.closest?.('[data-dkds-theme-mode]');if(!button)return;
      try{window.DKDSTheme?.set?.(button.dataset.dkdsThemeMode);}catch(err){ctx.status.set(`外观切换失败：${err?.message||err}`);}
    });
    ...
    function primaryMemoryBytes(status,memory){
      const m=memory||{};
      if(String(status?.runtime||'')==='desktop'&&Number(m.privateBytes)>0)return Number(m.privateBytes)||0;
      return Number(m.workingSetBytes||m.jsHeapUsedBytes)||0;
    }
    function renderMemoryPanel(){
      const status=runtimeStatus||{},desktop=String(status.runtime||'')==='desktop';
      const total=primaryMemoryBytes(status,status.memory||{});
      const rows=(Array.isArray(status.components)?status.components:[])
        .map(row=>({...row,_primaryBytes:primaryMemoryBytes(status,row)}))
        .filter(row=>row._primaryBytes>0)
        .sort((a,b)=>b._primaryBytes-a._primaryBytes);
      totalLabel.textContent=total>0?`${desktop?'私有总计':'总计'} ${formatBytes(total)}`:'—';
      ...
      componentList.innerHTML=rows.map(row=>{
        const used=row._primaryBytes;
        const ratio=total>0?Math.max(3,Math.min(100,used/total*100)):0;
        const footprint=desktop&&Number(row.workingSetBytes)>0?` · 工作集 ${formatBytes(row.workingSetBytes)}`:'';
        const meta=[String(row.type||''),Number(row.pid)>0?`PID ${row.pid}`:''].filter(Boolean).join(' · ')+footprint;
        const releasable=desktop&&!!row.pluginId&&row.visible===false&&typeof runtimeService.releaseActivityWindow==='function';
        const release=releasable?`<button type="button" class="dkds-memory-release danger-soft" data-dkds-component-variant="destructive" data-release-activity="${esc(row.activityId||'')}" data-release-project="${esc(row.projectTabId||'')}" data-release-pid="${Number(row.pid)||0}" title="关闭这个已隐藏插件进程并释放其缓存">释放</button>`:'';
        return `<div class="dkds-memory-component-row${releasable?' is-releasable':''}" data-plugin-id="${esc(row.pluginId||'')}" ${releasable?'tabindex="0"':''} data-memory-ratio="${ratio.toFixed(1)}">
          <div class="dkds-memory-component-main"><span class="dkds-memory-component-name">${esc(row.label||row.type||'组件')}</span><span class="dkds-memory-component-value">${release}<strong class="dkds-memory-component-bytes">${formatBytes(used)}</strong></span></div>
    ...
    function applyRuntime(status){
      ...
      const used=primaryMemoryBytes(status,m);
      const desktop=String(status.runtime||'')==='desktop';
      memoryItem.update({label:`内存 ${formatBytes(used)}`,state:'',title:desktop?`应用私有内存 ${formatBytes(used)}；工作集包含共享页，不直接跨进程相加。点击查看 ${Number(status.processCount)||0} 个组件 / 进程明细`:`运行时内存近似值 ${formatBytes(used)}；点击查看组件明细`});
    ...
    ctx.ui.dom.on(componentList,'click',async event=>{const button=event.target?.closest?.('[data-release-activity]');if(!button)return;event.preventDefault();event.stopPropagation();button.disabled=true;try{const result=await runtimeService.releaseActivityWindow?.({activityId:String(button.dataset.releaseActivity||''),projectTabId:String(button.dataset.releaseProject||''),pid:Number(button.dataset.releasePid)||0});ctx.status.set(result?.released?`已释放 ${result.released} 个隐藏插件进程。`:'没有找到可释放的隐藏插件进程。');await refreshRuntime();renderMemoryPanel();}catch(err){ctx.status.set(`释放插件内存失败：${err?.message||err}`);button.disabled=false;}});
