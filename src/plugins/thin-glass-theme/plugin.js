(() => {
  const manifest={
    id:'com.dkds.theme.liquid-glass',name:'Thin Glass',version:'1.8.0',apiVersion:'1.18.0',pluginType:'theme',
    requiresCore:['ui.theme'],capabilities:['ui.theme'],compatibility:{app:'>=3.62.0 <4.0.0',pluginApi:'^1.18.0',themeContract:'^3.6.0'}
  };
  DKDSPlugins.define(manifest,async ctx=>{
    const theme=ctx.ui.theme;
    const required=['contract:3.6.0','contract.material.recipes','renderer.recipes.thin-glass','renderer.thinGlass'];
    const missing=required.filter(feature=>!theme.supports(feature));
    if(missing.length)throw new Error(`Thin Glass 1.8.0 requires Theme Contract 3.6 thin-glass support: ${missing.join(', ')}`);
    ...
    const handle=theme.register('liquid-glass',{
      label:'Thin Glass',
      metadata:{family:'thin-glass',contract:'theme-3.6',renderer:'core-thin-glass',opticalModel:'single-layer-low-radius-backdrop'},
      ...
      modes:{
        light:{tokens:{canvas:'#E8EEF5',surface:'#F7F9FC',surfaceSoft:'rgba(235,241,247,.86)',surfaceHover:'#E5EDF7',surfaceElevated:'rgba(252,253,255,.72)',surfaceSidebar:'rgba(226,235,245,.78)',controlBg:'#F7FAFD',controlHover:'#EBF2F9',divider:'rgba(104,121,144,.14)',dividerHover:'rgba(73,94,122,.28)',controlBorder:'rgba(104,121,144,.22)',controlBorderHover:'rgba(73,94,122,.36)',scrollbar:'rgba(113,128,150,.30)',scrollbarHover:'rgba(82,101,128,.46)',text:'#172033',textSoft:'#33445B',muted:'#586A82',accent:'#2563EB',accentHover:'#1D4ED8',accentSoft:'rgba(37,99,235,.10)',focus:'rgba(37,99,235,.15)',shadow1:'0 2px 6px rgba(69,84,110,.07)',shadow2:'0 5px 14px rgba(69,84,110,.09)',shadowFloat:'0 8px 20px rgba(69,84,110,.12)',radius:10,radiusLg:13},material:{specularHighlight:'rgba(255,255,255,.20)',innerHighlight:'rgba(255,255,255,.34)',glassEdge:'rgba(183,196,214,.50)',roles:{popover:{glassEdge:'rgba(171,187,209,.62)'},floating:{glassEdge:'rgba(183,196,214,.54)'}}}},
        dark:{tokens:{canvas:'#0A1020',surface:'#111827',surfaceSoft:'rgba(20,29,45,.78)',surfaceHover:'#1C293B',surfaceElevated:'rgba(27,38,58,.56)',surfaceSidebar:'rgba(13,22,39,.62)',controlBg:'#10192B',controlHover:'#1D2A3D',divider:'rgba(100,116,139,.15)',dividerHover:'rgba(125,142,166,.30)',controlBorder:'rgba(100,116,139,.24)',controlBorderHover:'rgba(125,142,166,.40)',scrollbar:'rgba(71,85,105,.38)',scrollbarHover:'rgba(100,116,139,.56)',text:'#E6ECF4',textSoft:'#CBD5E1',muted:'#96A5B9',accent:'#3B74FF',accentHover:'#72AAFF',accentSoft:'rgba(59,116,255,.15)',focus:'rgba(114,170,255,.18)',shadow1:'0 2px 7px rgba(0,0,0,.18)',shadow2:'0 5px 15px rgba(0,0,0,.22)',shadowFloat:'0 8px 22px rgba(0,0,0,.27)',radius:10,radiusLg:13},material:{specularHighlight:'rgba(255,255,255,.10)',innerHighlight:'rgba(255,255,255,.14)',glassEdge:'rgba(100,116,139,.42)',roles:{popover:{glassEdge:'rgba(116,132,157,.52)'},floating:{glassEdge:'rgba(100,116,139,.46)'}}}}
      }
    });
