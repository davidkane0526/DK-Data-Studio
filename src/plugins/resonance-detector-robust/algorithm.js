(function(root,factory){
  const core=root.DKDSScience||{},api=Object.freeze(factory(core));root.DKDSResonanceAlgorithms=api;const modules=root.DKDSPluginModules;if(modules?.define&&!modules.get?.('builtin.resonance-detector-robust','algorithm'))modules.define('builtin.resonance-detector-robust','algorithm',api);
  ...
  function buildSweeps(dataset){
    ...
    function range(r){let lo=Infinity,hi=-Infinity;for(const p of r.points){const v=Number(p.v);if(!Number.isFinite(v))continue;if(v<lo)lo=v;if(v>hi)hi=v;}return [lo,hi];}
    const groups=[];
    for(const r of runs){
      const [r0,r1]=range(r);
      let target=null;
