(() => {
  DKDSPlugins.define({"id":"builtin.resonance-detector-robust","name":"Standard Resonance Algorithms","version":"2.3.0","apiVersion":"1.19.0","requiresCore":["science","analysis.algorithms","modules","execution.tasks"],"entry":"plugin.js","scripts":["algorithm.js","plugin.js"],"enabled":true,"order":80,"description":"Versioned built-in resonance peak detection and FWHM/peak-metrics algorithm providers. Implementations are plugin-local and replaceable.","capabilities":["analysis.algorithm","analysis.peak-detector","analysis.peak-metrics"],"algorithmProvider":true,"algorithmCategories":["peak-detector","peak-metrics"],"algorithmProvides":[{"category":"peak-detector","id":"robust-ricker-v1","version":"1.0.0","title":"稳健多通道 / 多尺度"},{"category":"peak-metrics","id":"baseline-fwhm-v1","version":"1.0.0","title":"局部基线 FWHM"}],"pluginType":"algorithm","pluginDependencies":[{"id":"builtin.scientific-data-contracts"}],"tasks":[{"id":"resonance-compute","entry":"resonance-task.js","imports":["task-core.js","algorithm.js"]}]}, async ctx => {
    const S=ctx.science;
    const A=ctx.modules.require('algorithm');
    const runTask=(op,input,key,options={})=>ctx.tasks.submit('resonance-compute',{op,...input},{key,latest:options.latest!==false}).promise;
    const metricQueue=[];
    let metricFlushQueued=false,metricBatchSequence=0;
    const queueFlush=fn=>Promise.resolve().then(fn);
    const flushMetricQueue=()=>{
      metricFlushQueued=false;
      if(!metricQueue.length)return;
      const rows=metricQueue.splice(0,128),batchId=++metricBatchSequence;
      runTask('metrics-batch',{items:rows.map(row=>row.input)},`metrics-batch:${batchId}`,{latest:false}).then(values=>{
        const out=Array.isArray(values)?values:[];
        rows.forEach((row,index)=>row.resolve(out[index]??null));
      },error=>rows.forEach(row=>row.reject(error))).finally(()=>{
        if(metricQueue.length&&!metricFlushQueued){metricFlushQueued=true;queueFlush(flushMetricQueue);}
      });
    };
    const runMetricTask=input=>new Promise((resolve,reject)=>{
      metricQueue.push({input,resolve,reject});
      if(!metricFlushQueued){metricFlushQueued=true;queueFlush(flushMetricQueue);}
    });
    ...
    ctx.analysis.algorithms.register('robust-ricker-v1',{
      ...
      run:(sweep,{parameters={},range=null}={})=>runTask('detect',{sweep,settings:parameters,options:{range}},`detect:${String(sweep?.id||sweep?.datasetPath||'main')}`)
    });
    ctx.analysis.algorithms.register('baseline-fwhm-v1',{
      ...
      run:(input)=>runMetricTask({peak:input?.peak,sweep:input?.sweep})
    });
    return {};
  });
})();
