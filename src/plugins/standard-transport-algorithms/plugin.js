(() => {
  const manifest={
    id:'builtin.standard-transport-algorithms',pluginType:'algorithm',name:'Standard Transport Algorithms',version:'1.1.0',apiVersion:'1.18.0',
    requiresCore:['analysis.algorithms','data.transforms','modules'],entry:'plugin.js',enabled:true,order:70,
  ...
  window.DKDSPlugins.define(manifest,ctx=>{
    ...
    const transforms=[
      ['raw','原始 I–V','science.iv.raw','science.transport.current-field','current','A',true],['detrend','去背景 I−Ibg','science.iv.background-removed','science.transport.background-removed-current-field','current','A',true],['didv','dI/dV（微分电导）','science.transport.didv','science.transport.conductance-field','conductance','A/V',true],['d2idv2','d²I/dV²','science.transport.d2idv2','science.transport.second-derivative-current-field','second-derivative-current','A/V²',true],['dlog','d ln|I|/dV','science.transport.dlnabsidv','science.transport.log-current-slope-field','log-current-slope','1/V',true],['dvdi','dV/dI（微分电阻）','science.transport.dvdi','science.transport.differential-resistance-field','differential-resistance','V/A',true],['resistance','R = |V/I|','science.transport.resistance','science.transport.resistance-field','resistance','Ω',false]
    ];
    const fieldAlgorithmRef={category:'transport-scalar-field',id:'transport.scalar-field',version:'1.0.0'};
    for(const [id,title,outputType,fieldType,quantity,unit,diverging] of transforms){ctx.data.transforms.register(id,{title,outputType,fieldType,quantity,unit,diverging,tags:['transport','transform'],transformKey:id,public:true,supportsScalarField:true,algorithmRef:{category:'transport-transform',id:`transport.${id}`,version:'1.0.0'},fieldAlgorithmRef});}
