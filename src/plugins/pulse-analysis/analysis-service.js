(() => {
  ...
  function modeName(mode) {
    return ({cycle:'按周期点数',timing:'按时间协议',waveform:'按记录电压','equal-count':'等点数分段',auto:'自动'})[mode] || '等点数分段';
  }
  ...
  window.DKDSPluginModules.define('builtin.pulse-analysis','analysis-service',{
    async create({setStatus,copyTextToClipboard,saveChartImage,scheduleSnapshot,science=null,io=null,charts=null,dom=null,artifacts=null,detachSource=null,tasks=null}) {
      const A=science;
      ...
      async function analyzeItem(item) {
        ...
        if (!item?.text || !item.inspection) {item.error='文件内容不可用。';if(!previousResult)item.result=null;return false;}
        if(!tasks?.submit)throw new Error('Pulse analysis requires the Core Task Runner.');
        const file={name:item.name,path:item.path,text:item.text,encoding:item.encoding},options=analysisOptions(item);
        try{
          const handle=tasks.submit('analyze-pulse-read',{file,options,inspection:item.inspection},{key:`pulse:${item.id}`,latest:true});
          const nextResult=await handle.promise;
          if(!nextResult||!Array.isArray(nextResult.points))throw new Error('脉冲分析没有返回有效结果。');
          item.result=nextResult;item.error='';item.analyzedAt=new Date().toISOString();item.lastResolvedAnalysis={segmentationMode:nextResult.segmentationMode||options.segmentationMode,cycleSamples:Number(nextResult.cycleSamples||options.cycleSamples)||0};return true;
        }catch(err){item.result=previousResult;item.analyzedAt=previousAnalyzedAt;if(err?.name!=='AbortError')item.error=err?.message||String(err);return false;}
      }
      ...
      function renderEditor() {
        ...
        // The removed #pulseAnalyzeCurrentBtn was removed when header actions
        ...
        // selection safely; only update an optional control when it exists.
      ...
      async function analyzeCurrent() {
        ...
        item.loading=true;renderFileList();const ok = await analyzeItem(item);item.loading=false;
      ...
      async function analyzeChecked() {
        ...
        for (let i=0;i<items.length;i++) {
          ...
          if (await analyzeItem(item)) ok++; else fail++;
          item.loading=false;
        }
        render(); scheduleSnapshot();
      ...
      function restore(saved) {
        const next=createState(),reanalyzeIds=new Set();
        if (saved && Array.isArray(saved.files)) {
          ...
          for (const source of saved.files) {
            try {
              const artifact=source.artifactId&&artifacts?.get?.(String(source.artifactId));
              if(!artifact)continue;
              const item=makeItemFromArtifact(artifact,source);
              if(source.analyzed && !item.result)reanalyzeIds.add(String(item.id));
        ...
        refreshSources();
        for(const item of state.files){
          if(!reanalyzeIds.has(String(item.id))||item.result)continue;
          item.loading=true;
          void analyzeItem(item).then(()=>{item.loading=false;render();scheduleSnapshot();});
        }
        if ($('#pulseFileList')) render();
      }
