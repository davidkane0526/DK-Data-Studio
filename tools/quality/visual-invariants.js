...
function validate(){
  ...
  forbidText(contextualComponentRuntime,"addEventListener?.('dkds:theme-changed'",'HARD-62: Component Appearance must not subscribe to broad Theme repaint events after synchronous Theme composition.');
  requireText(themeRuntime,"visualSynchronized:true,reason:'setting'",'HARD-62: active Theme setting changes must finish DOM composition before broadcasting semantic Theme state.');
  ...
  requireText(read('src/core/ui/modules/layout/workspace.js'),'if(Number.isFinite(raw))this.previewSize=raw','HARD-62: canonical split preview must coalesce raw pointer geometry.');
  requireText(read('src/core/ui/modules/layout/workspace.js'),'this.previewViewport=this.viewport(this.drag?.rect)','HARD-62: SplitController must cache drag-session geometry so clamping avoids raw-event layout reads.');
  requireText(read('src/core/ui/modules/layout/state-resolver.js'),'if(nativeMobile&&state.mobileOverlay)','HARD-62: Native Mobile split ceilings must remain inside the pure Core layout resolver.');
