...
for(const file of allPluginFiles){
  ...
  for(const [pattern,label] of forbidden)if(pattern.test(src))fail(`${rel}: ${label} must go through Core API v1.16.`);
...
if(!kernel.includes("const API_VERSION = '1.16.0'"))fail('Plugin API must be 1.16.0.');
if(process.exitCode)process.exit(process.exitCode);
console.log('Plugin boundary check OK: all first-party plugin infrastructure is routed through Core API v1.16.');
