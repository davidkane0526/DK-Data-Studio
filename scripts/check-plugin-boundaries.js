...
for(const file of allPluginFiles){
  ...
  for(const [pattern,label] of forbidden)if(pattern.test(src))fail(`${rel}: ${label} must go through Core API v1.17.`);
...
console.log('Plugin boundary check OK: all first-party plugin infrastructure is routed through Core API v1.17.');
