...
assert.equal(json('package.json').version,'3.61.35');
const contract=json('sdk/contract.json');
assert.equal(contract.sdkVersion,'1.17.2');
assert.equal(contract.pluginApiVersion,'1.17.0');
assert.equal(contract.minimumAppVersion,'3.61.35');


const app=read('src/app.js'),index=read('src/index.html'),pluginWindow=read('src/plugin-window/runtime.js');
assert(app.includes("version:'3.61.35'")&&app.includes("appVersion:'3.61.35'"),'Main renderer version metadata must match the release version.');
assert(index.includes('<span class="version">v3.61.35</span>'),'Visible shell version must match the release version.');
assert(pluginWindow.includes("appVersion:'3.61.35'"),'Dedicated plugin windows must report the release version.');

const chart=read('src/core/chart-runtime.js');
for(const token of [
  "const VERSION='1.8.1'",'function legendEntriesFor','function traceLegendGroup',
  'legendgroup:','function applyPlotLegendFocus','P.restyle','function ensurePlotPresentationHost',
  'function splitLegendRows','stickyMulti','stable-active-layout-solver','function renderPlotLegend'
...
assert(!chart.includes("displayModeBar:'hover'"),'Native Plotly modebar must stay suppressed.');

const css=read('src/style.css');
for(const token of ['.dkds-plot-legend-row','overflow-x:auto!important','.dkds-plotly-surface-host'])assert(css.includes(token),`Plot CSS closure missing ${token}`);
assert(!css.includes('.dkds-plotly-auto-legend{overflow:hidden!important}'),'Core legend must not clip crowded horizontal entries.');
...
console.log('v3.61.35 visual contract closure OK');
