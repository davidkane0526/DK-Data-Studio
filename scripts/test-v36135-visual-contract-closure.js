...
assert.equal(json('package.json').version,'3.61.53');
...
assert(app.includes("version:'3.61.53'")&&app.includes("appVersion:'3.61.53'"),'Main renderer version metadata must match the release version.');
assert(index.includes('<span class="version">v3.61.53</span>'),'Visible shell version must match the release version.');
assert(pluginWindow.includes("appVersion:'3.61.53'"),'Dedicated plugin windows must report the release version.');
