...
assert.equal(json('package.json').version,'3.61.54');
...
assert(app.includes("version:'3.61.54'")&&app.includes("appVersion:'3.61.54'"),'Main renderer version metadata must match the release version.');
assert(index.includes('<span class="version">v3.61.54</span>'),'Visible shell version must match the release version.');
assert(pluginWindow.includes("appVersion:'3.61.54'"),'Dedicated plugin windows must report the release version.');
