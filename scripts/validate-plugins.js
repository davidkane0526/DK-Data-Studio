...
for (const name of fs.readdirSync(pluginsDir).sort()) {
  ...
  if (!['1.9.0','1.10.0','1.11.0','1.12.0','1.13.0','1.14.0','1.15.0'].includes(String(m.apiVersion||''))) fail(`${name}: built-in plugins must target apiVersion 1.9.0, 1.10.0, 1.11.0, 1.12.0, 1.13.0, 1.14.0 or 1.15.0`);
