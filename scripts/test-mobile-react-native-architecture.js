...
assert(shell.includes('ProjectDrawer') && shell.includes('PanResponder.create') && shell.includes('projectDeleteAction') && shell.includes('translateX: slide') && shell.includes('gesture.dx < 0') && shell.includes('toValue: -360'), 'project management must open from the left and use iOS-style left-swipe reveal/delete animation');
...
assert(infrastructure.includes('inScrollbarGutter') && infrastructure.includes("header.style.touchAction='none'") && mobileHost.includes(".dkds-portable-header,.drag-handle"), 'held PRIME resizing must own the title gesture and ignore scrollbar/global held-swipe arbitration');
