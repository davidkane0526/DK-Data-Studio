...
const chartScope={
  ...
  bind(t,name,handler){t.handlers.set(name,handler);return()=>{if(t.handlers.get(name)===handler)t.handlers.delete(name);};},
...
(async()=>{
  ...
  const clickHandler=target.handlers.get('dkds_chart_click');
  target.domHandlers.get('dkds:chart-legend-activate')({detail:{curveNumber:1}});
  ...
  target.handlers.get('dkds_chart_selected')({points:[{curveNumber:0,pointNumber:0},{curveNumber:1,pointNumber:0}]});
  assert.strictEqual(selectedMany.length,2,'area selection must publish mapped entities when enabled');
  target.handlers.get('dkds_chart_relayout')({'xaxis.range[0]':-.5,'xaxis.range[1]':1.5,'yaxis.range[0]':0,'yaxis.range[1]':4});
  assert.deepStrictEqual(view.controllers.viewport.get().xRange,[-.5,1.5]);
  await view.controllers.viewport.set({xRange:[0,1]},{source:'test'});assert(calls.relayout.some(row=>Array.isArray(row['xaxis.range'])),'viewport.set must delegate to the Core chart relayout contract');
  ...
  assert.strictEqual(target.handlers.get('dkds_chart_click'),clickHandler,'react must not duplicate or replace shared event handlers');
  ...
  const reactBeforeSuspend=calls.react;await view.suspend({purgeManaged:true,reason:'test'});assert(view.lifecycleState().suspended&&view.lifecycleState().purged,'managed plot must enter purged suspended state');assert.strictEqual(target.data.length,0,'suspend must release renderer data');assert.strictEqual(target.handlers.size,0,'suspend must release renderer event bindings');
  ...
  scope.dispose();
  assert.strictEqual(target.handlers.size,0,'dispose must remove every shared renderer binding');
