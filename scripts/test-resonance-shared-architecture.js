...
assert(views.includes('wb.compose')&&views.includes('existingNode:inspector')&&views.includes('existingNode:group')&&views.includes("stateVersion:'canvas-v1'"),'GRS-parity PRIME surfaces must be hosted and placement-persisted by the shared PluginWorkspace/PortableView system.');
...
assert(views.includes('leftNode:leftPanel,mainNode:mainArea')&&views.includes("defaultPlacement:'bottom'"),'GRS 1/5 control rail and 4/5 scientific canvas must be expressed through PluginWorkspace composition; group PRIME defaults below the chart canvas.');
...
assert(views.includes("id:'undo'")&&views.includes("id:'deselect'")&&views.includes('resparRangeApplyIdentity'),'GRS parity semantic command model must expose undo, cancel-selection, and range identity controls without duplicating header buttons.');
...
assert(feature.includes("workspaceRuntime.portable(`resonance-group:${metric}`")&&feature.includes("placements:['home','left','right','bottom','float']"),'Every group subplot must be independently portable inside PluginWorkspace.');
