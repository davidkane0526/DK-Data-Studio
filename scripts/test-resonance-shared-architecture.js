...
assert(ui.includes('d3.drag().clickDistance(7)')&&ui.includes(".on('contextmenu'")&&ui.includes('getManipulators')&&ui.includes('emitManipulation'),'Core ScientificCurveSurface must own domain-neutral direct manipulation and modifier+right-click marker deletion.');
...
assert(ui.includes('dkds-direct-range-handle')&&ui.includes("kind==='range'")&&feature.includes("action:'analysis-window'")&&feature.includes('onManipulationCommit')&&!feature.includes('onWidthWindowCommit'),'Core must own generic range manipulators while Resonance maps the resulting geometry to its analysis-window/FWHM domain state.');
