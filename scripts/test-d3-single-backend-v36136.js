...
assert.equal(pkg.version,'3.61.36');
...
assert(index.includes('core/d3-chart-renderer.js')&&index.indexOf('core/d3-chart-renderer.js')<index.indexOf('core/chart-runtime.js'),'D3 adapter must load before the chart facade.');
...
assert(dedicated.includes("requestedScientificRenderer=requestedIds.includes('scientific-renderer')"),'Dedicated TOP must declare scientific rendering vendor-neutrally.');
...
const context={console,structuredClone,setTimeout,clearTimeout,queueMicrotask,performance:{now:()=>1},location:{href:'file:///src/index.html'},URL,
 ...
 document:{currentScript:{src:'file:///src/core/chart-runtime.js'},documentElement:{dataset:{dkdsTheme:'light'}},getElementById:id=>id==='d3-only'?target:null,querySelector:()=>null,querySelectorAll:()=>[]},
...
(async()=>{
  ...
  console.log('v3.61.36 D3 single-backend runtime PASS');
