...
assert.equal(json('package.json').version,'3.61.57','v3.61.57 shell polish test tracks the release source version.');
...
const windowManager=require(path.join(root,'plugin-window-manager.js'));
...
assert(css.includes('v3.61.55 · control unification / status cluster'),'release must carry the modern-control normalization layer.');
assert(css.includes('body.dkds-modern-ui .dkds-analysis-nav-btn')&&css.includes('border:0!important;'),'AnalysisWorkbench navigation may not retain the legacy bright outlined-button treatment.');
...
assert(css.includes('.statusbar-command-cluster')&&css.includes('body.dkds-modern-ui .plugin-status-item::before{display:none!important}'),'status actions must be one shared command cluster without stitched vertical separators.');
