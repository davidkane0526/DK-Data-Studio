...
assert(contract.pluginApiVersion==='1.9.0','SDK must target Plugin API 1.9.0.');
...
assert(read('sdk/plugin-api.d.ts').includes("apiVersion:'1.9.0'")&&read('sdk/plugin-api.d.ts').includes('DKDSPluginContext'),'SDK must ship editor-readable Plugin API declarations.');
...
for(const name of ['workspace-plugin','algorithm-provider']){
  ...
  assert(pkg.manifest.apiVersion==='1.9.0'&&pkg.manifest.source==='external',`${name} SDK package must be installable by the application normalizer.`);
}
...
assert(packages.includes('node sdk/tools/dkds-plugin.js validate')&&packages.includes('"apiVersion": "1.9.0"'),'Plugin package guide must document the standalone v1.9 SDK workflow.');
console.log('v3.59 standalone Plugin SDK + host-independent first-party plugin contract checks passed.');
