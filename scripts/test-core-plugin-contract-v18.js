...
assert(contract&&contract.API_VERSION==='1.10.0','Core contract must target Plugin API 1.10.0.');
...
assert(contract.validateManifest({apiVersion:'1.9.0',requiresCore:['io']}).ok,'Plugin API 1.10 host must retain 1.9 manifest compatibility.');
...
for(const dir of fs.readdirSync(path.join(root,'src/plugins'))){
  ...
  assert(['1.9.0','1.10.0'].includes(manifest.apiVersion),`${dir}: built-in manifest must use compatible API 1.9/1.10.`);
...
console.log('Core Plugin Contract v1.10 checks passed.');
