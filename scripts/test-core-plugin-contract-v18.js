...
assert(contract&&contract.API_VERSION==='1.13.0','Core contract must target Plugin API 1.13.0.');
...
assert(contract.validateManifest({apiVersion:'1.13.0',requiresCore:['io','charts','data.reactive','ui.scientific-plot']}).ok,'Current Plugin API 1.13 manifests must validate against the public Core contract.');
...
assert(contract.validateManifest({apiVersion:'1.9.0',requiresCore:['io']}).ok,'Plugin API 1.13 host must retain 1.9 manifest compatibility.');
...
for(const dir of fs.readdirSync(path.join(root,'src/plugins'))){
  ...
  assert(['1.9.0','1.10.0','1.11.0','1.12.0','1.13.0'].includes(manifest.apiVersion),`${dir}: built-in manifest must use compatible API 1.9/1.10/1.11/1.12/1.13.`);
...
console.log('Core Plugin Contract v1.13 checks passed.');
