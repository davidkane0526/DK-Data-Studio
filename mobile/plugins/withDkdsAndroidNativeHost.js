...
class DkdsNativeHostModule(private val context: ReactApplicationContext) : ReactContextBaseJavaModule(context), ActivityEventListener {
  ...
  private data class DataReadSession(val token:String,val uri:Uri,val size:Long,var stream:InputStream,var offset:Long=0L,@Volatile var touchedAt:Long=System.currentTimeMillis())
  private val dataReadSessions = ConcurrentHashMap<String, DataReadSession>()
  private val dataReadTtlMs = 5L * 60L * 1000L

  private fun documentSize(uri:Uri, hint:Long):Long {
    if(hint>0)return hint
    try { context.contentResolver.openAssetFileDescriptor(uri,"r")?.use { afd -> if(afd.length>=0)return afd.length } } catch (_:Throwable) {}
    try { context.contentResolver.query(uri,arrayOf(OpenableColumns.SIZE),null,null,null)?.use { c -> if(c.moveToFirst()){ val i=c.getColumnIndex(OpenableColumns.SIZE); if(i>=0&&!c.isNull(i))return c.getLong(i) } } } catch (_:Throwable) {}
    return 0L
  }
  private fun openReadStream(uri:Uri):InputStream = context.contentResolver.openInputStream(uri) ?: throw IllegalStateException("无法读取所选文档。")
  private fun sweepDataReadSessions(){ val now=System.currentTimeMillis(); for((token,row) in dataReadSessions) if(now-row.touchedAt>dataReadTtlMs){ if(dataReadSessions.remove(token,row))try{row.stream.close()}catch(_:Throwable){} } }
  ...
  @ReactMethod fun openDocumentRead(uriValue: String, sizeHintValue: Double, promise: Promise) {
    thread(name="dkds-open-document-read",isDaemon=true) {
      try {
        sweepDataReadSessions(); val uri=Uri.parse(uriValue); val token=UUID.randomUUID().toString(); val size=documentSize(uri,sizeHintValue.toLong().coerceAtLeast(0L)); val row=DataReadSession(token,uri,size,openReadStream(uri)); dataReadSessions[token]=row
        promise.resolve(WritableNativeMap().apply { putString("token",token); putDouble("size",size.toDouble()); putInt("maxChunkBytes",512*1024) })
      } catch(error:Throwable){ promise.reject("E_OPEN_DOCUMENT_READ",error) }
    }
  }

  @ReactMethod fun readDocumentReadChunk(tokenValue: String, offsetValue: Double, lengthValue: Double, promise: Promise) {
    thread(name="dkds-read-document-session",isDaemon=true) {
      try {
        val token=tokenValue.trim(); val row=dataReadSessions[token] ?: throw IllegalStateException("Data read token is invalid or expired."); val offset=offsetValue.toLong().coerceAtLeast(0L); val length=lengthValue.toInt().coerceIn(1,512*1024)
        val bytes=synchronized(row){
          row.touchedAt=System.currentTimeMillis()
          if(offset<row.offset){ try{row.stream.close()}catch(_:Throwable){}; row.stream=openReadStream(row.uri); row.offset=0L }
          var remaining=offset-row.offset
          while(remaining>0){ val skipped=row.stream.skip(remaining); if(skipped>0){row.offset+=skipped;remaining-=skipped}else{val one=row.stream.read();if(one<0){remaining=0;break};row.offset+=1;remaining-=1} }
          if(row.offset<offset)ByteArray(0) else { val buffer=ByteArray(length);var total=0;while(total<length){val count=row.stream.read(buffer,total,length-total);if(count<0)break;total+=count};row.offset+=total;buffer.copyOf(total) }
        }
        val nextOffset=offset+bytes.size; val eof=(row.size>0&&nextOffset>=row.size)||bytes.size<length
        promise.resolve(WritableNativeMap().apply { putString("base64",Base64.encodeToString(bytes,Base64.NO_WRAP));putInt("bytesRead",bytes.size);putDouble("nextOffset",nextOffset.toDouble());putDouble("size",row.size.toDouble());putBoolean("eof",eof) })
      } catch(error:Throwable){ promise.reject("E_READ_DOCUMENT_SESSION",error) }
    }
  }

  @ReactMethod fun closeDocumentRead(tokenValue: String, promise: Promise) {
    val row=dataReadSessions.remove(tokenValue.trim()); try{row?.stream?.close()}catch(_:Throwable){}; promise.resolve(row!=null)
  }
