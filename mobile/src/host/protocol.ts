...
export type DkdsNativeHostApi = {
  ...
  openDocumentRead?: (uri: string, sizeHint: number) => Promise<{ token:string; size:number; maxChunkBytes:number }>;
  readDocumentReadChunk?: (token: string, offset: number, length: number) => Promise<{ base64:string; bytesRead:number; nextOffset:number; eof:boolean; size:number }>;
  closeDocumentRead?: (token: string) => Promise<boolean>;
