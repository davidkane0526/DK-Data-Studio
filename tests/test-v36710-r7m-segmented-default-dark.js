...
assert(index.includes('class="toolbar-group file-command-group dkds-segmented-command-group"')&&index.includes('class="toolbar-group system-core-tools-group dkds-segmented-command-group" role="group"'),'Both persistent shell command clusters must share the exact toolbar-group + segmented structural base.');
assert(shell.includes('.dkds-segmented-command-group{gap:0;padding:1px;height:var(--dkds-shell-group-height);min-height:var(--dkds-shell-group-height);box-sizing:border-box;overflow:hidden;}'),'The segmented parent must clip child fills to one outer silhouette.');
assert(component.includes('.dkds-segmented-command-group[data-dkds-component-identity="toolbarGroup"]')&&component.includes('box-shadow:none;')&&component.includes('border-color:transparent;border-radius:0;box-shadow:none;'),'Segmented shell groups must own the only edge/radius/depth; Import and menu-wrapped children may not keep persistent halos or private rounded cards.');
assert(navigation.includes('[data-dkds-presentation-compact="true"]{\n  width:48px;min-width:48px;max-width:48px;padding:0;line-height:1;'),'Compact presentation commands must center short labels in the full 48 px hit box rather than through asymmetric font padding.');
assert(runtime.includes("surfaceHover:'#303844',surfaceActive:'#333c49',surfaceSelected:'#333c49'")&&runtime.includes("contexts:{grouped:{shadow:'none',shadowHover:'none',shadowActive:'none',shadowSelected:'none'")&&!runtime.includes("surfaceActive:'#202d55'"),'Default dark active/selected commands must use neutral graphite depth, not Thin-Glass-like blue glow.');
...
console.log('v3.67.10 R7M segmented shell + centered compact commands + neutral Default dark contract PASS.');
