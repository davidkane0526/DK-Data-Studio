...
assert(views.includes('leftNode:leftPanel,mainNode:mainArea')&&views.includes('const groupDefault=')&&views.includes(":'bottom'"),'GRS-derived control rail and scientific canvas must use PluginWorkspace without a fixed width ratio; group PRIME keeps bottom as its fallback default while settings may override it.');
