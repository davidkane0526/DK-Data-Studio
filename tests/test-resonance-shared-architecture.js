...
const featureLayers=[feature,featureContext,groupFeature,analysisFeature,peakFeature,selectionFeature,inspectorFeature,mainPlotFeature,controlsFeature].join('\n');
...
assert((manifest.scripts||[]).join(',')==='workbench-shared.js,view-components.js,feature-context.js,feature-group-runtime.js,feature-analysis-runtime.js,feature-peak-runtime.js,feature-selection-runtime.js,feature-inspector-runtime.js,feature-main-plot-runtime.js,feature-controls-runtime.js,feature-runtime.js,super-layout.js,plugin.js','Resonance main renderer must load Controller, shared Views, feature runtime, SUPER adapter, then thin entry.');
assert((manifest.window?.scripts||[]).join(',')==='workbench-shared.js,view-components.js,feature-context.js,feature-group-runtime.js,feature-analysis-runtime.js,feature-peak-runtime.js,feature-selection-runtime.js,feature-inspector-runtime.js,feature-main-plot-runtime.js,feature-controls-runtime.js,feature-runtime.js','Resonance TOP must load the same Controller/View/feature layers; runtime is only a host adapter.');
...
assert(views.includes('wb.compose')&&views.includes('existingNode:inspector')&&views.includes('existingNode:group')&&views.includes("stateVersion:'workspace-v5'"),'GRS-parity PRIME surfaces must be hosted and placement-persisted by the shared PluginWorkspace/PortableView system.');
assert(views.includes('const workspaceFactory=ctx.ui.workspaceSurface')&&views.includes("hostMode:isTop?'top':'super'"),'Resonance must mount the same canonical workspaceSurface for SUPER and TOP; host mode may not select a different internal view.');
...
assert(!views.includes('leftNode:leftPanel')&&views.includes("presentationSurface('prime','data-control')")&&views.includes('existingNode:leftPanel')&&views.includes('const groupDefault=')&&views.includes(":'bottom'"),'GRS-derived control rail must be a semantic data-control PRIME rather than hidden inside PRIMARY; group PRIME keeps bottom as its fallback default while settings may override it.');
...
assert(groupFeature.includes('live.uiRuntime?.plotViews?.bind?.(`resonance-group:${key}`')&&groupFeature.includes("placements:['home','left','right','bottom','float','global']"),'Every group subplot must consume Core PlotView, remain independently portable, and support whole-interface free float.');
