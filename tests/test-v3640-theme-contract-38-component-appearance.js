...
assert(Theme.supports('contract:3.6.0')&&Theme.supports('contract:3.7.0')&&Theme.supports('contract:3.8.0'),'Theme 3.8 must preserve additive 3.x contract capability IDs from the supported baseline.');
assert(!Theme.supports('contract:3.9.0')&&!Theme.supports('contract:2.9.0'),'Theme runtime must not advertise future or different-major contract capabilities.');
...
for(const token of ["version:'3.0.0'",'appearanceCoverage','authoredUnused','appearanceOk','consumption']) assert(coverage.includes(token),`Theme Coverage missing ${token}`);
const debug=read('src/core/theme/debug-runtime.js');
for(const token of ['appearanceComponent','appearanceResolved','appearanceStatus','source']) assert(debug.includes(token),`Theme Debug missing ${token}`);
const gallery=read('src/core/theme/test-gallery.js');
for(const token of ["VERSION='3.0.0'",'Theme 3.8 Component Gallery','Toolbar actions','Panel / Inspector headers','Menu / Context menu','ScientificPlot / Legend','Tooltip / Popover','data-gallery-meta']) assert(gallery.includes(token),`Theme Gallery missing ${token}`);
...
for(const token of ["contractVersion:'3.8.0'",'DKDSThemeAppearanceComponent','DKDSThemeComponentAppearanceValues','appearanceComponents()','consumption():DKDSThemeConsumptionReport',"mode:'fallback-only'"]) assert(dts.includes(token),`SDK type missing ${token}`);
