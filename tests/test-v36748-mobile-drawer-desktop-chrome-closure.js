...
// activating controls behind the drawer.
{
  ...
  const activeDrawer={
    ...
    getAttribute(name){return name==='data-dkds-workspace-surface-id'?'parameters':'';}
  };
  global.window={DKDSPlugins:{activities:{active:()=> 'resonance'}}};
  global.document={
    addEventListener(type,fn){listeners[type]=fn;},
    querySelector(selector){return selector.includes('dkds-mobile-frame-region="drawer"')?activeDrawer:null;}
  };
