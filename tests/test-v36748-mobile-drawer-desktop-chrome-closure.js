...
assert(/\.project-tab-close\{[\s\S]*?--dkds-project-tab-close-size:20px;[\s\S]*?flex:0 0 var\(--dkds-project-tab-close-size\);[\s\S]*?width:var\(--dkds-project-tab-close-size\);[\s\S]*?height:var\(--dkds-project-tab-close-size\);[\s\S]*?box-sizing:border-box;/.test(shellNav),'Project tab close must retain one Structure-owned compact size slot.');
assert(shellNav.includes('html[data-dkds-host="desktop"] .project-tab-close{')&&shellNav.includes('--dkds-project-tab-close-size:16px;'),'Desktop project-tab close must consume the 20% reduced 16 px slot without re-owning rendered geometry.');
