...
sandbox.DKDSPerformance={
  memoWeak(_namespace,_target,_key,compute){sandbox.performanceRuntimeCalls+=1;return compute();},
  memo(_namespace,_key,compute){sandbox.performanceRuntimeCalls+=1;return compute();},
  stage(_namespace,_revision,_key,compute){sandbox.performanceRuntimeCalls+=1;return compute();},
  measure(_namespace,compute){sandbox.performanceRuntimeCalls+=1;return compute();},
  trimPrefix(){return null;}
};
sandbox.performanceRuntimeCalls=0;
...
let value=7;
let activations=0;
let performanceComputes=0;
const tab={pluginState:{}};

P.define({
  id:'test.stateful',pluginType:'extension',name:'Stateful',version:'1.0.0',enabled:true,apiVersion:'1.19.0'
},async ctx=>{
  activations++;
  if(activations===1){
    assert(ctx.performance.memo('undefined','key',()=>{performanceComputes+=1;return undefined;})===undefined,'memo must preserve undefined results.');
    assert(ctx.performance.memoWeak('null',{},'key',()=>{performanceComputes+=1;return null;})===null,'memoWeak must preserve null results.');
    assert(ctx.performance.stage('undefined-stage',1,'key',()=>{performanceComputes+=1;return undefined;})===undefined,'stage must preserve undefined results.');
    assert(ctx.performance.measure('undefined-measure',()=>{performanceComputes+=1;return undefined;})===undefined,'measure must preserve undefined results.');
  }
...
P.define({
  id:'test.project-slice-reset',pluginType:'extension',name:'Project Slice Reset',version:'1.0.0',enabled:true,apiVersion:'1.19.0'
},async ctx=>{
...
P.define({
  id:'test.default-off',pluginType:'extension',name:'Default Off',version:'1.0.0',enabled:false,apiVersion:'1.19.0'
},async ctx=>{
...
P.define({
  id:'test.error',pluginType:'extension',name:'Broken',version:'1.0.0',enabled:false,apiVersion:'1.19.0'
},async ctx=>{
...
P.define({
  id:'test.system',name:'System Foundation',version:'1.0.0',enabled:true,apiVersion:'1.19.0',pluginType:'foundation',source:'builtin',systemCritical:true
},async()=>({}));

P.define({
  id:'test.windowed',name:'Windowed',version:'1.0.0',enabled:true,apiVersion:'1.19.0',pluginType:'task',
  window:{activity:'windowed',prewarm:false,reuse:true,persistence:'project'}
},async()=>({}));

P.define({
  id:'test.duplicate',pluginType:'extension',name:'Duplicate provider',version:'1.0.0',enabled:false,apiVersion:'1.19.0'
},async ctx=>{
  ctx.registry.add('analysis.providers','stateful',{id:'stateful'});
  return {};
});
...
(async()=>{
  await P.activateAll();
  assert(sandbox.performanceRuntimeCalls===4,'Performance facade must call each Core method once.');
  assert(performanceComputes===4,'Nullish compute results must not cause a second execution.');
