...
const Semver=require(path.join(root,'desktop','semver-compat'));
...
const pkg=normalizePluginPackage({schema:1,manifest:{id:'demo.algorithm-provider',name:'Demo Algorithm Provider',version:'2.0.0',apiVersion:'1.19.0',entry:'plugin.js',requiresCore:['analysis.algorithms'],algorithmProvider:true,algorithmCategories:['peak-metrics'],algorithmProvides:[{category:'peak-metrics',id:'demo.fwhm',version:'1.0.0',title:'Demo FWHM'}],compatibility:{app:'>=3.50.0 <4.0.0',pluginApi:'^1.19.0'},pluginDependencies:[{id:'demo.dependency',range:'^2.0.0'}]},files:{'plugin.js':'(()=>{})();'}});
...
let out=Catalog.catalog(packages,{category:'peak-metrics',id:'demo.fwhm',version:'1.0.0'},{appVersion:'3.55.0',pluginApiVersion:'1.19.0',installedVersions:{'demo.dependency':'2.3.0'}});
...
out=Catalog.catalog(packages,{category:'peak-metrics',id:'demo.fwhm',version:'1.0.0'},{appVersion:'3.55.0',pluginApiVersion:'1.19.0',installedVersions:{'demo.dependency':'1.9.0'}});
...
const main=fs.readFileSync(path.join(root,'desktop/main.js'),'utf8');assert(main.includes("ipcMain.handle('plugins:algorithmCatalog'"));assert(main.includes('pluginInstallPlan(')&&main.includes("code:'PLUGIN_INCOMPATIBLE'"));assert(main.includes("assertPackageCompatible(plan.manifest,'rollback')"));
