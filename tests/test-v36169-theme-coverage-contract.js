...
assert.equal(C.version,'1.0.0');assert.equal(Theme.version,'3.5.0');assert(Theme.supports('theme.coverage'));assert(Theme.supports('theme.legacy-style-audit'));assert(Theme.materialRoles().includes('floating'));
...
const runtime=read('src/core/theme/coverage-runtime.js');assert(runtime.includes("version:'2.2.0'"));assert(runtime.includes("id:'app-chrome'")&&runtime.includes("id:'scientific-floating'")&&runtime.includes("role:'floating'")&&runtime.includes('style[data-plugin-id]'));
const themeRuntime=read('src/core/theme/runtime.js');assert(themeRuntime.includes('coverage:()=>globalThis.DKDSThemeCoverage'));
const kernel=read('src/core/plugin-kernel.js');assert(kernel.includes('coverage: () => window.DKDSTheme?.coverage?.()'));
...
const css=read('src/styles/modern/92-material-roles.css');for(const role of ['chrome','sidebar','surface','elevated','popover','control','floating'])assert(css.includes(`--dkds-material-role:${role}`));
...
const materialRuntime=read('src/core/theme/material-renderer.js');for(const selector of ['.topbar','.left-panel','.dkds-plugin-workspace','.dkds-table-surface-host','.dkds-dialog-shell','.command-menu','button,input,select,textarea','.dkds-scientific-nav-tools','.floating-panel'])assert(materialRuntime.includes(selector),`runtime material-role authority missing selector: ${selector}`);
assert(!/(?:\.pulse-|\.ter-|\.respar-|\.reswin-|\.dc-|\.dkds-vth-|\.dksmb-|\.dkai-)/.test(materialRuntime.match(/const ROLE_BINDINGS=[\s\S]*?\]\);/)?.[0]||''),'Core material-role mapping must never target plugin identity selectors.');
const infra=read('src/core/ui-infrastructure.js');for(const cls of ['dkds-analysis-left dkds-material-role-sidebar','dkds-analysis-right dkds-material-role-sidebar','dkds-analysis-main dkds-material-role-surface','dkds-plugin-canvas-left dkds-material-role-sidebar','dkds-plugin-canvas-center dkds-material-role-surface'])assert(infra.includes(cls),`Core PluginWorkspace must assign material role: ${cls}`);
...
const manager=read('src/core/plugin-manager-ui.js');assert(manager.includes('coverage=window.DKDSTheme?.coverage?.()?.summary')&&manager.includes('Coverage ${coverage.managed}/${coverage.areas}'));
const automation=read('src/core/automation-test-runtime.js');assert(automation.includes("'ui.theme-coverage'")&&automation.includes('Core Theme coverage incomplete'));
