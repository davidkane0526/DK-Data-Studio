...
assert(adapters.includes('activeDrawerFrame?.contains?.(event.target)'),'Outside-dismiss must test the entire Drawer frame, including its resize rail.');
{
  const prior={window:global.window,document:global.document,KeyboardEvent:global.KeyboardEvent};
  const listeners={};
  const frame={contains(target){return target===handle;}};
  const activeDrawer={contains(){return false;},closest(selector){return selector.includes('dkds-mobile-surface-frame')?frame:null;},getAttribute(name){return name==='data-dkds-workspace-surface-id'?'parameters':'';}};
  const handle={closest(selector){if(selector.includes('dkds-mobile-drawer-resize')||selector.includes('data-dkds-touch-gesture-owner'))return handle;return null;}};
  global.window={DKDSPlugins:{activities:{active:()=> 'resonance'}}};
  global.document={addEventListener(type,fn){listeners[type]=fn;},querySelector(selector){return selector.includes('dkds-mobile-frame-region="drawer"')?activeDrawer:null;}};
  ...
  listeners.pointerdown({isPrimary:true,pointerType:'touch',pointerId:1,clientX:400,clientY:200,target:handle,cancelable:true,preventDefault(){},stopPropagation(){}});
...
// segmented selection actions and a flex-filling artifact list.
...
assert(!split.includes("require('./mobile-split-performance')")&&!split.includes('__dkdsMobilePreviewTotal'),'Mobile preview optimization must remain an external adapter rather than being injected into the base SplitController implementation.');
const mobileSplit=read('src/core/ui/modules/layout/mobile-split-performance.js');
assert(mobileSplit.includes("root?.dataset?.dkdsHost==='mobile'")&&mobileSplit.includes("react-native-client"),'Split preview optimization must be hard-gated to the native Mobile host.');
assert(mobileSplit.includes('this.__dkdsMobilePreviewTotal=Number.isFinite(total)&&total>0?total:null'),'Mobile split drag must cache container geometry once at drag start.');
assert(mobileSplit.includes('if(Number.isFinite(raw))this.previewSize=raw')&&mobileSplit.includes('if(this.previewFrame)return'),'Raw pointermove must only queue the latest size; clamping/layout occurs once in rAF.');
assert(mobileSplit.includes('if(!this.previewActive||!Number.isFinite(total)||total<=0)return baseLimits()'),'Cached limits must only replace layout reads during an active Mobile drag.');
...
assert(workbench.includes('enhanceMobileSplitController(this.canvasBottomSplit)'),'Plugin canvas splits must opt into the Mobile-only preview adapter.');
