...
const {PortableView}=load('src/core/ui/modules/layout/portable-view.js',{'../foundation/shortcuts':{},'../interaction/context-actions':{},'./docking':{},'ui/style-ownership-gate':gate});
...
stored={artifacts:[{...artifact,values:[9,2]}]};await saver.saveProject({mode:'current'});assert.equal(saveCalls,1,'A real data change must be written once on explicit save');assert(/^3\.68\./.test(saveTab.savedProject.version));
