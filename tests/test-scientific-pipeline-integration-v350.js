...
const index=read('src/index.html'),runtime=read('src/plugin-window/runtime.js'),manager=read('desktop/plugin-window-manager.js'),kernel=read('src/generated/runtime/plugin-kernel.js'),contract=read('src/core/plugins/contract-runtime.js'),data=read('src/core/data/model.js'),automation=(read('src/diagnostics/automation-test-runtime.js')+read('src/diagnostics/automation-smoke-cases.js'));
...
assert(terPlugin.includes('"data.pipeline"')&&terService.includes("pipeline.register('ter-matrix'")&&terService.includes("pipeline.run('ter-matrix'")&&terService.includes('transforms.fieldStageId(transform.type)')&&terService.includes('pipeline.runSync(stageId,'),'TER must execute heavy TER asynchronously and generic scalar-field derivations through Core Scientific Pipeline.');
assert(resPlugin.includes('"data.pipeline"')&&resFeature.includes("pipeline.register('gate-analysis'")&&resFeature.includes("live.pipelineRuntime.run('gate-analysis'"),'Resonance gate analysis must execute through Core Scientific Pipeline.');
