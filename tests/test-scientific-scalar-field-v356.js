...
const resonanceData=read('src/plugins/resonance-workbench/feature-data-runtime.js');
const resonanceTerRuntime=read('src/plugins/resonance-workbench/feature-ter-runtime.js');
...
vm.runInContext(resonanceData,context,{filename:'feature-data-runtime.js'});
vm.runInContext(resonanceTerRuntime,context,{filename:'feature-ter-runtime.js'});
...
(async()=>{
  ...
  const runtime=await feature.createTop({project:{format:'dk-data-studio-project',schemaVersion:3,dataModel:{schema:2,artifacts:[]},plugins:{'builtin.resonance-workbench':{workspace:{datasetMeta:[{path:'dataset',name:'dataset',vg:0}],peaks,gateAnalysisSettings:{featureMetric:'fwhm',featureDirection:'all'}}}}},artifacts:{list:()=>[{id:'dataset-table',dataset}]},science:context.DKDSScience,dataModel:context.DKDSData,algorithms,pipeline,setStatus(){},scheduleSnapshot(){},copyTextToClipboard(){},saveChartImage(){}});
  // Metric providers are task/async-backed on current releases. The first
  // projection schedules missing peak metrics; a later projection consumes
  // the resolved cache without restoring the retired main-thread fallback.
  runtime.service.getGateFeatureField();
  const deadline=Date.now()+1000;
  while(providerRuns<4&&Date.now()<deadline)await new Promise(resolve=>setTimeout(resolve,5));
  ...
  assert(providerRuns>=4,'Task-backed peak-metrics Provider must populate the feature-field cache before the resolved projection.');
