...
const pkg=json('package.json'),plugin=read('src/plugins/connectivity-center/plugin.js'),manifest=json('src/plugins/connectivity-center/plugin.json'),html=read('src/index.html'),app=read('mobile/App.tsx'),shell=read('mobile/src/Shell.tsx'),runtime=read('src/core/host/mobile-host-runtime.js');

assert(/^1\.2\.(?:[4-9]|[1-9]\d+)$/.test(manifest.version),'SMB & AI Services must retain the v1.2.4+ visual-ownership baseline.');
...
console.log('v3.61.54 SMB/import and AI/MCP plugin-management navigation regression passed.');
