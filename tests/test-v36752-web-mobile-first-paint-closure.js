...
assert(workbench.includes("owned.dataset.dkdsPrimeOwned='1'")&&workbench.includes("owned.dataset.dkdsPrimeContentInset=contentInset")&&workbench.includes("this.markSurfaceNode(owned,row,'prime')"),'Existing PRIME nodes must receive ownership, inset and semantic role synchronously during registration.');
...
const terShared=read('src/plugins/ter-analysis/unit-presentation.js');
...
assert(pulseUnit.includes("variant:'stack-comfortable'"),'Pulse Designer Drawer content must retain Unit-owned breathing room without plugin Mobile CSS.');
...
const pulseShared=read('src/plugins/pulse-analysis/unit-presentation.js');
assert(/dataset:\{[^}]*dkdsMobileDensity:'compact'[^}]*\}/.test(pulseShared)&&nativeShell.includes('[data-dkds-mobile-density="compact"]{')&&nativeShell.includes('--dkds-mobile-control-min-height:30px;')&&nativeShell.includes('--dkds-generic-button-min-height:30px;'),'Pulse Analysis must request compact native density semantically and let the Core platform feed generic density slots instead of raw control geometry.');

for(const rel of ['src/styles/platform/native-workspace-presentation.css','src/styles/platform/native-client-shell.css','src/plugins/pulse-analysis/mobile.css'])
