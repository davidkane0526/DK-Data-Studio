...
const surface=read('src/generated/runtime/ui-infrastructure.js');
...
assert(dc.includes("id:'dcDataActionsBtn'")&&dc.includes("label:'编辑 ▾'"),'Data Center Unit action button must preserve the accepted 编辑 label.');
const index=read('src/index.html'),css=readCoreCss(root);
assert(index.includes('class="toolbar-group system-core-tools-group dkds-segmented-command-group"')&&css.includes('.dkds-segmented-command-group>.toolbar-btn,.dkds-segmented-command-group>.menu-anchor>.toolbar-btn'),'Data Management, Tools and Software Management must consume the same canonical segmented command group as Import/Save/Export.');
