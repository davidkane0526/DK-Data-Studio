...
assert.equal(contract.minimumAppVersion,'3.65.0','Current SDK minimum app must be the v3.65 Theme semantic-core baseline.');
...
const audit=inspectWorkspaceStyles({apiVersion:'1.18.0',pluginType:'tool',workspace:{role:'top'},styles:[{name:'bad.css',content:'.pulse-shell{overflow:hidden}.pulse-main{display:grid;grid-template-rows:minmax(250px,1fr)}'}]});
assert(audit.errors.some(x=>x.includes('overflow:hidden')),'SDK 1.18 layout lint must reject clipped semantic UI');
assert(audit.errors.some(x=>x.includes('positive-pixel')),'SDK 1.18 layout lint must reject positive-pixel minmax(...,1fr) workspace rows');
const ordinaryGrid=inspectWorkspaceStyles({apiVersion:'1.18.0',pluginType:'tool',workspace:{role:'top'},styles:[{name:'ordinary.css',content:'.settings-options{display:grid;grid-template-rows:minmax(120px,1fr)}'}]});
...
assert.equal(inspectWorkspaceStyles({apiVersion:'1.18.0',pluginType:'tool',workspace:{role:'top'},styles:[{name:'minimum.css',content:'.tool-main{min-height:100vh}.tool-panel{min-height:420px}'}]}).errors.length,0,'min-height must not be misclassified as host viewport ownership or a fixed-height violation');
assert.equal(inspectWorkspaceStyles({apiVersion:'1.18.0',pluginType:'tool',workspace:{role:'top'},styles:[{name:'text.css',content:'.tool-card-head>span{overflow:hidden;text-overflow:ellipsis}'}]}).errors.length,0,'layout lint must not reject harmless text truncation merely because an ancestor class contains card/panel semantics');
assert(inspectWorkspaceStyles({apiVersion:'1.18.0',pluginType:'tool',workspace:{role:'top'},styles:[{name:'internal.css',content:'.pulse-card{overflow:hidden}'}]}).errors.some(x=>x.includes('overflow:hidden')),'API 1.18 must reject clipping semantic card content.');

const {normalizePluginPackage}=require('../desktop/plugin-package');
const manifest={id:'com.example.layout-guard',name:'Layout Guard',version:'1.0.0',apiVersion:'1.18.0',pluginType:'tool',entry:'plugin.js',scripts:['plugin.js'],styles:['plugin.css'],requiresCore:['workspace','ui.activities','ui.top-workspace'],workspace:{role:'top',activity:'layout-guard'},window:{activity:'layout-guard',reuse:true}};
...
assert.throws(()=>normalizePluginPackage(pkg,{allowBuiltinId:false}),/Plugin layout contract failed/,'application install path must enforce the same API 1.18 layout contract even when standalone validation was skipped');
...
try{
  ...
  assert(failed,'standalone SDK validator must fail an unsafe API 1.18 workspace before packaging');
}finally{fs.rmSync(temp,{recursive:true,force:true});}

const api=read('sdk/plugin-api.d.ts');
assert(api.includes("readonly apiVersion:'1.18.0'")&&!api.includes("'1.15.0'|'1.16.0'"),'editor manifest declaration must expose only the Plugin API 1.18 breaking contract');
for(const token of ["readonly apiVersion:'1.18.0'",'DKDSDomRuntime','DKDSStatusBarRuntime',"primaryScroll?:'safe'|'auto'|'contained'",'hardMinHeight?:number','layoutDiagnostics()'])assert(api.includes(token),`editor SDK declaration missing ${token}`);
