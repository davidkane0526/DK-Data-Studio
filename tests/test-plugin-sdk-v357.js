...
assert(contract.pluginApiVersion==='1.17.0','SDK must target Plugin API 1.17.0.');
...
assert(sdkTypes.includes("apiVersion:'1.17.0'")&&sdkTypes.includes('DKDSPluginContext'),'SDK must ship editor-readable Plugin API declarations.');
...
for(const name of ['workspace-plugin','top-workspace-plugin','algorithm-provider']){
  ...
  assert(pkg.manifest.apiVersion==='1.17.0'&&pkg.manifest.source==='external',`${name} SDK package must be installable by the application normalizer.`);
...
assert(packages.includes('node sdk/tools/dkds-plugin.js validate')&&packages.includes('"apiVersion": "1.17.0"'),'Plugin package guide must document the standalone v1.17 SDK workflow.');
