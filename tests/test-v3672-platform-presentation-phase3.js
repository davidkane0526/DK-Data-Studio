...
const portrait=presentation.present('mobile',{orientation:'portrait'});
assert.strictEqual(portrait.surfaces.find(row=>row.surfaceId==='main').presentation.region,'main');
assert.strictEqual(portrait.surfaces.find(row=>row.surfaceId==='data-control').presentation.region,'sheet');
assert.strictEqual(portrait.surfaces.find(row=>row.surfaceId==='curve-inspector').presentation.region,'sheet');
assert.strictEqual(portrait.surfaces.find(row=>row.surfaceId==='group-analysis').presentation.region,'route','semantic role must outrank legacy PRIME kind for mobile placement');
const landscape=presentation.present('mobile',{orientation:'landscape'});
assert.strictEqual(landscape.surfaces.find(row=>row.surfaceId==='data-control').presentation.region,'rail');
assert.strictEqual(landscape.surfaces.find(row=>row.surfaceId==='curve-inspector').presentation.region,'rail');
assert.strictEqual(landscape.surfaces.find(row=>row.surfaceId==='physics').presentation.region,'route');
delete global.window;

console.log('v3.67.2 Platform Presentation Phase 3 PASS: Resonance uses one semantic surface contract, Desktop host surface chrome is Presenter-owned, and Mobile maps the same surfaces to main/sheet/rail/route.');
