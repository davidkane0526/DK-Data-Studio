...
assert(ter.includes('ctx.ui.scientificPlot.react')&&ter.includes('ctx.ui.scientificPlot.scalarField'),'TER Feature Runtime must be the sole Core ScientificPlot owner.');
assert(!read('src/plugins/ter-analysis/analysis-service.js').includes('charts.react')&&!read('src/plugins/ter-analysis/analysis-service.js').includes('charts.scalarField'),'TER analysis service must remain presentation-free.');
...
assert(dataCenter.includes("ctx.modules.require('chart-runtime')")&&dataCenterChart.includes('ctx.ui.scientificPlot.react'),'Data Center delegated chart preview must use Core ScientificPlot.');
for(const [name,source] of [['resonance',resonance],['ter',ter],['data-center',`${dataCenter}\n${dataCenterChart}`]])assert(!source.includes('window.Plotly.newPlot')&&!source.includes('window.Plotly.react'),`${name} must not bypass Core with direct window.Plotly rendering.`);
