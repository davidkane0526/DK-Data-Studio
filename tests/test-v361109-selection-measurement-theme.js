...
assert(curve.includes('dkds-scientific-width-band')&&curve.includes("widthSpec.provisional?' is-provisional':''"),'Core ScientificCurve must render the FWHM/measurement width band, including provisional width presentation.');
assert(curve.includes('if(widthSpec.measureLine!==false)'),'Core ScientificCurve must keep the exact half-height line/crossings conditional so provisional masks do not fabricate FWHM.');
...
assert(materialRenderer.includes(':where(.dkds-legend-item,.dkds-plot-legend-item)')&&materialRenderer.includes('box-shadow:none'),'GroupPlot legend items must remain shadow-free through the shared Core Material Renderer contract.');
