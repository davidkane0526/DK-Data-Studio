...
assert(split.includes('this.previewFrame=raf(()=>{this.previewFrame=0;this.paintPreview();')&&mobileSplit.includes('this.__dkdsMobilePreviewTotal')&&mobileSplit.includes('if(Number.isFinite(raw))this.previewSize=raw')&&analysis.includes("if(document.documentElement?.classList?.contains('dkds-split-drag-active'))return;this.syncRegions()"),'Split drag must remain frame-coalesced; native Mobile caches drag geometry in its platform adapter while expensive workbench/chart resize paths stay frozen until commit.');
