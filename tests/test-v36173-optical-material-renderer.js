...
assert.equal(json('package.json').version,'3.61.85');assert.equal(json('sdk/contract.json').sdkVersion,'1.17.16');assert.equal(json('sdk/contract.json').themeContractVersion,'3.5.0');
const runtime=read('src/core/theme/material-renderer.js');
for(const x of ["const VERSION='3.6.0'","clear','thin-glass','soft-glass','liquid-glass","globalThis.DKDSTheme?.recipePolicy?.()","renderer.liquidGlass","renderer.edgeRefraction","renderer.dynamicSpecular","renderer.profilePolicy","BROKEN_OPTICAL_RENDERER","prefers-reduced-motion","--dkds-optical-shift-x"])assert(runtime.includes(x),`missing optical renderer token: ${x}`);
...
const css=read('src/styles/modern/98-theme-material-renderer.css');for(const x of ['data-dkds-material-recipe="clear"','data-dkds-material-recipe="soft-glass"','data-dkds-material-recipe="liquid-glass"','::before','mask-image:radial-gradient','scale(1.012)','::after','--dkds-material-blur-strong','prefers-reduced-motion'])assert(css.includes(x),`missing optical CSS recipe: ${x}`);
...
const automation=read('src/core/automation-test-runtime.js');for(const x of ['Material Renderer 3.6','REAL_LIQUID_MATERIAL',"probeRecipe?.('liquid-glass','popover')","probeRecipe?.('thin-glass','popover')"])assert(automation.includes(x),`automation does not verify ${x}`);
