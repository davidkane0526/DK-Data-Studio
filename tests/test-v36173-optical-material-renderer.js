...
assert.equal(json('package.json').version,'3.61.94');assert.equal(json('sdk/contract.json').sdkVersion,'1.17.16');assert.equal(json('sdk/contract.json').themeContractVersion,'3.5.0');
...
const automation=read('src/core/diagnostics/automation-test-runtime.js');for(const x of ['Material Renderer 3.6','REAL_LIQUID_MATERIAL',"probeRecipe?.('liquid-glass','popover')","probeRecipe?.('thin-glass','popover')"])assert(automation.includes(x),`automation does not verify ${x}`);
