...
(async()=>{
  ...
  assert(/const\s*\{[^}]*\bnativeTheme\b[^}]*\}\s*=\s*require\(\'electron\'\)/s.test(main),'Desktop main process must import Electron nativeTheme regardless of additional Electron services.');
