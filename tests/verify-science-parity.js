...
const baselineRoot = process.env.DKDS_PARITY_BASELINE_DIR
  ...
  : path.join(root, 'tests', 'fixtures', 'science-baseline-v36158');

function readBaselineSource(relativePath) {
  const resolved = path.resolve(baselineRoot, relativePath);
  const relative = path.relative(baselineRoot, resolved);
  if (relative.startsWith('..') || path.isAbsolute(relative)) {
    throw new Error(`Baseline path escapes fixture root: ${relativePath}`);
  }
...
function loadBaselineModule(relativePath, context, cache) {
  ...
  const wrapper = vm.runInContext(
    ...
    context,
    { filename: `${baselineLabel}/${normalizedPath}` }
  );
...
console.log(`Scientific engine rewrite matches the preserved ${baselineLabel} for representative mature workflows.`);
