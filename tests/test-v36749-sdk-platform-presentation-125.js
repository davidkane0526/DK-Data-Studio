...
for(const token of ['manifest.platformPresentation?.desktop?.styles','manifest.platformPresentation?.desktop?.scripts','manifest.platformPresentation?.mobile?.styles','manifest.platformPresentation?.mobile?.scripts'])assert(packageScript.includes(token),`Packager must include ${token}.`);
