...
assert(a>3||(a===3&&(b>71||(b===71&&c>=75))),'v3.71.75+ Mobile Drawer hard-constraint geometry required.');
...
store.set('dkds.mobile.drawer-width.v12.resonance:data-control','720');
assert(Number.isNaN(presenter.savedDrawerWidth('data-control','ter:data-control')),'A historical Drawer-generation value must not contaminate another activity.');
...
// 5) Content-width ownership stays semantic: the Presenter reads the shared
// Unit constraint registry and never restores retired private overflow scanners.
assert(!presenterSource.includes('measureSurfaceOverflow(')&&!presenterSource.includes('normalizeShrinkableControls('),'Retired private DOM overflow scanners must stay removed.');
assert(presenterSource.includes('constraintDeficit(frame)'),'The Drawer fitter must use one generic Unit-constraint deficit path.');

// 6) Source ownership guard: this acceptance is intentionally narrow. Drawer
...
console.log(`v3.71.75 Mobile Drawer hard constraints PASS: v12 state, non-greedy controls, fixed gaps, label-independent width, first-valid responsive sizing, Inspector ${desktop}px -> ${mobile}px.`);
