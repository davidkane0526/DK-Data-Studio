...
runtime.bindView('legend',legend,{selector:'.item',itemKey:el=>el.dataset.key,focusKey,dimOthers:true,horizontalWheel:true,onActivate:({element})=>runtime.selection.select({type:'core.entity',id:`s-${element.dataset.key}`,ref:{entityId:`s-${element.dataset.key}`,datasetPath:element.dataset.key}},{source:'legend'})});
runtime.bindView('list',list,{selector:'.item',itemKey:el=>el.dataset.key,focusKey,itemVariant:'row',revealFocus:true});
runtime.selection.select({type:'core.entity',id:'s-b',ref:{entityId:'s-b',datasetPath:'b'}},{source:'curve'});
