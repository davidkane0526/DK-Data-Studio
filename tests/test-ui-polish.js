...
assert(css.includes('--dkds-shell-group-height:40px') && /\.global-commandbar\s+\.file-command-group\s*\{[^}]*height\s*:\s*var\(--dkds-shell-group-height,40px\)/i.test(css), 'file command group must consume the canonical shell-group outer height');
