...
assert(css.includes('--dkds-shell-group-height:38px') && /\.global-commandbar\s+\.file-command-group\s*\{[^}]*height\s*:\s*var\(--dkds-shell-group-height,38px\)/i.test(css), 'file command group must consume the canonical shell-group outer height');
...
assert(html.includes('class="system-core-tools-group" role="group"')&&/\.system-core-tools-group\s*\{[^}]*display\s*:\s*flex/i.test(css)&&/\.system-core-tools-group>\.toolbar-btn,\.system-core-tools-group>\.menu-anchor>\.toolbar-btn\s*\{[^}]*min-width\s*:\s*0/i.test(css), 'Data Management, Tools and Software Management must share one compact integrated system command group instead of reserving fixed button widths.');
