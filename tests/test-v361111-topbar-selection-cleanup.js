...
assert(components.includes('.activity-tab:not(.top-level-activity-tab)'),'Top-level workspace selectors must be excluded from the generic tab recipe.');
assert(componentRuntime.includes('.primary-activity-bar .activity-tab.top-level-activity-tab')&&componentRuntime.includes('toolbarAction:Object.freeze'),'Top-level workspace selectors must be classified as toolbar actions.');
assert(/top-level-activity-tab:is\(\.active,[^}]+box-shadow:var\(--dkui-selected-shadow\)/s.test(components),'Top-level active selector must keep the single centered semantic halo.');
assert(!/top-level-activity-tab:is\(\.active,[^}]+inset\s+0\s+-2px/s.test(components),'Top-level workspace selection must never receive the generic tab underline indicator.');
