...
assert(components.includes('--dkds-ca-action-shadow-active:var(--dkui-component-toolbar-action-shadow-active,var(--dkds-ca-action-shadow))')&&components.includes('box-shadow:var(--dkds-ca-action-shadow-active)'),'Active toolbarAction identity must consume the Theme 3.10 active-depth slot through canonical Component Appearance.');
...
assert(components.includes('[data-dkds-component-identity="toolbarGroup"]:not(:is(')&&components.includes('.dkds-floating-surface')&&components.includes('background:var(--dkui-component-toolbar-group-surface')&&components.includes('border:1px solid var(--dkui-component-toolbar-group-border'),'Non-Material ToolbarGroup must keep direct Component Appearance paint while Material-owning groups are excluded.');
...
assert(themeRuntime.includes('toolbarGroup:{surface:')&&themeRuntime.includes('contexts:{grouped:{shadow:')&&thinGlass.includes('components:{toolbarGroup:')&&thinGlass.includes('roles:{floating:{surface:')&&thinGlass.includes('roles:{floating:{contexts:{grouped:{shadow:'),'Core Default and Thin Glass must both declare contextual toolbar composition through Theme 3.10 rather than rebuilding DOM chrome.');
