...
assert(components.includes('--dkds-ca-action-state-shadow:var(--dkui-selected-shadow)')&&/data-dkds-component-identity="toolbarAction"[^\n]*:is\(\.active,[^}]+box-shadow:var\(--dkds-ca-action-state-shadow\)/s.test(components),'Active toolbarAction identity must keep the single centered semantic halo through the canonical resolved state shadow.');
