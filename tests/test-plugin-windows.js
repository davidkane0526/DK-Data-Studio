...
const os=require('os');
const vm=require('vm');
const {readBuiltinPluginWindows,listBuiltinPluginWindows,readPluginWindows}=require('../desktop/plugin-window-manager');
...
const shellRuntime=read('src/plugin-window/runtime.js'),shellLifecycle=read('src/plugin-window/lifecycle.js');
...
const resonanceTerTask=read('src/plugins/resonance-workbench/resonant-ter-task.js');
...
assert(shellLifecycle.includes('onActivityWillHide'),'Dedicated lifecycle owner must flush its project snapshot before a cached window is hidden.');
...
assert(shellLifecycle.includes('onActivityWillShow'),'dedicated lifecycle owner must relayout the scientific renderer when a prewarmed/cached window becomes visible.');
...
const resonanceFeatureGraph=[
  ...
  read('src/plugins/resonance-workbench/feature-ter-runtime.js'),
...
for(const marker of ['function renderInspection()','function renderGroup()','function renderPhysics()','function renderSpacing()','function renderGate()','analyzePhysicalFamilies','pairGateSeries']){
  assert(resonanceFeatureGraph.includes(marker),`Resonance feature module graph parity marker missing: ${marker}`);
}
assert(resonanceTerTask.includes('computeResonantTerForLabel'),'Resonance dedicated/SUPER parity must retain resonant TER through the Core-managed task worker.');
...
assert(pulseService.includes('if(source.analyzed && !item.result)reanalyzeIds.add(String(item.id))')&&pulseService.includes('if(!reanalyzeIds.has(String(item.id))||item.result)continue'),'Pulse restore must recompute only legacy projects without cached results, after current state/source hydration is complete.');
...
assert(read('package.json').includes('desktop/**/*'),'electron-builder files must include the desktop host tree.');
