...
assert(group.includes("return `${visibleIds.join(',')}##${peaks}`"),
  'Group data fingerprint must contain scientific data only; layout/orientation changes must not force data redraw.');
...
console.log('v3.68.36 group layout performance regression PASS: resize is resize-only, layout/data invalidation are separated, and identical grid apply calls are no-ops.');
