...
const coverage=read('src/core/theme/coverage-runtime.js');for(const x of ['renderStatus','brokenMaterial','rendererCapabilities','OPAQUE_MATERIAL_OCCLUSION'])assert(coverage.includes(x));
...
const css=read('src/styles/theme/material-renderer.css');assert(css.includes('backdrop-filter:blur(')&&css.includes('--dkds-material-renderer-version:"3.11.0"'));assert(!fs.existsSync(path.join(root,'src/styles/modern/99-theme-material-renderer-33.css')));
...
const contract=JSON.parse(read('sdk/contract.json'));assert.equal(contract.sdkVersion,'1.28.0');assert.equal(contract.themeContractVersion,'3.10.0');
