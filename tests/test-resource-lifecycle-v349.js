...
const lifecycle=read('src/plugin-window/lifecycle.js');
...
assert(lifecycle.includes("DKDSUI?.lifecycle?.('hidden'")&&lifecycle.includes("DKDSUI?.lifecycle?.('visible'"),'Dedicated TOP lifecycle owner must route hide/show through Core UI lifecycle.');
assert(lifecycle.includes("DKDSPerformance?.lifecycle?.('hidden'"),'TOP lifecycle owner must still contract Core scientific caches after UI suspension.');
