...
assert(resonance.includes("pipeline.register('gate-analysis'")&&resonance.includes('cacheLimit:6')&&resonance.includes("execution:'async'")&&resonance.includes('renderKey:`gate:'),'resonance gate analysis must delegate computation caching to the Core Pipeline while retaining stable plot render keys');
assert(kernel.includes('performance: Object.freeze({')&&kernel.includes("performanceCall('memoWeak'")&&kernel.includes('`${pluginId}.${String(namespace'),'plugins must receive a namespaced single-call Core performance scope');
