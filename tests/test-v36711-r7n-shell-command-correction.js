...
assert(read('package.json').includes('\"version\": \"3.67.11\"')&&index.includes('<span class=\"version\">v3.67.11</span>'),'R7N delivery must bump the application patch version and keep visible/runtime release identity synchronized.');
...
assert(index.includes('class="toolbar-group file-command-group dkds-segmented-command-group"')&&index.includes('class="system-core-tools-group dkds-segmented-command-group" role="group"'),'File and system command clusters keep their semantic DOM roles while sharing the same segmented visual contract.');
