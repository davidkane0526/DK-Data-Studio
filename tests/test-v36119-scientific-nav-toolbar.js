...
assert(json('package.json').version==='3.61.84','Application version must be 3.61.20.');
...
assert(css.includes('.dkds-scientific-nav-tools{position:absolute')&&css.includes('background:transparent;box-shadow:none'),'Scientific navigation shell must not own material shadow/background.');const material=read('src/styles/modern/98-theme-material-renderer.css');assert(material.includes('[data-dkds-material-recipe=\"thin-glass\"]')&&material.includes('--dkds-material-shadow'),'Core Material Renderer must own floating surface depth.');
