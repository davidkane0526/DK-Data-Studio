...
context.window.window=context.window;context.window.document=document;context.window.requestAnimationFrame=context.requestAnimationFrame;context.globalThis=context;context.self=context.window;
...
vm.runInContext(fs.readFileSync(path.join(root,'src/plugins/pulse-analysis/pulse-analysis-task.js'),'utf8'),context,{filename:'pulse-analysis-task.js'});

(async()=>{
  const statuses=[];
  const taskDefinition=context.window.DKDSTaskDefinition;
  assert(taskDefinition&&typeof taskDefinition.run==='function','Pulse task worker definition must load in repeatability test.');
  const tasks={submit:(taskId,payload)=>{assert.strictEqual(taskId,'analyze-pulse-read');return {promise:Promise.resolve().then(()=>taskDefinition.run(payload))};}};
  const waitFor=async predicate=>{for(let i=0;i<80;i++){if(predicate())return true;await new Promise(resolve=>setTimeout(resolve,1));}return false;};
  ...
  const runtime=await pulseAnalysis.create({
    science:Analysis,setStatus:s=>statuses.push(String(s)),copyTextToClipboard:()=>true,saveChartImage:()=>true,scheduleSnapshot:()=>{},artifacts,tasks
  });
  const file={
  ...
  assert(await waitFor(()=>runtime.service.getState().files[0]?.result||runtime.service.getState().files[0]?.error),'Canonical saved Pulse Artifact restore must settle through the Task Runner.');
  let item=runtime.service.getState().files[0];
  assert(item.result,'Canonical saved Pulse Artifact with analyzed=true must be re-analyzed during async restore.');
