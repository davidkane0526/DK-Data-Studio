...
assert(resonanceMainPlot.includes('const sweepVoltageBoundsCache=new WeakMap()')&&resonanceMainPlot.includes('bounds=sweepVoltageBounds(sw)'),'Resonance main-plot analysis-window commit must cache sweep voltage bounds.');
...
assert(resonancePeak.includes('function commitPeakMetricEdit')&&resonancePeak.includes('invalidatePeakMetric(p,{geometry,reason,refresh:false})'),'Interactive scientific commits must avoid creating an async metric placeholder before authoritative redraw.');
assert(resonanceMainPlot.includes('getManipulators:()=>manipulators()')&&resonanceMainPlot.includes("action:'peak-position'")&&resonanceMainPlot.includes('onManipulationCommit')&&!resonanceMainPlot.includes('onMarkerDragCommit:'),'Resonance main-plot adapter must consume the generic Core manipulation API instead of a peak/marker-named drag contract.');
assert(!resonanceMainPlot.includes("publishPeakSelection(p,'resonance-peak-drag")&&!resonanceMainPlot.includes("publishPeakSelection(p,'resonance-main-drag"),'Dragging a point manipulator must not mutate Selection/focus or dim unrelated curves.');
assert(resonanceMainPlot.includes("action:'analysis-window'")&&resonanceMainPlot.includes('p.analysisLeft=left;p.analysisRight=right;p.analysisManual=true')&&!resonanceMainPlot.includes('onWidthWindowCommit:'),'FWHM analysis-window editing must be a domain mapping of a generic atomic range commit.');
