...
assert.equal(json('sdk/contract.json').themeContractVersion,'3.7.0');
...
for(const token of ["const VERSION='3.7.0'","'thin-glass'",'DKDSMaterialSurface','OPAQUE_PARENT_OCCLUSION','ROLE_MISSING','RECIPE_MISSING','BACKDROP_FILTER_NONE','renderer.thinGlass']) assert(renderer.includes(token),`renderer missing ${token}`);
