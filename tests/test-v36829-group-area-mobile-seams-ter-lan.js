...
assert(d3Renderer.includes('if(px<m.l||px>m.l+state.innerW||py<m.t||py>m.t+state.innerH)return;')&&d3Renderer.includes('source.stopPropagation?.()'),'D3 wheel zoom must be captured only inside the actual XY plot rectangle; panel/title/axis margins must continue scrolling the parent GroupArea.');
...
console.log('v3.68.36 GroupArea / Mobile seam / TER parity / LAN shell regressions PASS.');
