...
assert(appearance.includes('const recomposeAll=event=>{if(event?.detail?.visualSynchronized)return;scheduleAppearance(document);}')&&appearance.includes("globalThis.addEventListener?.('dkds:theme-changed',recomposeAll)")&&!appearance.includes("globalThis.addEventListener?.('dkds:theme-profile-changed',recomposeAll)"),
  'R7I Theme Runtime must own synchronous profile/mode recomposition; Component Appearance may only schedule a fallback pass for unsynchronized external events.');
