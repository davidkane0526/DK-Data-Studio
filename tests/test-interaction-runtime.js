...
assert(Number(String(UI.version).split('.')[0])>=6,'interaction/resize runtime requires UI infrastructure v6+');
...
scope.dataTypes.register('demo.large',{title:'Large table-like result',parent:'result.analysis',kind:'result',key:v=>v.id,selection:v=>({id:v.id,ref:{entityId:v.id,resultId:v.id},meta:{label:v.label,rowCount:v.rows?.length||0}}),resolve:ref=>({resolved:ref.resultId})});
...
assert(compact.meta.rowCount===5000&&!('value' in compact),'data type selection projection must be reference-only');
...
assert(compactSnap.focus.ref.resultId==='huge-1'&&compactSnap.focus.meta.rowCount===5000&&!('value' in compactSnap.focus),'typed selection must store bounded references and metadata only');
...
runtime.region({id:'box',min:0,max:1},[{type:'demo.result',id:'r1',ref:{entityId:'r1'}},{type:'demo.result',id:'r2',ref:{entityId:'r2'}}],{rangeType:'data.range',source:'range-test',sourceRef:{entityId:'demo-range-source'}});
