...
const css=readCoreCss(root,['presentation','theme','motion']);
...
const platformGeometry=fs.readFileSync(path.join(root,'src','styles','platform','touch.css'),'utf8');
...
assert(scientificStructure.includes('--dkds-scientific-nav-item-width:28px')&&scientificStructure.includes('--dkds-scientific-nav-item-height:28px')&&scientificStructure.includes('--dkds-header-action-height:var(--dkds-scientific-nav-item-height)'),'Shared scientific navigation must keep the frozen semantic geometry contract');
assert(platformGeometry.includes('html[data-dkds-host="desktop"] .dkds-scientific-nav-tools')&&platformGeometry.includes('--dkds-scientific-nav-item-width:')&&platformGeometry.includes('--dkds-scientific-nav-item-height:'),'Desktop host geometry must apply a compact host-specific scientific floating-control contract without changing shared semantic structure');
...
assert(/@media\s*\(prefers-reduced-motion:reduce\)/.test(css),'short motion must include a reduced-motion fallback');
