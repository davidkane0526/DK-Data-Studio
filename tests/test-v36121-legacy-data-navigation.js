...
{const v=(automation.match(/const VERSION='(\d+)\.(\d+)\.(\d+)'/)||[]).slice(1).map(Number);assert(v.length===3&&(v[0]>1||(v[0]===1&&v[1]>=26))&&automation.includes("'project.resonance-groups'")&&automation.includes('buildTrendModel()'),'Windows automation must diagnose current-project Resonance group-data integrity on runner v1.26+.');}
