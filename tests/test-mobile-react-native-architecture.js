...
assert(mobileStyle.includes('#mainWorkspace') && mobileStyle.includes('display:none'), 'React Native must own phone command chrome and hide the desktop workspace shell');
...
assert(mobileStyle.includes('.dkds-plugin-canvas-right-resizer.active') && /dkds-plugin-canvas-right-resizer\.active,[\s\S]{0,220}display:none/.test(mobileStyle), 'mobile must hide full-screen split handles that previously rendered as a blue crosshair');
...
assert(mobileStyle.includes('#statusBar.statusbar{display:none}') && shell.includes('NativeStatusBar'), 'React Native must own the mobile status bar instead of stacking desktop status chrome');
...
assert(mobileStyle.includes('[data-dkds-mobile-summary]') && !/#reswinSummary|#terSummary|\.respar-status-row/.test(mobileStyle), 'space-consuming analysis summaries must leave renderer layout through the plugin-neutral mobile-summary contract');
assert(mobileStyle.includes('min-height:280px'), 'scientific plots must retain a usable phone viewport');
...
assert(!/\.pulse-|#pulse/i.test(mobileStyle), 'mobile Core CSS must not hard-code Pulse layout; domain geometry belongs to the plugin stylesheet');
