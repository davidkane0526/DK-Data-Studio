...
const nativeStyle = [nativeShellStyle,nativeSemanticStyle].join('\n');
...
assert(mobileHost.includes('currentOrientation')&&mobileHost.includes('DKDSPlatform?.profile?.orientation')&&mobileHost.includes('const currentViewport=()=>{const visual=window.visualViewport')&&mobileHost.includes("addEventListener('dkds:platform-change',publishSettledViewport)")&&mobileHost.includes("visualViewport?.addEventListener?.('resize',publishSettledViewport")&&mobileHost.includes('viewportSettleFrame=raf(()=>{viewportSettleFrame=raf(()=>{'), 'Mobile Host must publish platform orientation together with the settled live WebView visualViewport so contextual surfaces cannot reuse stale portrait geometry in landscape.');
