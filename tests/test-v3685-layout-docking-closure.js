...
assert(/const scientificPlot=this\.wrapper\.classList\.contains\('dkds-plot-view'\);[\s\S]*?else if\(this\.spec\.sizing!=='fill'&&!scientificPlot&&Number\(docked\.height\)>0\)/.test(portable),'Side-docked scientific plots must not restore a stale persisted card height over the dock flex contract.');
