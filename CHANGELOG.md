...
- Pulse keeps batch execution as a single Header-owned primary action: the existing `分析勾选` action is relabeled `批量分析勾选` and continues through the existing `analyzeChecked()` Task Runner path. The parameter PRIME remains data/configuration-only and does not duplicate primary actions.
