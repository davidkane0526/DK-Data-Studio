(() => {
  const manifest={
    id:'com.dkds.transfer-vth-lab',name:'Transfer Curve Vth Lab',version:'3.0.2',apiVersion:'1.18.0',entry:'plugin.js',scripts:['plugin.js'],styles:['plugin.css'],enabled:true,order:420,
  ...
  DKDSPlugins.define(manifest, async ctx => {
    ...
    workbench=ctx.ui.workspaceSurface.create(host,{header:false,activity:'transfer-vth-lab',primaryScroll:'safe',canvasLeftWidth:300});
    left=ctx.ui.dom.create('div',{className:'dkds-vth-sidebar'});main=ctx.ui.dom.create('div',{className:'dkds-vth-main'});
    left.innerHTML=`<section class="dkds-vth-card dkds-surface"><div class="dkds-vth-card-head dkds-surface-header"><h3>数据</h3><span class="dkds-vth-source-badge dkds-chip" data-vth="source-badge">—</span></div><div class="dkds-vth-field dkds-field"><label>当前曲线</label><select data-vth="curve"></select></div><p class="dkds-vth-hint dkds-note">数据导入由 Core 统一提供；这里只显示分配给 Vth 工作台的数据。</p><div class="dkds-vth-actions dkds-toolbar"><button type="button" data-vth="refresh">刷新</button><button type="button" data-vth="demo">示例</button></div></section><section class="dkds-vth-card dkds-surface"><h3>阈值提取</h3><div class="dkds-vth-field dkds-field"><label>方法</label><select data-vth="method"><option value="linear-window">恒流邻域线性回归</option><option value="interpolation">恒流插值</option></select></div><div class="dkds-vth-field dkds-field"><label>扫描段</label><select data-vth="branch"><option value="auto">自动</option><option value="first">第一扫描段</option><option value="second">第二扫描段</option><option value="all">全部数据</option></select></div><div class="dkds-vth-field dkds-field"><label>目标电流 / A</label><input data-vth="targetCurrent" type="number" step="any"></div><div class="dkds-vth-field dkds-field"><label>拟合下限 / A</label><input data-vth="lowCurrent" type="number" step="any"></div><div class="dkds-vth-field dkds-field"><label>拟合上限 / A</label><input data-vth="highCurrent" type="number" step="any"></div><label class="dkds-vth-check dkds-check"><input data-vth="absoluteCurrent" type="checkbox">使用 |I|</label><label class="dkds-vth-check dkds-check"><input data-vth="logY" type="checkbox">对数显示</label><label class="dkds-vth-check dkds-check"><input data-vth="showAllCurves" type="checkbox">显示全部曲线</label></section>`;
    main.innerHTML=`<div class="dkds-vth-metrics"><div class="dkds-vth-metric dkds-metric"><span>Vth</span><strong data-vth="metric-vth">—</strong></div><div class="dkds-vth-metric dkds-metric"><span>扫描段</span><strong data-vth="metric-branch">—</strong></div><div class="dkds-vth-metric dkds-metric"><span>R²</span><strong data-vth="metric-r2">—</strong></div><div class="dkds-vth-metric dkds-metric"><span>拟合点数</span><strong data-vth="metric-n">—</strong></div></div><section class="dkds-vth-plot-card dkds-surface"><div class="dkds-vth-plot-head dkds-surface-header"><strong data-vth="plot-title">转移曲线</strong><span data-vth="plot-status">未加载数据</span></div><div class="dkds-vth-plot-target" data-vth="plot"></div></section><section class="dkds-vth-results-host" data-vth="results"></section>`;
    workbench.mountPrimary({id:'vth-main',label:'Vth 分析',scroll:'safe',leftNode:left,mainNode:main});
    ...
    tableSurface=ctx.ui.tables.mount('vth-results',main.querySelector('[data-vth="results"]'),{persistKey:'transfer-vth-results',appearance:{density:'compact',stripe:'subtle'}});
