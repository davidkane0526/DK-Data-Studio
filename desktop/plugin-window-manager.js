...
function attachAlgorithmProviders(spec,providers=[]) {
  ...
  const selfProvider=spec?.algorithmProvider===true
    ?(providers||[]).find(row=>row.pluginId===spec.pluginId&&row.algorithmCategories.some(category=>wanted.has(category)))||null
    :null;
  // A TOP that is itself an Algorithm Provider is already loaded through its
  // normal target entry. Do not load its plugin script a second time as an
  // injected provider; keep only metadata so the renderer can validate and
  // report the algorithms registered by the target plugin itself.
  const matched=(providers||[]).filter(row=>row.pluginId!==spec.pluginId&&row.algorithmCategories.some(category=>wanted.has(category)));
  const dependencies=[...(spec.dependencies||[])];
  for(const provider of matched)for(const dependency of (provider.dependencies||[]))if(!dependencies.includes(dependency))dependencies.push(dependency);
  const selfAlgorithmProvider=selfProvider?Object.freeze({
    source:selfProvider.source,
    pluginId:selfProvider.pluginId,
    version:selfProvider.version,
    algorithmCategories:selfProvider.algorithmCategories
  }):null;
  return Object.freeze({...spec,algorithmCategories:categories,algorithmProviders:Object.freeze(matched),selfAlgorithmProvider,dependencies:Object.freeze(dependencies)});
}
