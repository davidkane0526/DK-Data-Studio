...
function createPluginPackageRuntime({app,BrowserWindow}){
  ...
  function readBuiltinPluginPackage(id){
    ...
    for(const name of fs.readdirSync(base).sort()){
      ...
      const referenced=new Set([manifest.entry,...(manifest.scripts||[]),...(manifest.styles||[]),...PlatformPresentation.referencedPlatformAssets(manifest),...(manifest.window?.runtime?[manifest.window.runtime]:[]),...(manifest.window?.scripts||[])]);
