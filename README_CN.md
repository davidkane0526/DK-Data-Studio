...
本版继续收口 Android/平板交互：SMB 暗色与扫描反馈、AI 对话窗标题栏、项目 `×` 删除与保存提醒、紧凑插件管理、常用数据/工程文件类型过滤、状态栏完整显示，以及更平滑的原生 Sheet 动画。工程在已有可写保存位置后支持空闲自动保存。

更重要的是，数据图吸附遮挡被下沉为 Core `GridController` 能力：sticky 图会真正保留一个网格列，其他数据图自动重新排布到剩余位置；单列布局则取消覆盖式 sticky。该能力同时适用于桌面和移动端，不再由 TER 插件维护专用避让逻辑。
...
本版不引入新功能或新架构，重点清理已确认不可达、无调用方、仅写不读或完全重复的代码与样式，并保持现有 D3 绘图、插件契约、科学计算和工程兼容行为不变。发布前完整回归通过后，将本地 Git 分支收敛为 `main` / `dev` 并执行对象压缩与垃圾回收，降低交付包中的 `.git` 体积。


## v3.61.41 D3 默认悬浮工具栏内边距与居中修正

- D3 数据图默认悬浮工具栏保持 **20 px** 按钮高度和 **23 px** 按钮宽度，拖动手柄宽度保持 **11 px**；外壳左侧内边距为 **2 px**，右侧内边距调整为 **4 px**。
- 按钮采用显式 flex 居中并清除继承阴影偏移，修正 hover 高亮视觉不居中的问题；SDK 仍为 **1.17.6**，Plugin API 仍为 **1.17.0**。
...
- 修复 Electron 主进程安装器仍硬编码 Plugin API `1.15.0` 的问题。安装器现在直接读取 `sdk/contract.json`，与 SDK / Renderer Core 共享同一个 Plugin API 版本来源。
- Plugin API `^1.16.0`、应用要求 `>=3.61.29 <4.0.0` 的外部插件可在 v3.61.31 正常通过兼容性检查。
- 插件安装改为“选择并校验 → DKDS 自绘确认 → 原子提交”，系统文件选择器只负责选文件，不再使用 Electron 默认 MessageBox。
- Core 新增统一 Dialog Runtime，提供 `alert / confirm / prompt`，支持亮暗主题、结构化版本信息、详细技术信息、键盘操作和统一现代视觉。插件安装失败属于阻断性错误，必须弹出自绘模态框；状态栏只保留简短记录。
- 插件管理器的安装、更新、卸载、版本回退和恢复默认确认均迁移到同一套弹窗。
...
本版本不改变科学算法、数据契约、工程格式或插件布局结构，重点将现有 UI 从“多轮局部覆盖”收敛为一套可持续维护的统一视觉系统。

- 新增 `src/ui-polish.css`，作为 Core 级最终视觉覆盖层，同时加载于主窗口与独立插件窗口。按钮、输入框、菜单、项目标签、浮动面板、卡片、表格、状态栏、插件管理、Data Center、Resonance、TER、Pulse 等共用一致的边框、圆角、阴影、选中态、焦点态和交互动效。
- 保留现有 DOM、TOP/SUPER/PRIME/SUB 排布、控件高度和工作区几何契约。视觉层不重新定义顶栏高度等关键布局尺寸，避免美化引入按钮挤压或工作区错位。
- 顶栏命令采用“组容器承担描边/层次，组内按钮保持轻量”的视觉层级，减少重复阴影和重复边框。
- Core Chart Runtime 统一 Plotly 字体、坐标轴文字、网格线、零线、图面背景、图例和 colorbar 字体，同时保持科学 trace 颜色、热图 colorscale 与数据表达不变。Plotly modebar、Core D3 工具条也统一到同一套控件风格。
- 增加短时 hover / menu / page 动效，并完整支持 `prefers-reduced-motion`。动效只使用 opacity / shadow / transform 等不参与文档流的属性，不改变控件排布。
- 增加 1180 px / 860 px 窄窗口防拥挤规则，操作区允许在必要时换行而不是缩成互相覆盖。云端静态渲染检查覆盖 1440×900、1100×760、820×900，页面与顶栏均未产生水平溢出。
- 新增 `test-ui-visual-system-v36123.js`，锁定主窗口/插件窗口共用视觉层、科学图表主题、动效降级和“不修改 Shell 核心几何”的约束。
...
```text
每个文件 = 一条 Core Scientific Series
```
...
网页版复用同一套 D3 Scientific Renderer / Analysis / app.js，支持：
- 数据导入工作台
- 多项目
- 寻峰
- 框选与峰编辑
...
- 组图子图双击放大后继续使用 Core 统一缩放/复位交互，并导出 SVG/PNG。
...
- 每个小图使用自己的 Core 自动图例，不再把多个 Surface 合并为一个公共图例；
- 图例由 Scientific Presentation 自动选择紧凑的顶部/底部位置并保持数据区可读；
- 小图使用自适应列布局，图本身占据更多面积；
- **双击任意小图**会打开可移动、可缩放的放大面板；
- 放大图继续使用 Core 统一的缩放、复位、选择与导出交互。
...
- 组图中的每个小图双击可在独立悬浮面板放大，并提供 Core 统一缩放/复位操作。
